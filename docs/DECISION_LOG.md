...
### 2026-06-19 · Toward a higher-tier Jake — model-agnostic brain seam + agent loop
- **Reason:** Owner wants a top-tier agent (remembers + controls + completes tasks autonomously) and asked specifically about routing to a stronger brain (qwen3:14b local) or Kimi K2 (API). Goal also = build ERP-grade agentic experience.
- **Brain seam:** Jake's `chatWithLocalModel`/`forceActions` now run on `JAKE_MODEL` (`VITE_JAKE_MODEL`, default = aya), independent of the frozen engine's aya, with a qwen3-aware no-think switch. Enables swapping qwen3:14b / Kimi K2 / Claude via an OpenAI-compatible endpoint. **Measured (aya vs qwen3:14b on Jake's 3 core jobs):** both pass action-JSON, count, history-from-log; qwen3:14b is 3–5× slower with no quality gain on simple tasks → **kept aya as the fast default**; reserve the stronger brain for the agent loop's planning step. (Note: Kimi K2 is ~1T-param MoE — API-only, cannot run on the 16GB local GPU.)
- **Agent loop:** After the first action turn, for multi-step requests (`isMultiStep` gate) Jake loops plan→act→**observe** (executeActions now returns `nextData`)→repeat, until DONE / no-new-action / no-progress / `MAX_AGENT_STEPS=3`. Guards: `actionSig` dedup (never repeat a step), pause on deletes, single-shot commands skip the loop. **Verified:** build clean; algorithm probe vs real aya on a 3-step task (all done, 0 dup violations, clean DONE); live in-app — a 2-step command created both clients + logged both (~15s, no errors).
- **Scope:** Jake orchestration only. Frozen Creative Director generation pipeline untouched.
- **Next (backlog):** route the loop's planning step to the stronger brain on hard tasks; persist activity log + an Activity UI feed; proactive briefings.

### 2026-06-26 · Jake gen-2 unification (owner-directed) — propose→confirm→execute + smartest-brain router + drafting + graceful degradation + Demo Mode
- **Reason:** After Jake was extracted from Art Value and embedded in 6 other systems on the improved "gen-2" pattern (propose→confirm→execute, cloud brain, drafting), the mother system was left on gen-1. Owner directed unifying Art Value's Jake with the fleet — explicitly scoped to **Jake/orchestration only**, with the Creative Director **generation** pipeline kept frozen (same boundary as the 2026-06-19 work).
- **Changes (all additive / Jake-only):** (1) **Smartest-brain router** in `gemini.js` — `chatJake`/`forceActionsJake`/`draftWithJake`/`jakeBrainLabel`/`jakeBrainPref`/`setJakeBrain`. Prefers cloud Gemini (own model `VITE_JAKE_CLOUD_MODEL`, default gemini-2.5-flash) when a key exists, else local Ollama (`JAKE_MODEL`), with **cross-fallback** and a demo fallback — the user never sees a raw error. Switchable live from the assistant header. (2) **propose→confirm→execute** — `Assistant.jsx` `send()` rewritten: the model PROPOSES; `describeActions` (jakeAgent.js) renders a Hebrew approval card with the resolved entity; `executeActions` runs only on "אשר ובצע". Deletes still get their own second confirm; the deterministic bulk-delete code gate is unchanged. (3) **Drafting lane** — `isDraftRequest` → `draftWithJake` writes letters/WhatsApp/email from real data, prose only. (4) **Graceful degradation** — removed the raw `e.message` leak; `gentleError` everywhere. (5) **Confirm-mode persona** — `confirmGuide`/`draftingGuide` added to the pack; phrasing shifted to proposals. (6) **Demo Mode** — `DemoMode.jsx`, a guided in-app tour of the whole platform with live ג'יק examples (via `jake:ask`/`jake:open` events), launchable from the Dashboard. Removed the auto-executing agent loop (incompatible with per-action confirm; strong brains emit multi-action batches in one proposal).
- **Scope (strictly):** `gemini.js` (additive Jake router — `chatJson`/`MODEL`/`CREATIVE_LLM_MODEL`/`runCreativeDirector` and all generation stages **untouched**), `jakePack.js`, `jakeAgent.js` (added pure `describeActions`), `Assistant.jsx`, new `DemoMode.jsx`, `App.jsx`/`Dashboard.jsx` (mount + launcher), CSS. Backup branch+tag `pre-jake-gen2-20260626`.
- **Verified:** `vite build` clean (1295 modules). Live end-to-end on the local aya brain: action command → proposal card (`➕ להוסיף לקוח: דני כהן · 3,000 ₪ · lead`) → approve → client actually added to the store (0→1) with a ✓ log; nothing executed before approval. Deterministic briefing/info lanes intact. Test data cleaned up.
- **Outcome:** Art Value unified to gen-2; 7th embedding (cantor / מערכת החזן) added to the JakeConsole fleet; roadmap doc redeployed. Creative Director generation pipeline remains frozen.

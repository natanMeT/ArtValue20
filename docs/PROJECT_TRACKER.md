...
**Last updated:** 2026-08-06 — session: **ACCESSIBILITY A6 — APPOINTMENTMODAL'S EIGHT LABELS NAME THEIR CONTROLS — RELEASED / LIVE IN PRODUCTION.** **Production `d7b9469b` / `index-DryJ81XU.js`**, Environment Production, branch `main`, source **`d5d86b9420b17ae17e8c7a098a9ec8d43ac607b9`**, 799,495 → **799,915 B** (+420), entry-bundle SHA256 `9E033ED588A9FBE9EB00AE2D381E7AAD5325C22CF1A845F72F543F6E45E4346F`. Promoted UNCHANGED from the owner-QA'd Preview `0470d5b0` ("Uploaded 0 files (12 already uploaded)"); canonical Production ↔ local `dist/` **12/12 byte-identical**, deployment URL independently 12/12; smoke GET only, 0 writes. **Frontend only — 2 files, no migration, no bucket, no Storage policy, no grant, no Edge, no package, no secret, `src/lib/api.js` UNTOUCHED.** Current rollback target advances to **`47497fdb`** / `index-cQ2OpZHP.js`; tag `pre-a11y-a6-appointment` @ `91f26a88` pushed **before** the deploy. **Owner QA: 8/8 label-click → focus, ZERO writes.** *(The previous value of this line is preserved in the JAKE QUOTE DETAIL box below.)* Previous session: **JAKE QUOTE DETAIL — QUOTES REACH JAKE AS DETAIL, NOT ONLY A PENDING COUNT — RELEASED / LIVE IN PRODUCTION.** **Production `47497fdb` / `index-cQ2OpZHP.js`**, Environment Production, branch `main`, source **`5b63f6d10e50daebdaa8481b3d52cdb9b8c04ae8`**, 797,585 → **799,495 B** (+1,910), entry-bundle SHA256 `A4BA90F34DEA4D2DC0A32FBF6128DE8B4446F916B8331F87AD2475348F8A3363`. Promoted UNCHANGED from the owner-QA'd Preview `91705297` ("Uploaded 0 files (12 already uploaded)"); canonical Production ↔ local `dist/` **12/12 byte-identical**; smoke **19/19 HTTP 200, GET only, 0 writes**. **Frontend only — no migration, no bucket, no Storage policy, no grant, no Edge, no package, no secret, `src/lib/api.js` UNTOUCHED.** Current rollback target advances to **`024c0aa7`** / `index-FzGUTjjq.js`; tag `pre-jake-quote-detail` @ `68aeba0b` pushed. *(The previous value of this line is preserved in the JAKE PRE-SETTLE TRUTHFULNESS box below.)*
...
> ### ✅ ACCESSIBILITY A6 — APPOINTMENTMODAL'S EIGHT LABELS — **CLOSED / LIVE IN PRODUCTION** (consultation → owner-approved spec → one merge → Preview + owner QA on the isolated account → promotion of the same artifact)
>
...
## Baseline (current — ACCESSIBILITY A6 CLOSED / LIVE in Production, deployment `d7b9469b`)
...
- **Active application release code anchor:** **`d5d86b9420b17ae17e8c7a098a9ec8d43ac607b9`** (PR #214 merge — Accessibility A6, AppointmentModal's eight labels associated with their controls; the running Production artifact was built from this commit and carries every earlier slice's behaviour forward). ⚠️ **The 2026-08-05 correction that first fixed this line still stands, and it was needed AGAIN on 2026-08-06:** it had been left at `8882461731c90f06fca0fc7fb44b5f65e8e20873` (PR #191) through several releases, and the Layer A checker reads `live_src` off THIS EXACT PHRASING (`checks/tracker_status.py` rule 6), so a stale value here makes every CURRENT-labelled block downstream look like it names a superseded release. *(Historical anchors, retained only as the sources of older deployments: `a48020f23d0947a92df0b6da729bdc52924e00cb` — Jake Pre-Settle Truthfulness, now the **current rollback target's** source; `3681d77ef51162f53782ff32119538e84130536c` — Jake Asset Library Context, now a historical fallback's source; `45243b53` — Asset Library slice 4; `521b7a0b` — Jake Campaigns Context; `e3555eed` — Jake sees the calendar; `8882461731c90f06fca0fc7fb44b5f65e8e20873` — PR #191; `c138f852` — Accessibility slice A5.)*
- **Three DISTINCT anchors — never collapse them.** This tracker records no fixed value for the repository head, because documentation merges advance it continuously and any pinned value would be false within minutes:
  ...
  3. **Deployed Production source:** **`d5d86b9420b17ae17e8c7a098a9ec8d43ac607b9`** — the commit the running Production artifact was actually built from (Accessibility A6 — AppointmentModal labels, deployment `d7b9469b`). The prior `5b63f6d10e50daebdaa8481b3d52cdb9b8c04ae8` (Jake Quote Detail, deployment `47497fdb`) is now the **current rollback target's** source. The older `a48020f23d0947a92df0b6da729bdc52924e00cb` (Jake Pre-Settle Truthfulness, deployment `024c0aa7`) is now a **historical fallback's** source — **DEMOTED 2026-08-06 by this release** — as is `3681d77ef51162f53782ff32119538e84130536c` (Jake Asset Library Context, deployment `0a2669bb`). ⚠️ **CORRECTED 2026-08-06 AFTER STANDING FIVE RELEASES STALE.** This line read `45243b53…` / deployment `378115bc` (Asset Library slice 4) from 2026-08-05 through the Jake Campaigns Context, Jake Asset Library Context, Jake Pre-Settle Truthfulness and Jake Quote Detail releases. **Layer A does NOT catch it:** the checker's anchor regexes are `Current Production deploy`, `CURRENT frontend rollback target` and `Active application release code anchor` — this bullet matches none of them, so it passed every automated gate while contradicting the `Current Production deploy` line three lines below it. **It must be re-read BY HAND every release.**
...
- **Current Production deploy: `d7b9469b`** (Environment Production, branch `main`, source **`d5d86b9420b17ae17e8c7a098a9ec8d43ac607b9`**, bundle **`index-DryJ81XU.js`**, SHA256 `9E033ED588A9FBE9EB00AE2D381E7AAD5325C22CF1A845F72F543F6E45E4346F`, **799,915 B**) — **LIVE (Accessibility A6 — AppointmentModal's eight `.field` labels are now programmatically associated with their controls via additive `id`/`htmlFor`; every earlier slice's behaviour is carried forward in this bundle).** Promoted **UNCHANGED** from the owner-QA'd Preview `0470d5b0` — `wrangler` reported **"Uploaded 0 files (12 already uploaded)"**, and canonical Production ↔ local `dist/` are **12/12 byte-identical by SHA-256** (the deployment URL independently 12/12 as well). Unauthenticated non-mutating smoke: every request a `GET`, all HTTP 200, **0 writes**, no sign-in. **FRONTEND ONLY — 2 files (`src/components/forms/AppointmentModal.jsx` +16 −11, `src/lib/__tests__/accessibleNames.test.js` +268 −2)**, no migration, bucket, Storage policy, grant, Edge, package or secret, and `src/lib/api.js` UNTOUCHED; migrations remain **16 applied of 16 tracked, none pending**, `ai-gateway` **v36 ACTIVE / `verify_jwt=true`**, untouched — so **a frontend rollback is COMPLETE**. Delta **+420 B**, measured against a control build from clean `main` that reproduced the outgoing artifact `index-cQ2OpZHP.js` at 799,495 B **byte-for-byte**. ⚠️ **The first canonical parity fetch again reported a mismatch on `index.html` — same 1,285 B length, different SHA, differing on exactly one line (`src="./assets/index-cQ2OpZHP.js"`). Edge cache, NOT a failed promotion: the deployment URL already served the new bundle, and a cache-busting re-poll returned an `index.html` identical to local on the first retry, then 12/12. SIXTH consecutive release to show this.** *(The previous value of this line is preserved in the JAKE QUOTE DETAIL box below.)*
...
- **Rollback hierarchy — exactly ONE deployment may be called the current target. Do not collapse these levels.**
  1. **CURRENT frontend rollback target: `47497fdb`** (source `5b63f6d10e50daebdaa8481b3d52cdb9b8c04ae8`, bundle `index-cQ2OpZHP.js`, 799,495 B, SHA256 `A4BA90F34DEA4D2DC0A32FBF6128DE8B4446F916B8331F87AD2475348F8A3363`, Jake Quote Detail; git tag `pre-a11y-a6-appointment` @ `91f26a883f9f600dde810c811dcbac90bdbcfcc8`, created **before** the deploy AND pushed — confirmed on `origin` as annotated object `ab3c9bf12669db6d666f517745b977761e584d9b`, dereferencing to `91f26a88` via `refs/tags/pre-a11y-a6-appointment^{}`). **HTTP 200 confirmed AFTER the A6 deploy, still serving its own bundle `index-cQ2OpZHP.js` at 799,495 B / SHA-256 `a4ba90f3…8f8a3363`** — the bundle was re-downloaded and re-hashed, not assumed. ⚠️ **Rolling back to it reverts ONLY the label association**: AppointmentModal's eight visible Hebrew labels go back to naming nothing for a screen reader, and clicking a label focuses nothing. The slice added no database object, so a frontend rollback is COMPLETE.
...
- [x] **Accessibility A6 (AppointmentModal's eight `.field` labels → `id`/`htmlFor`): DONE, CLOSED / LIVE, OWNER-QA PASSED 8/8 with ZERO writes** *(Production `d7b9469b` / `assets/index-DryJ81XU.js` / 799,915 B, source `d5d86b94`, PR [#214](https://github.com/natanMeT/ArtValue20/pull/214))*. **Do not re-select it.** ⚠️ **The bucket is now 74, not 82** (TOTAL 84, not 92), and the hidden-file class is **6, not 5**. ⚠️ **The shared-`Field`-component decision was NOT taken here** — hand edits continued by owner direction, and it stays open for whoever takes the next `.field` slice.
...
Production is `d7b9469b` / `assets/index-DryJ81XU.js` / **799,915 B** (source `d5d86b9420b17ae17e8c7a098a9ec8d43ac607b9`, Accessibility A6 — AppointmentModal labels), migrations are **16 applied of 16 tracked, none pending**, Edge `ai-gateway` v36 is ACTIVE / `verify_jwt=true`, and the current rollback target is **`47497fdb`** / `index-cQ2OpZHP.js` (with `024c0aa7` / `index-FzGUTjjq.js` demoted to historical fallback). Rollback tag `pre-a11y-a6-appointment` @ `91f26a883f9f600dde810c811dcbac90bdbcfcc8`, created before the deploy and pushed.

**FOUR ITEMS CARRIED FROM THIS RELEASE, none a blocker:**
1. **No authenticated Production QA was run.** Preview QA passed on the isolated QA account with the byte-identical artifact, so behaviour is established; Production behaviour for a signed-in user is not independently observed. Same posture as several prior releases.
2. **The shared-`Field`-component decision is STILL untaken** for the remaining 74 `.field` gaps. A6 continued hand edits by owner direction; the choice is deferred, not made.
3. **`src/lib/calc.js:131` null-row fragility** — still booked, unreachable via `fetchAll`, an owner decision. Untouched by A6.
4. **The a11y bucket figures live in a hand-edited comment**, not a CI invariant — `accessibleNames.test.js` must be edited by hand each slice and the tool re-run afterwards.

...
> ### ✅ CAMPAIGNS SLICE 3 (`tasks.campaign_id` surfaced) — **CLOSED / LIVE IN PRODUCTION** (merge + Preview + authenticated Preview UI QA + Production release 2026-07-31)
>
...
>
> ### 🔎 TWO FOLLOW-UPS FOUND DURING QA — **PRE-EXISTING, NOT CAUSED BY THIS SLICE, NOT FIXED HERE**
>
...
> deletes a campaign **and unlinks every task attached to it**, with no warning and no undo. The unlink itself
> is correct accounting of the constraint, and adding warning copy was **explicitly deferred by the owner**
> when this slice was scoped — but the absence of *any* confirmation makes that deferred gap sharper than it
> read on paper. **Recommended: raise its priority.**
>
> **2. Dialogs stay open after a successful mutation.** After a successful save the task modal stays open with
> its data intact, so a user could click `הוספה` / `שמירה` again and create a duplicate. **This was NOT
> attributed by assumption — a control was run:** the **delete** confirmation dialog does the same thing after
> a successful delete, and `remove()` is code this slice never touches. Same symptom in an untouched path ⇒
> **pre-existing Tasks-page behaviour, not a regression.** The save path itself is correct and confirmed-write:
> the success toast fires only after the server confirms. Worth its own slice.
>
> ---
>

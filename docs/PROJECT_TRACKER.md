...
**Last updated:** 2026-08-07 — session: **J1 STALE-CHAT CONTEXT ORDERING — THE LIVE CONTEXT NOW RIDES THE CURRENT USER TURN, NOT THE OLDEST — RELEASED / LIVE, THE FIRST EDGE-ONLY RELEASE: `ai-gateway` v37 ACTIVE / `verify_jwt=true`, FRONTEND UNCHANGED.** The Gateway used to fold `context.summary` into `messages[0]` — the OLDEST turn of the chat window — so a persisted stale answer ("אין קמפיינים") read as a conclusion reached AFTER consulting today's data and measurably overrode the fresh context (the carried C1 echo item). For `jake.chat` + `jake.draft_message` ONLY, the context now folds EXACTLY ONCE into the LAST (current) user turn, and the server system prompt carries an explicit precedence rule; every other action delegates byte-identically to the legacy contract. Code PR [#220](https://github.com/natanMeT/ArtValue20/pull/220) merged as `2bf78bf8ef87a8ac73aa17cdd1efe4d38108ed61` (4 files / +430 −13, server + tests only). **Edge deployed 2026-08-07T02:24:17.804Z: `ai-gateway` v37, bundle `ezbr_sha256` `026b9594983eb0a037a473efaa760d29777055d83324c95ef95df539a0e83029`.** **Frontend deliberately NOT rebuilt: Production stays `a78d3ecc` / `index-Bere1YEI.js` / 802,041 B — a control build from merged `main` reproduced the live artifact 12/12 byte-identical, so the Edge-only claim is proven, not assumed.** Immediate smoke: unauthenticated 401; authenticated `jake.chat` / `jake.draft_message` / legacy `crm.lead_ideas` all 200. **Real-data owner QA on the isolated QA account PASSED the exact C1 scenario:** the truthful empty answer stayed in the SAME conversation, one disposable active campaign + one linked open task were created through the shipped UI, and WITHOUT clearing chat or reloading Jake named both and reported exactly one active campaign; continuity, unchanged-domain and drafting controls passed; cleanup restored 0/0/0/0 DB-backed; Jake performed ZERO writes. **Edge rollback = the byte-proven v36 SOURCE at tag `pre-j1-context-ordering` → `66853d4e229e7a5bd548e32f99ca41500cf1486d`, redeployed as a NEW Edge version if ever needed — version numbers never move backward.** The frontend rollback target is unchanged. *(Full evidence: the J1 box below.)* Previous session: **C1 CAMPAIGN INTERCEPTION — CREATIVE CONTAINMENT TRIGGERS ON INTENT, NOT ON THE WORD `קמפיין` — RELEASED / LIVE IN PRODUCTION.** **Production `a78d3ecc` / `index-Bere1YEI.js`**, Environment Production, branch `main`, source **`5dc2a763a35e3d52c4ee0547b8fe5a370c47082c`**, 801,488 → **802,041 B** (+553), entry-bundle SHA256 `9A28C36989D55B79808B7ABE269D7C5D89C84355BAC807B61C38BC15C6CBC26E`. Promoted UNCHANGED from the owner-QA'd Preview `b8ee1ad8` ("Uploaded 0 files (12 already uploaded)"); Preview ↔ local `dist/` **12/12 byte-identical**, canonical Production ↔ local `dist/` **12/12** after the deploy, deployment URL independently confirmed; propagation correct on the **first** no-cache poll; unauthenticated smoke **10/10 HTTP 200, GET only, 0 writes**. **Frontend only — 2 files (`Assistant.jsx` + one new test), no migration, no bucket, no Storage policy, no grant, no Edge, no package, no secret, `src/lib/api.js` UNTOUCHED.** Code PR [#218](https://github.com/natanMeT/ArtValue20/pull/218) merged as `5dc2a763`. Current rollback target advances to **`08f818d3`** / `index-C53vjfHW.js`; tag `pre-c1-campaign-interception` @ `cceaf222` pushed **before** the deploy. **Owner QA on the isolated QA account: 13/13 routing controls passed, then a REAL-DATA pass on one disposable active campaign + one linked open task — Jake named both, reported exactly one active campaign and the true status; cleanup restored campaigns/tasks to 0/0.** *(The previous value of this line is preserved in the JAKE TASK↔CAMPAIGN JOIN box below.)* Previous session: **JAKE TASK↔CAMPAIGN JOIN — JAKE LEARNS WHICH TASKS BELONG TO WHICH CAMPAIGN — RELEASED / LIVE IN PRODUCTION.** **Production `08f818d3` / `index-C53vjfHW.js`**, source **`cceaf2228c527d7ef511230a90e1e0eb8e9ce946`**, **801,488 B**, SHA256 `0A1072BD…D126C29`; **now the CURRENT ROLLBACK TARGET, no longer live.** Session before that: **ACCESSIBILITY A6 — APPOINTMENTMODAL'S EIGHT LABELS NAME THEIR CONTROLS — RELEASED / LIVE IN PRODUCTION.** **Production `d7b9469b` / `index-DryJ81XU.js`**, source **`d5d86b9420b17ae17e8c7a098a9ec8d43ac607b9`**, **799,915 B**, SHA256 `9E033ED5…45E4346F`; ⚠️ **DEMOTED 2026-08-07 — it was the rollback target until the C1 release and is now a HISTORICAL FALLBACK, neither live nor the current target.** Session before that: **JAKE QUOTE DETAIL — QUOTES REACH JAKE AS DETAIL, NOT ONLY A PENDING COUNT — RELEASED / LIVE IN PRODUCTION.** **Production `47497fdb` / `index-cQ2OpZHP.js`**, Environment Production, branch `main`, source **`5b63f6d10e50daebdaa8481b3d52cdb9b8c04ae8`**, 797,585 → **799,495 B** (+1,910), entry-bundle SHA256 `A4BA90F34DEA4D2DC0A32FBF6128DE8B4446F916B8331F87AD2475348F8A3363`. Promoted UNCHANGED from the owner-QA'd Preview `91705297` ("Uploaded 0 files (12 already uploaded)"); canonical Production ↔ local `dist/` **12/12 byte-identical**; smoke **19/19 HTTP 200, GET only, 0 writes**. **Frontend only — no migration, no bucket, no Storage policy, no grant, no Edge, no package, no secret, `src/lib/api.js` UNTOUCHED.** Current rollback target advances to **`024c0aa7`** / `index-FzGUTjjq.js`; tag `pre-jake-quote-detail` @ `68aeba0b` pushed. *(The previous value of this line is preserved in the JAKE PRE-SETTLE TRUTHFULNESS box below.)*

> **OWNER QA FOR JAKE ASSET LIBRARY CONTEXT (2026-08-05) — PREVIEW `3102b003`, TEN CASES, ALL PASS, ON THE ISOLATED QA ACCOUNT ONLY.** Identity was verified **from the JWT before anything ran** (`sub 54281dc5-a1ea-4328-81df-937681778d37`, `natanturgeman5@gmail.com`, role `authenticated`); the owner's real account was never the session. **Cases proven:** the roll-up and the exact five-item detail list with the `ועוד N שאינם מפורטים כאן` overflow; the campaign label on a linked asset; the empty-account wording `אין נכסים בחשבון הזה` with no other account's asset ever visible; a **forced read failure** answering *"לא הצלחתי לטעון"* with **no count and no detail**; the **stale-row control** — the context went from a 40-asset block to the failure line ALONE, so the previously seen list was gone; the **unknown campaign label** `משויך לקמפיין (השם אינו זמין כרגע)` with the linked count still correct and **no fabricated title**; the **40-asset quota ceiling** answering normally at **3,512 / 12,000 chars with `promptLeak: false`**; a **write refusal** (*"לא יכול למחוק תמונות או לסמן כמועדפות"*); and no regression in the calendar / campaigns / not-connected lines. **Write proof: 27 asset GETs, 0 asset writes, 0 writes of any kind** across the whole session.
>
> **THE REAL-DATA PATCH THAT CLOSED THE HAPPY PATH (2026-08-05).** Four cases had been exercised by injecting the asset/campaign HTTP responses at the browser's `fetch` boundary — the shipped bundle ran end-to-end but the rows were synthetic, and that was stated rather than hidden. The owner then uploaded **six real images through the gallery UI** on the QA account and asked `מה יש לי בגלריה?`. Jake answered **6 נכסים · 1 מועדף · 1 משויך לקמפיין**, and all three numbers were confirmed **three independent ways**: the gallery UI (`הכל (6)` / `מועדפים (1)`), and SQL — 6 rows in `public.assets` (all `source='upload'`, mixed `image/png` + `image/jpeg`, one `is_favorite`, one `campaign_id` → campaign `השקת בדיקה`) and 6 matching `storage.objects`. **CLEANUP WAS VERIFIED AGAINST THE DATABASE AND STORAGE, NOT THE UI:** all six assets deleted one by one through the gallery's own control (`גלריה (6)→(0)`) and the campaign through its own confirmation modal (*"הפעולה אינה הפיכה"*), leaving **0 asset rows · 0 storage objects under the QA prefix · 0 campaigns · 0 rows without an object · 0 objects without a row · 0 tasks referencing a deleted campaign**. Baseline restored exactly.
>
> **LIMITATIONS ACCEPTED AT RELEASE — recorded verbatim so a later session does not mistake them for untested behaviour.** (1) **The pre-settle window is INHERITED from the campaigns lane, not introduced here:** in cloud mode, between panel open and the read settling (≤4 s), `assets` is `undefined` with the error flag false, so the context reads *"המודול אינו מחובר"* — inaccurate in cloud. The 4,000 ms timer bounds it. Closing it is a separate slice covering **all three** seams. (2) **Local/demo was never exercised live** and cannot be on a cloud Preview; that branch rests on unit test + source containment only and is **not** claimed as live QA. (3) **The 40-asset ceiling is proven with injected rows**, not 40 real uploads; the unit test enforces the same ceiling in CI. (4) **Campaign-worded questions still route to the pre-existing S0F.1 creative lane** before the chat lane sees them — pre-existing behaviour, unrelated to this slice; the campaign label on an asset renders correctly regardless. (5) **`listAssets()` signs up to 40 URLs per panel open** for data this lane discards — accepted to keep `src/lib/api.js` untouched, booked as a known inefficiency. (6) **Two pre-existing scope guards were NARROWED, not deleted** (`jakeCalendar.test.js` #23 and the local/demo not-connected count in `jakeContextTruthfulness.test.js`), because assets moved from out-of-scope to in-scope — the same edit the campaigns slice made before.
>
...
**(HISTORICAL — previous release)** **UI POLISH + DASHBOARD ACTION CARD — RELEASED / LIVE IN PRODUCTION.** **Production `478e4d62` / `index-PcQFaAu-.js`**, Environment Production, branch `main`, commit-hash `83f2dfa` — the **accepted Preview artifact promoted unchanged** (`wrangler` reported **"Uploaded 0 files (12 already uploaded)"**, and Production↔Preview are **7/7 byte-identical by SHA-256**, matching the local `dist/` hashes as well). A **UI-only** slice: it replaces the Dashboard conversion-rate KPI with a truthful near-term action card built **only** from data `fetchAll` already hydrates, and fixes the visible defects behind the QA screenshots. The root cause of most of them was not styling drift but **five component classes that had no definition anywhere in the stylesheet** (`.table`, `.form-row`, `.form-actions`, `.form-error`, `.row-actions`), so Campaigns and Schedule were rendering raw unstyled tables and forms; and a **missing `color-scheme` declaration**, which is why native date/time pickers rendered white on the dark UI. Merge `83f2dfaf46e540df408298b8e918b52630a1e95a` (PR [#144](https://github.com/natanMeT/ArtValue20/pull/144), parents `e80a1341` + the owner-approved head `145c970e`, merged with `--match-head-commit`). **NO migration, NO DB/schema change, NO Edge/Auth/secret/package change** — migrations remain **12 applied of 12 tracked, none pending**; Edge `ai-gateway` v36 (ACTIVE at that release) / `verify_jwt=true`, untouched. **Current rollback target is now `ad09b631` / `index-QaS25VkC.js`**, verified HTTP 200 after the deploy; `88b20584` is demoted to a historical fallback, retained. Full suite **126 files / 3,681 passed / 0 failed**. Rollback tag `pre-ui-polish-action-card` @ `e80a1341`. Previous session: **SCHEDULE CORE SLICE 1 — RELEASED / LIVE IN PRODUCTION.** **Production `ad09b631-8d70-421c-b3fc-543972b95723` / `index-QaS25VkC.js`**, Environment Production, branch `main`, commit-hash `660f671e` — the **accepted Preview artifact promoted unchanged** (`wrangler` reported **"Uploaded 0 files (12 already uploaded)"**, and Production↔Preview are **12/12 byte-identical by SHA-256**). Delivers `public.appointments`: the product’s **first durable time-of-day entity**, since every existing date in the schema is a `date` with no time of day. One table with a `kind` (`appointment`/`lesson`/`event`), a **stored** outcome `status`, and **two composite same-owner foreign keys** to `clients` and `tasks`, both `ON DELETE SET NULL` **with the column list**. Named `appointments`, deliberately NOT `calendar_events` — "calendar" is already the Growth OS monthly action board (`src/data/growthCalendar.js`), which persists nothing. Merge `660f671ee923e2fdd75a2aed5f2c4979304d7701` (PR [#142](https://github.com/natanMeT/ArtValue20/pull/142), parents `b2bd9c4b` + the owner-approved head `a5804d1b`, merged with `--match-head-commit`). **Migration `20260801120000` APPLIED and VERIFIED LIVE 2026-07-30 on the FIRST attempt — 12 applied of 12 tracked, none pending; `db push --dry-run --linked` = "Remote database is up to date".** It applied first time **because the exact file was executed against a real PostgreSQL 17.6 before the PR was opened** — the rule F1 established after two failed applies — and that rehearsal caught a brittle assertion in this very file, which was rewritten before merge. PART A matched exactly and **every PART B acceptance control was decisive on disposable QA records**, including **`23503` on both same-owner keys**, four `23514` domain refusals, and the column-list `ON DELETE SET NULL` semantics proven by deleting a parent; QA cleanup **verified zero under RLS and RLS-independently**. **Authenticated Preview UI QA PASSED 2026-07-30** with real clicks and typing only, on the isolated QA account (identity verified from the JWT first; the **owner account was never written to**). **Authenticated non-mutating Production smoke PASSED**: cloud mode, `/schedule` renders signed in, `GET /rest/v1/appointments` **200**, **48/48 network requests HTTP 200 and every one a `GET` — 0 writes**, 0 console messages, and row counts **identical before and after**. **Current rollback target is now `88b20584` / `index-BLR2aev7.js`.** Full suite **125 files / 3,658 passed / 0 failed**. Rollback tag `pre-schedule-core-slice1` @ `b2bd9c4`. **No Edge/Auth/secret/package change:** Edge `ai-gateway` v36 (ACTIVE at that release) / `verify_jwt=true`, untouched. Previous session: **F1 CORE RECEIVABLES SLICE 1 — RELEASED / LIVE IN PRODUCTION.** **Production `88b20584-b375-4073-a762-f91dc2f1a1e8` / `index-BLR2aev7.js`**, Environment Production, branch `main`, commit-hash `c281cda` — the **accepted Preview artifact promoted unchanged** (`wrangler` reported **"Uploaded 0 files (12 already uploaded)"**, and Production↔Preview are **10/10 byte-identical by SHA-256**). Authenticated **non-mutating Production smoke PASSED** on the isolated **QA account B**: cloud mode, Finance renders signed in, KPI/empty state correct, **49/49 network requests HTTP 200 and every one a `GET` — 0 writes**, 0 console messages, and `clients/charges/payments/quotes/transactions/tasks` **identical before and after**. **Current rollback target is now `90a7dc15` / `index-9FYipeQ9.js`.** Preview **`10dbbf8d-d02d-4fcb-8255-6d83a5bff70b`** (branch `f1-preview-110bb1e`, source `110bb1e`, bundle `index-BLR2aev7.js`) — https://10dbbf8d.artvalue-product.pages.dev; the first smoke was **non-mutating and UNAUTHENTICATED** (8/8 assets 200, 0 console messages, **0 REST calls**), and on **2026-07-30 the AUTHENTICATED Preview UI QA PASSED** in a foreground Chrome with **real clicks and real typing only** — the receivables screen was rendered signed in on the isolated **QA account B** (never the **owner account**), a charge of **₪1,000** (service `2026-02-15`, `net60`) showed the computed due date **`2026-04-29` on screen before submit**, a **₪400** payment produced **balance ₪600** and the derived badge **`שולם חלקית`**, cancellation returned the KPIs to ₪0 while **retaining** the payment in actual revenue, and a new payment on the cancelled charge was **refused** (submit disabled in the UI; **`23514`** from `payment_reject_cancelled_charge()` reproduced independently under RLS, with a positive control). **0 console messages of any level**, and QA cleanup **verified zero residue under RLS and RLS-independently**. **Production is UNCHANGED at `90a7dc15` / `index-9FYipeQ9.js`** — the release gate is now the only one left open. Feature merge `56d13ef738c7c01b6fd24c2e7faa988b0a042df9` (PR [#134](https://github.com/natanMeT/ArtValue20/pull/134)), then **two migration-repair merges**: `f4423550a660f5f4b7e3a85fb135d661daf139a0` (PR [#136](https://github.com/natanMeT/ArtValue20/pull/136), head-gated to `7f4e537`) and `601b3c9480cf7a59e999af03a403983d0a90a1a2` (PR [#137](https://github.com/natanMeT/ArtValue20/pull/137), head-gated to `590d987`). **Migration `20260731120000` APPLIED and VERIFIED LIVE 2026-07-30 — 11 applied of 11 tracked, none pending; `db push --dry-run --linked` = "Remote database is up to date".** **It took three apply attempts, and the first two failures are the story of this slice** — the migration had never been executed against a real PostgreSQL before it was merged, so two execution-only defects survived 21 Codex rounds and a 122-file suite (see the box below). PART A matched exactly and **all nine PART B acceptance controls were decisive on disposable QA records**, including **two `23503` refusals**, a **`42703`**, a **`23514`** trigger refusal and the column-list `ON DELETE SET NULL` semantics; QA cleanup **verified zero by query, per-account and RLS-independently**. Full suite **122 files / 3,522 passed / 0 failed**. Rollback tag `pre-finance-receivables-slice1` @ `110baf1`. **No build, no deployment, no Edge/Auth/secret change:** Production remains `90a7dc15` / `index-9FYipeQ9.js`, Edge `ai-gateway` v36 untouched. Previous session: **CAMPAIGNS SLICE 2 (`tasks.campaign_id`) — MIGRATION APPLIED / VERIFIED LIVE / STILL NOT RELEASED.** Merge `8b6a78a792aa019e1d38c3edeb4e629a92de02e5` (PR [#131](https://github.com/natanMeT/ArtValue20/pull/131), parents `735309b` + the owner-approved head `c6b489c`, merged with `--match-head-commit`). **Migration `20260729120000` APPLIED and VERIFIED LIVE 2026-07-30 — 10 applied of 10 tracked, none pending *at that moment*** (today: **11 applied of 11 tracked, none pending**), so the live schema matched `supabase/migrations/**` then and matches it now. `8b6a78a` is the historical anchor for that feature merge, **contained in** `main`; the current `main` HEAD is not pinned here and must be resolved live. PART A (read-only catalog) matched exactly and PART B's five acceptance controls were all decisive on **disposable QA records**, including **both `23503` refusals** (cross-account and nonexistent campaign) and the `on delete set null (campaign_id)` semantics; QA cleanup **verified zero by query**. Codex CLEAN on the merged head; 3 P2s raised, fixed and resolved; 0 unresolved. **No build, no deployment, no Edge/Auth/secret change:** Production remains `90a7dc15` / `index-9FYipeQ9.js`, Edge `ai-gateway` v36 untouched. Full suite 119 files / 3,265 passed / 0 failed *at the merged head*. Rollback tag `pre-campaigns-slice-2` @ `735309b`. Previous session: **REVOKE `anon` EXECUTE ON THE QUOTA COUNTERS — CLOSED (migration-only).** Merge `a8501c3da902cc37c4ad52b701ad25dceae91f6e` (PR [#129](https://github.com/natanMeT/ArtValue20/pull/129), parents `c2a076b` + the owner-gated head `3c4f432`). Migration **`20260728130000` APPLIED and verified live — 9 applied of 9 tracked *at that moment*** (today: **11 applied of 11 tracked, none pending**); checker CLEAN. **`anon` → EXECUTE denied on all three counters; `authenticated` → retained.** Behavioural proof, not just the ACL flag: an `anon` call to `campaign_row_count()` returned **`0`** before and raises **`42501 permission denied`** after. **NO frontend change, therefore NO build and NO deployment: Production remains `90a7dc15-1ed1-4d2a-ad6e-9044c786334c` / `index-9FYipeQ9.js` (source `97b4229`), current rollback target remains `5bcf1ef0`, Edge `ai-gateway` v36 untouched.** Full suite 118 files / 3,227 passed / 0 failed. Rollback tag `pre-revoke-anon-counter-execute` @ `c2a076b`. **The closure box in the Baseline below remains the authoritative RELEASE state — this slice did not change it.**

> ### ✅ J1 STALE-CHAT CONTEXT ORDERING — **CLOSED / LIVE as EDGE `ai-gateway` v37** (consultation → diagnosis → owner-approved spec → one merge → pre-deploy gates → separately gated Edge deploy → immediate smoke → real-data owner QA on the isolated account)
>
> **WHAT SHIPPED — THE FIRST EDGE-ONLY RELEASE.** `toProviderMessages()` folded `context.summary` into `messages[0]` — the OLDEST turn of the chat window — so in a continued conversation the fresh "נתוני המערכת" snapshot was read BEFORE every persisted assistant answer: a stale "אין קמפיינים" turn read as a conclusion reached AFTER consulting today's data, a false in-context exemplar that measurably overrode the fresh context (the C1 QA echo). For **`jake.chat` + `jake.draft_message` ONLY**, the context now folds **exactly once into the LAST (current) user turn** (new pure server module `supabase/functions/_shared/aiGatewayContextFold.js`; fail-closed with no first-turn fallback), and `JAKE_CONTEXT_DATA_GUIDE` / the draft system prompt carry an explicit precedence rule: the current request's data supersedes any earlier assistant answer that conflicts with it. **Every other action — `text.multi_turn` (owner decision: API-first surface unchanged), `jake.force_actions` (single-message, first == last, provably equivalent), `crm.*`, prompt-only — delegates byte-identically to the legacy contract**, pinned by an executable sweep over every registered action id. `aiGatewayContract.js` was deliberately NOT edited: it is re-exported into the frontend bundle graph, and leaving it untouched made the Edge-only claim provable by construction.
>
> | | |
> |---|---|
> | Repository `main` | **`2bf78bf8ef87a8ac73aa17cdd1efe4d38108ed61`** |
> | Merge | `2bf78bf8ef87a8ac73aa17cdd1efe4d38108ed61` — PR [#220](https://github.com/natanMeT/ArtValue20/pull/220), head-gated `77637c49a93cd57a258256ae287f4e78faa0d31a` (merge-commit parents `66853d4e229e7a5bd548e32f99ca41500cf1486d` + `77637c49a93cd57a258256ae287f4e78faa0d31a` are the head-gating proof), **4 files / +430 −13**, one commit |
> | Files | `supabase/functions/_shared/aiGatewayContextFold.js` (new, +94) · `supabase/functions/ai-gateway/geminiProvider.ts` (import swap + comment) · `supabase/functions/ai-gateway/actionProfiles.ts` (guide + draft precedence text; the four verbatim `JAKE_PACK_*` drift-guarded constants byte-identical) · `src/lib/__tests__/jakeContextOrdering.test.js` (new, +308 — ships nothing) |
> | **Migration** | **NONE — no database object, no bucket, no policy, no grant, no package, no secret. Migrations remain 16 applied of 16 tracked, NONE PENDING.** |
> | ✅ Frontend byte-identity (the Edge-only proof) | Control build from merged `main`: **12/12 files SHA-256-identical to canonical Production**, entry exactly `assets/index-Bere1YEI.js` / 802,041 B / `9A28C36989D55B79808B7ABE269D7C5D89C84355BAC807B61C38BC15C6CBC26E`. ⚠️ One measurement artifact recorded: `/index.html` on Cloudflare answers **308 → `/`** with an empty body, so a naive per-file fetch "mismatches" against the SHA of the empty string; `/` itself is byte-identical |
> | ✅ Local rehearsal | Real function served on the local Docker stack (supabase-edge-runtime-1.74.2 / Deno 2.1.4, 16/16 migrations): unauth **401**; 12,001-char context **400**; valid `jake.chat` / `jake.draft_message` / `text.multi_turn` / `crm.lead_ideas` reached the provider boundary and failed CLOSED **503 `provider_not_configured`** with **no provider key present — zero external calls by construction**. Fold assertions (15/15) executed on the byte-identical shipped modules under node; the fold sits BEHIND the key check, so executing it inside the served Deno function would have required contacting the provider — declared, not glossed |
> | ✅ Edge deploy | **v37 ACTIVE / `verify_jwt=true`**, deployed 2026-08-07T02:24:17.804Z from `main` @ `2bf78bf8ef87a8ac73aa17cdd1efe4d38108ed61` (separately owner-gated; exact approved command; no `--no-verify-jwt` anywhere). Deployed bundle `ezbr_sha256` `026b9594983eb0a037a473efaa760d29777055d83324c95ef95df539a0e83029` (v36 was `77e982e0dc4bd279c2bbbfe358270cd9430d4e58bb19ea337ce60dbe0fd83f21`); only `ai-gateway` exists on the project |
> | ✅ Immediate smoke | Unauthenticated POST → **401**; authenticated from the isolated QA account: `jake.chat` **200** (fed the C1 stale-history fixture — answered from the fresh context), `jake.draft_message` **200**, legacy `crm.lead_ideas` **200** with the structured result. Frontend re-hashed after the deploy: byte-identical |
> | ✅ Owner QA (real data) | Isolated QA account `natanturgeman5@gmail.com` / uid `54281dc5-a1ea-4328-81df-937681778d37`, identity read from the JWT before anything ran; baseline **0 campaigns / 0 tasks / 0 quotes / 0 clients** DB-backed. Chat cleared ONCE at setup, then never again. Empty-state ask → truthful **"אין קמפיינים בחשבון הזה."** (stayed in history); disposable ACTIVE campaign `QA-J1-קמפיין-חד-פעמי` + linked open task `QA-J1-משימה-חד-פעמית` created through the shipped UI (link DB-verified via `tasks.campaign_id`); **WITHOUT clearing or reloading**, the same question → **"יש לך קמפיין פעיל אחד: QA-J1-קמפיין-חד-פעמי. יש משימה אחת פתוחה המשויכת אליו: QA-J1-משימה-חד-פעמית."** — names both, exactly one active, no stale repeat, no disconnection claim. Continuity, unchanged-domain (quotes) and drafting controls passed; **Jake performed ZERO writes** (12 persisted messages, 0 action cards); cleanup through the UI's own dialogs restored **0/0/0/0 DB-backed, both disposable ids → 0 rows** |
> | ✅ Tests | **159 files / 4,571 passed / 0 failed** (was 158 / 4,549 → +1 file, +22 tests); `jakeContextOrdering.test.js` executes the REAL fold + REAL input validation + REAL legacy contract, including the ordering-inversion, exact-byte fold, passthrough, fail-closed, whole-registry delegation sweep, budget-limit and system-prompt pins |
> | ✅ Mutation controls | **9/9 bite** (first-turn restore ×6 fails, duplicate fold ×4, folded-role ×4, reorder ×5, question-loss ×5, draft-lane reroute ×2, `text.multi_turn` reroute ×2, precedence-sentence deletion ×1, delimiter byte ×6); every mutated file restored byte-identical by SHA-256. ⚠️ The harness crashed once mid-M1 on a Windows cp1255 decode and left the mutation on disk — caught, restored SHA-verified, harness fixed, all nine re-run from the restored state |
> | ✅ Layer A | CLEAN (0 blockers, 0 actionable) at the merged head and again after this tracker close-out |
>
> ⚠️ **EVIDENCE-CHAIN HONESTY — direct in-page wire capture was UNAVAILABLE, and that is recorded rather than papered over.** The app's supabase-js binds `fetch` at client creation, so a post-load wrapper is bypassed (measured), and the extension network layer surfaced only the CORS `OPTIONS` preflight. The chain that stands instead: **(a) semantic impossibility** — Jake's answer and the drafting control contain both unique disposable names, which appear NOWHERE in the 12-message persisted conversation, so they can only have arrived in the fresh context of the CURRENT request; **(b)** the app's own persisted history proves the stale turn remained outbound-eligible and the re-ask was the last user turn; **(c)** deployed-source identity (v37 built from the verified tree; fold behaviour pinned by executable contract tests + rehearsal on the same bytes); **(d)** the pre-deploy smoke ran the same fixture as a controlled payload with the correct result.
>
> **THE ROLLBACK CONVENTION THIS RELEASE ESTABLISHES.** Frontend rollback and Edge rollback are now SEPARATE layers. The frontend target is unchanged (`08f818d3` / `index-C53vjfHW.js`). The Edge target is the **byte-proven v36 SOURCE** at annotated tag `pre-j1-context-ordering` → `66853d4e229e7a5bd548e32f99ca41500cf1486d` (on `origin` as annotated object `8e305098257cf0360975061b10718281a5a1d4c9`; 13/13 downloaded-vs-repo function files SHA-identical after EOL normalization — the deploy was made from a CRLF Windows checkout, so raw hashes differ by line endings alone, measured not assumed). **Rolling back = redeploying that source as a NEW Edge version (v38). Version numbers never move backward.**
>
> **DELIBERATE NON-GOALS, each its own slice if wanted:** J3 `systemCapabilities()` truthfulness (**next by the approved roadmap, NOT started, pending an explicit Nathan gate**); J2 Jake Receivables Detail (after J3); Calendar Write (deferred to fourth); the pre-settle/re-open seam-timing item (still carried, untouched); any change to `toProviderMessages` itself, `selectJakeChatHistory`, chat persistence or the Assistant UI; mechanism (b) outbound-history annotation (rejected as primary, reserved as an additive follow-up if echo ever recurs with a proven-correct payload).
>> ### ✅ JAKE TASK↔CAMPAIGN JOIN — **CLOSED / LIVE IN PRODUCTION** (consultation → owner-approved spec → one merge → Preview + owner QA on the isolated account → promotion of the same artifact)
>
> **WHAT SHIPPED.** `tasks.campaign_id` and its composite FK have been live since migration `20260729120000`, `TaskModal` has submitted the link since Campaigns slice 2 and `Tasks.jsx` renders a campaign column — yet Jake knew campaign **titles** and task **titles** and nothing about the relation between them. He now gets, read-only: each shown active campaign with up to **3** of its open tasks (titles sliced at **60**, exact overflow count, `אין משימות פתוחות` when empty), plus **one O(1) roll-up** of how many open tasks are linked versus not, scoped so the count (which spans every campaign, including non-active) cannot be read as contradicting the listing (which covers only the shown actives).
>
> | | |
> |---|---|
> | Repository `main` | **`cceaf2228c527d7ef511230a90e1e0eb8e9ce946`** |
> | Merge | `cceaf222` — PR [#216](https://github.com/natanMeT/ArtValue20/pull/216), head-gated `7c32902d9e832951b151db69d81bf853d0d65dc8`, **3 files / +700 −5**, one commit. Merge-commit parents `e06cc698` + `7c32902d` are the head-gating proof |
> | Files | `src/lib/jakePack.js` (+176 −5) · `src/lib/__tests__/jakeCampaignTasks.test.js` (new, 53 tests) · `src/lib/__tests__/jakeCampaigns.test.js` (+11 −5, one budget bound re-scoped on a measurement) |
> | ✅ Preview | **`62595b33`** (branch `task-campaign-preview-cceaf222`, source `cceaf222`, bundle **`index-C53vjfHW.js`**, **801,488 B**, SHA256 `0A1072BD…D126C29`) — https://62595b33.artvalue-product.pages.dev, **12/12 byte-identical** to local `dist/` on the first fetch, smoke GET only, 0 writes |
> | ✅ Owner QA | Isolated QA account `natanturgeman5@gmail.com` / uid `54281dc5-…`; identity read from the JWT **before** the first write; the real account never signed in and Production never opened. DB-backed baseline **0 campaigns / 0 tasks**. **8 cases passed**, **Jake performed ZERO writes**, cleanup verified **zero residue DB-backed** |
> | ✅ Production | **`08f818d3`** / `assets/index-C53vjfHW.js` / **801,488 B** / SHA256 `0A1072BD…D126C29`. Promoted **UNCHANGED** — "Uploaded 0 files (12 already uploaded)", canonical ↔ local `dist/` **12/12** and the deployment URL independently **12/12**, smoke **19/19 GET, zero writes** |
>
> **THE CROSS-LIFECYCLE RULE — the whole reason this is not a two-line change.** Tasks ride the **store**; campaigns ride the **Jake seam**; they settle **independently**. A naive join reports *"המשימה אינה משויכת לקמפיין"* at exactly the moment the truth is *"I could not read the campaigns"*. The split is therefore made on **what each source can actually answer**: **WHETHER** a task carries a campaign id is knowable from `tasks` **alone**, so the roll-up is emitted in **every** campaigns state (error, pending, not-connected, and even a truthy non-array); **WHICH** campaign it is needs the campaigns read, so the per-campaign listing is gated on hydration. Consequence: **zero new absence states** — it inherits the campaigns lane's existing three rather than inventing a fourth — and `campaignLines()` is left **byte-identical**, so no existing wording moved.
>
> **THREE BUCKETS, NOT TWO.** A task whose `campaignId` resolves to nothing is **still linked** (campaign deleted, seam read failed, or not yet settled) and is reported as *"משויכות לקמפיין ששמו אינו זמין לי כרגע — אל תאמר שהן אינן משויכות לקמפיין ואל תנחש את שמו"* **without asserting which cause**. Folding it into "unlinked" would contradict the database; inventing a title would be worse. `campaignLabelForAsset` is reused **as-is** — `assetLibrary.js` untouched.
>
> ⚠️ **THE BUDGET CASE IS WORSE THAN THE QUOTE SLICE'S, AND THE APPROVED SPEC'S BOUND WAS WRONG.** `public.tasks` has **no row quota**, `title` is `text not null` with **no length check**, and `TaskModal`'s title input has **no `maxLength`** — unbounded from **both** sides, so the hard `.slice(60)` is the only bound that exists. Substitute property, MEASURED: **1,986 chars at 5,000 tasks, 1,994 at 50,000 — a 10× row increase costs EIGHT CHARACTERS**; worst-case context **5,067 of `MAX_CONTEXT_CHARS` 12,000**, which the Gateway **REJECTS** rather than truncating. The spec published the bound as `5 × 3 × 60 ≈ 0.9KB`, **omitting the campaign-title term**; the real bound is `CAMPAIGN_ACTIVE_CAP × (CAMPAIGN_LIMITS.title + 3 × 60)` ≈ 1.8KB. Every term is still a constant and no term is a row count, but the published figure was understated and is corrected in the code and the test. The 8,000-char floor in `jakeAssets.test.js` was **measured first and did not need to move**.
>
> **THE QA EVIDENCE IS THE ACTUAL GATEWAY PAYLOAD, NOT THE PROSE.** The `POST /functions/v1/ai-gateway` body was intercepted and its `context` inspected directly: **2,609 chars**, **positive controls fire** (`QA-משימה-4`, `QA-TASK-CAMP-A` present, so the absence checks are live), while the notes marker, the capped-out task, the `done` task, the unlinked task's name and the completed-campaign task's name are **all absent**. ⚠️ **One measurement error caught mid-QA:** the first leak check searched the **whole payload** and reported the marker present — it was **Claude's own typed question** in `messages`, not the context.
>
> ⚠️ **MUTATION CONTROLS: 9 OF 10 BITE, AND BOTH EXCEPTIONS ARE INSTRUCTIVE.** **M5a SURVIVED and is an EQUIVALENT MUTANT, not a coverage hole** — the campaigns guard is doubled (`isHydrated` at the decision point, `Array.isArray` inside `shownActiveCampaigns`), so removing either half alone changes nothing observable; removing **both** crashes the suite, which M5b pins. The redundancy is now documented in the source so neither half is "cleaned up" later. **M7 initially SURVIVED and that was a REAL GAP IN THE TEST, not the code** — every fixture task had a truthy title, so a predicate forked to `status !== 'done' && t.title` was indistinguishable; a task with an **empty** title (`text not null` is satisfied by `''`) was added to the parity fixture and M7 now bites. ⚠️ **THE HARNESS ITSELF FAILED ONCE: it left two mutations in the working tree while reporting a matching SHA.** Caught by a direct source read, both guards restored, the whole diff re-verified line-by-line against `origin/main`, and every figure re-measured from the restored state; the harness was rewritten to keep the pristine copy **outside** the repo and assert SHA-256 after every mutation.
>
> **DELIBERATE NON-GOALS, each a separate slice if wanted:** any **Jake task or campaign write op** (none exists, none added, `jakeAgent.js` untouched and pinned); **task notes in context** (unbounded free text, the `meta.prompt` exclusion class); **per-task campaign attribution** (task → campaign, rejected because tasks are not individually listed and that would mean a new unbounded listing surface); **transaction detail**; and any **Assistant UI change**.
>
> **(HISTORICAL — the previous release, superseded and NOT current; `d7b9469b` is now the ROLLBACK TARGET)**
> ### ✅ ACCESSIBILITY A6 — APPOINTMENTMODAL'S EIGHT LABELS — **CLOSED / LIVE** (superseded in Production by `08f818d3`)
>
> **WHAT SHIPPED.** Eight `.field` blocks in `AppointmentModal` carried a visible Hebrew `<label>` with no `htmlFor` and a control with no `id`: sighted users read the label, a screen reader announced a **blank field**, and clicking the label focused nothing. Eight `id`/`htmlFor` pairs close it. **Additive attributes only** — no CSS, no layout, no `useId`, no wrapping label, no shared `Field` component, no behaviour and no data-flow change.
>
> | | |
> |---|---|
> | Repository `main` | **`d5d86b9420b17ae17e8c7a098a9ec8d43ac607b9`** |
> | Merge | `d5d86b9` — PR [#214](https://github.com/natanMeT/ArtValue20/pull/214), head-gated `bdd4829294cefaa08292780f12bfe44ed2ff9d37`, **2 files / +284 −13**, one commit. Merge-commit parents `91f26a88` + `bdd48292` are the head-gating proof |
> | Files | `src/components/forms/AppointmentModal.jsx` (+16 −11 — the deletions are the same lines re-emitted) · `src/lib/__tests__/accessibleNames.test.js` (+268 −2) |
> | Ids | `appointment-kind` · `appointment-title` · `appointment-date` · `appointment-start-time` · `appointment-end-time` · `appointment-client` · `appointment-task` · `appointment-notes`. **All eight unconditional** — simpler than A5, where two of nine rendered only on a non-empty list |
> | **Migration** | **NONE — no database object, no bucket, no policy, no API, no Edge, no package, no secret.** Migrations remain **16 applied of 16 tracked, NONE PENDING**; `ai-gateway` v36 (ACTIVE at that release) / `verify_jwt=true`, untouched |
> | ✅ Preview | **`0470d5b0`** (branch `a6-preview-d5d86b94`, source `d5d86b9`, bundle **`index-DryJ81XU.js`**, **799,915 B**, SHA256 `9E033ED5…45E4346F`) — https://0470d5b0.artvalue-product.pages.dev, **12/12 byte-identical** to local `dist/`, smoke GET only, 0 writes |
> | ✅ Served-artifact proof | **The component was EXTRACTED FROM THE BUNDLE DOWNLOADED OFF THE PREVIEW URL AND EXECUTED** under a stub JSX runtime (SHA-256 matched to local *before* extraction). From the tree it actually produced: **8 labels with `htmlFor`, 8 controls with an `id`, 0 duplicates, 0 unpaired**, each resolving to the right tag. Positive controls (`className="select"`, `rows=3`) prove the read is live; a fabricated `appointment-nope` resolves to nothing; and the **live Production bundle contains 0 occurrences of `appointment-`**, so the strings are new |
> | ✅ Owner QA | **PASSED — isolated QA account ONLY** (`natanturgeman5@gmail.com`, uid `54281dc5-a1ea-4328-81df-937681778d37`, read **from the JWT before anything ran**). Nathan's real account was never the session; no credential entered, no account created |
> | ✅ Production | **`d7b9469b`** / `assets/index-DryJ81XU.js` / **799,915 B** / SHA256 `9E033ED5…45E4346F`. Promoted **UNCHANGED** — "Uploaded 0 files (12 already uploaded)", canonical ↔ local `dist/` **12/12** and the deployment URL independently **12/12**, smoke GET only, zero writes |
> | ✅ Tests | **156 files / 4,459 passed / 0 failed** (was 156 / 4,439 → **+20 tests, no new file**); production build green |
> | ✅ Build control | A control build from clean `main` reproduced the OUTGOING Production artifact `index-cQ2OpZHP.js` at **799,495 B byte-for-byte**, so the **+420 B** delta is measured, not inferred |
> | ✅ Mutation controls | **11/11 bite** (see the M7 note below). Every mutated file restored **byte-identical by SHA-256** |
>
> **OWNER QA — 8/8 LABEL-CLICK → FOCUS, WITH ZERO WRITES.** Real mouse clicks on each Hebrew label; after each, `document.activeElement` was the expected control: `סוג→appointment-kind <select>`, `כותרת→appointment-title <input>`, `תאריך→appointment-date <input>`, `שעת התחלה→appointment-start-time <input>`, `שעת סיום→appointment-end-time <input>`, `לקוח→appointment-client <select>`, `משימה מקושרת→appointment-task <select>`, `הערות→appointment-notes <textarea>`. **Independent corroboration: Chrome forwarded 8 synthetic clicks onto the controls themselves** — its own label-activation behaviour, which only fires when the association is real. In the open dialog: `[id^="appointment-"]` = **8**, duplicates **0**, 8 labels of which **0 lack `htmlFor`**, and every `label.control` is the **same node** as `getElementById(htmlFor)` (node identity, not truthiness). Before opening: **0**; after closing: **0** — the closed-modal gate confirmed live. Non-regression: title accepts text with `maxLength` **160**, notes with **2000**, the `סוג` select changed by a real `ArrowDown`, date/time keep `type` and `dir="ltr"`, and an **empty submit** produced the existing client-side errors *"צריך למלא כותרת."* / *"צריך למלא תאריך ושעת התחלה."* with the dialog staying open. **Whole session: 3 requests, all `GET`, all inline `data:` URIs — 0 POST / 0 PATCH / 0 DELETE, no REST/Auth/Storage call at all. No appointment was created, so there is no residue.**
>
> ⚠️ **THE DURABLE LESSON — A GUARD I WROTE FAILED ITS OWN MUTATION, AND THAT IS THE A5 LESSON LANDING ON THE GUARD ITSELF.** The first MOUNT SITES test collected **files containing a mount** — the shape A5 uses, because there the expected answer was a *list* of two hosts. A mutation adding a **second `<AppointmentModal>` inside `Schedule.jsx`** — precisely the state that puts two sets of literal `appointment-*` ids in one document — **SURVIVED it**, because the file-level answer is `['pages/Schedule.jsx']` whether the dialog is mounted once or twice. The test now counts mount **OCCURRENCES**; both the same-file and cross-file mutations bite, and a `SINGLE OPEN FLAG` test was added. **"How many mount sites" was still a proxy, one level further in than A5 found it.** The failure is recorded in the test's own comment rather than quietly fixed.
>
> **THE SINGLETON PROOF, WHICH IS WHAT JUSTIFIES LITERAL IDS.** (a) `<AppointmentModal>` is mounted **exactly once**, in `pages/Schedule.jsx`, driven by one `modalOpen` boolean that `openNew` and `openEdit` share, not inside a `.map()`; (b) `<Schedule />` is bound to exactly one route and `App.jsx` keeps `<AnimatePresence mode="wait">`, so an outgoing Schedule is gone before any incoming page mounts; (c) `Modal.jsx` renders `{open && …}`, so a closed dialog contributes **zero** DOM nodes; (d) `Schedule.jsx` declares no `id=` at all, so no collision. Each is pinned by a test — **if any changes, the literal-id decision is void** and the ids must be derived from the appointment's own id.
>
> **A11Y COUNT — MEASURED WITH THE TOOL, NOT BY HAND.** `visible-label-not-associated` **82 → 74** · `no-visible-label` **10 → 10, untouched** · **TOTAL remaining gaps 92 → 84** · hidden `input[type=file]` **6 → 6, untouched**. `AppointmentModal.jsx` disappears from the per-file gap table entirely. Gate: `npm run a11y:count -- --expect 74,10,6` → *"all supplied counts match."*
>
> ⚠️ **THE HIDDEN-FILE COUNT IS 6, NOT THE 5 QUOTED IN OLDER BOXES — corrected here.** The approved A6 spec predicted a gate of `74,10,5` because it inherited the stale figure. Asset Library slice 4 added the gallery upload input, taking that bucket 5 → 6 (already recorded at the slice-4 box, but the newer release boxes still said 5). **A6 changed nothing in that bucket** — it is a separate fix class and is excluded from `totalRemainingGaps`. The corrected value is now also written into the hand-edited comment in `accessibleNames.test.js`.
>
> ⚠️ **NO AUTHENTICATED PRODUCTION QA WAS RUN — same posture as the previous several releases.** Preview QA passed on the byte-identical artifact, so the behaviour is established; Production behaviour for a signed-in user is not independently observed. The Production gate was an unauthenticated, non-mutating, GET-only smoke.
>
> ⚠️ **ONE MEASUREMENT ARTIFACT, RECORDED SO IT IS NOT MISREAD AS A DEFECT.** The QA tab reported `visibilityState: "hidden"` throughout (`hasFocus: true`). Right after `ביטול`, the overlay node still measured as present — the exit animation's clock was frozen — and read **0** once a paint occurred. **That is the documented hidden-tab trap, not a "modal does not close" defect.** It does not touch the focus results: frozen clocks fabricate *negatives*, and every click demonstrably reached the page. Two clicks also initially landed on nothing because the error card pushed the footer ~40 px down; re-clicked at the corrected coordinate.
>
> **DELIBERATE NON-GOALS, each a separate slice if wanted:** the shared-`Field`-component decision (**still open and untaken** across the whole bucket — A6 continued hand edits by owner direction); the unconfirmed "modal does not close after a successful save" report (**not this slice's code, and not re-tested here**); and every other accessibility file — `ChargeModal` 9, `QuoteModal` 8, `BusinessContextEditor` 7, `OnboardingWizard` 7, `Intake` 6 and the rest remain the candidate list.
>
> ### (HISTORICAL — previous release; its behaviour is still LIVE, carried forward in the bundle running today) JAKE QUOTE DETAIL — **CLOSED; SUPERSEDED IN PRODUCTION 2026-08-06** (consultation → owner-approved spec → one merge → Preview + owner QA on the isolated account → promotion of the same artifact)
>
> **WHAT SHIPPED.** Jake received exactly ONE quote fact — the scalar `k.pendingQuotes`, a `.filter().length` from `dashboardKpis` — so he could restate *"3 ממתינות"* and answer nothing else: not whose, not how much, not which. He now receives capped quote **detail**: number · client · VAT-inclusive total · status · date. Frontend only — **no migration, no bucket, no Storage policy, no grant, no Edge, no package, no secret, and `src/lib/api.js` UNTOUCHED**.
>
> **ZERO NEW IO.** `api.fetchAll()` already hydrates `quotes` **with their line items** (`select('*')` plus a client-side `quote_items` join), and `artValueContext()` already called `dashboardKpis(data)`, which does `quotes.filter(...)` unguarded — so the data was not merely available, the pack would already have thrown without it. No new select, no new lane, no timer.
>
> **NOT A SEAM, AND THAT REMOVED FOUR STATES RATHER THAN ONE.** Quotes ride the STORE, not the Jake seam, so there is no `quotesPending`, no `quotesError` and **no ≤4s pre-settle window**. The entire taxonomy the Jake Pre-Settle Truthfulness slice exists to fix **does not apply here**, and inventing a "not connected" wording for a collection that cannot disconnect would have been its own untruth. Two states only: hydrated-empty and hydrated-non-empty.
>
> | | |
> |---|---|
> | Repository `main` | **`5b63f6d10e50daebdaa8481b3d52cdb9b8c04ae8`** |
> | Merge | `5b63f6d` — PR [#212](https://github.com/natanMeT/ArtValue20/pull/212), head-gated `cc97bcbd94c8e86534139f022c068c967588ce89`, **2 files / +553 −1**, one commit. Merge-commit parents `68aeba0b` + `cc97bcbd` are the head-gating proof |
> | Files | `src/lib/jakePack.js` (+153 −1) · `src/lib/__tests__/jakeQuotes.test.js` (+400, new). **The single deletion is the `calc.js` import line, extended for `quoteTotal` — no existing context line was modified or moved** |
> | **Migration** | **NONE — no database object, no bucket, no policy, no API, no Edge, no package, no secret.** Migrations remain **16 applied of 16 tracked, NONE PENDING**; `ai-gateway` v36 (ACTIVE at that release) / `verify_jwt=true`, untouched |
> | ✅ Preview | **`91705297`** (branch `quote-detail-preview-5b63f6d1`, source `5b63f6d10e50daebdaa8481b3d52cdb9b8c04ae8`, bundle **`assets/index-cQ2OpZHP.js`**, **799,495 B**, SHA256 `A4BA90F34DEA4D2DC0A32FBF6128DE8B4446F916B8331F87AD2475348F8A3363`) — https://91705297.artvalue-product.pages.dev, **12/12 byte-identical to local `dist/`**, 18/18 smoke requests `200`, every one a `GET`, 0 writes |
> | ✅ Served-artifact proof | **The builder was EXTRACTED FROM THE BUNDLE DOWNLOADED OFF THE PREVIEW URL AND EXECUTED — 24/24 controls pass.** Not a string grep: the downloaded bundle's SHA-256 was matched to local before extraction, so what ran is what Cloudflare serves. `CAP === 6` was read out as a **live value** (`const Zg=6` … `d.slice(0,Zg)`), not inferred from a `slice(0,6)` grep that appears 3× in the bundle for unrelated reasons |
> | ✅ Excluded-field proof | Each excluded field carried a **unique marker** into the executed builder: `notes` → `MARKER_NOTES_7b2c`, item `desc` → `MARKER_DESC_3a8f`, quote `id` → `MARKER_ID_9f3a`, item `id` → `MARKER_ITEMID_5d1e`, `validDays` → `4242424242`. **All absent.** **Five POSITIVE controls prove those six absences are real and not a dead check** — the same fixture *does* show `Q-2026-014`, `דנה כהן`, `נשלחה`, the date and `₪2,360` |
> | ✅ Owner QA | **PASSED — isolated QA account ONLY** (`natanturgeman5@gmail.com`, uid `54281dc5-a1ea-4328-81df-937681778d37`, verified on screen before any write). Nathan's real account was never signed in. Executed by Claude in the owner's already-authenticated browser at the owner's explicit request; **no credential was entered and no account was created** |
> | ✅ Production | **`47497fdb`** / `assets/index-cQ2OpZHP.js` / **799,495 B** / SHA256 `A4BA90F3…48F8A3363`. Promoted **UNCHANGED** — `wrangler` reported **"Uploaded 0 files (12 already uploaded)"**, canonical↔local `dist/` **12/12 byte-identical** and the deployment URL independently **12/12**, smoke **19/19 HTTP 200, GET only, zero writes** |
> | ✅ Tests | **156 files / 4,439 passed / 0 failed** (was 155 / 4,407 → **+1 file, +32 tests**); production build green |
> | ✅ Mutation controls | **6/6 bite.** M1 cap 6→7 → **4 fail**. M2 drop pending-first ordering → 1. M3 uncomputable total → `₪0` → 1. M4 collapse the missing-client third outcome → 1. M5 fork the pending predicate (drop `draft`) → **2, including the `dashboardKpis` parity control**. M6 emit `q.notes` → 2. `jakePack.js` restored byte-identical after each |
>
> **OWNER QA — WHAT WAS ACTUALLY EXERCISED, AND WHAT WAS NOT.** Run against Preview `91705297` on the isolated QA account, which began **empty** — so a disposable client and two disposable quotes were created first, making a wrong *"אין הצעות"* demonstrably FALSE rather than coincidentally true. **PASSED (7):** (0) zero quotes → *"אין הצעות מחיר ממתינות בחשבון הזה"*, no invention and no disconnection claim; (1) disposable data created, truthful save toast *"לקוח נוסף · נשמר במערכת"*; (2) **`אילו הצעות מחיר ממתינות?` → *"הצעת מחיר אחת ממתינה: AV-1041 ללקוח "QA בדיקה זמנית" בסך ₪2,360, נשלחה ב-5.8.2026."*** — all five fields correct, and the **accepted** quote correctly absent from the pending list; (3) `למי יש הצעת מחיר פתוחה?` → the correct client and number; (4) **`כמה שווה הצעה AV-1042?` → *"שווה ₪590"*, asked deliberately about the ACCEPTED quote to exercise D4** (non-pending quotes fill the cap), matching the screen exactly; (7) **the leak test — the string `סודי-בדיקה-QA-7b2c` was planted in AV-1041's notes and `עיצוב לוגו` in its line items, then Jake was asked directly: *"אין לי גישה לפרטי הערות או תיאור פריטים ספציפיים... אני יכול לראות רק את מספר ההצעה, הלקוח, הסכום והסטטוס."* ZERO leakage, and he names his own boundary correctly**; (8) cleanup — both quotes and the client deleted, **zero residue verified after a full reload** (0 quotes, 0 clients) and Jake returned to the no-quotes wording.
>
> ⚠️ **TWO CASES ARE NOT UI-TESTABLE AND ARE RECORDED AS SUCH, NOT AS PASSES.** (a) **A quote with NO client** — the form's client field is **required (`*`)**, so it cannot be created through the UI. (b) **A ₪0 quote** — the form **refuses** a zero-total quote; deleting the last item row is also not permitted. **Both branches are covered by the executed-artifact controls ONLY** (`סכום לא זמין` vs a real `₪0`, and `לקוח שאינו זמין כרגע` vs `ללא לקוח`). **A control that never fired has not passed** — they are marked ⚪, not ✅, and the distinction between "a real zero" and "no computable total" is therefore proven in code and never against a live user.
>
> **THE BUDGET IS ARGUED DIFFERENTLY HERE, AND THAT IS THE DURABLE LESSON.** **`public.quotes` has NO ROW QUOTA** — `campaigns` caps at 200 in `campaigns_insert_own` and `assets` at 40 in the storage INSERT policy, but `quotes_own` carries ownership and **nothing else**. So the *"fits at the quota ceiling"* proof used by `jakeAssets.test.js` is **unavailable: there is no ceiling to build**. The bound comes from `QUOTE_DETAIL_CAP = 6`, and the property standing in for the missing quota is that the section is **O(1) in row count**. **MEASURED** on the heavy fixture (60 clients, 80 tasks, 50 leads, 200 campaigns, 40 assets with full 2,000-char prompts, max-length quote numbers, 500-char item descriptions), limit `MAX_CONTEXT_CHARS = 12,000` which the Gateway **REJECTS** rather than truncates:
>
> | fixture | chars | delta |
> | --- | --- | --- |
> | heavy account, ZERO quotes | 4,766 | — |
> | 200 max-length quotes | 5,464 | +698 |
> | 5,000 quotes | 5,478 | +712 (14 extra digits) |
> | **200 quotes + the S0D business brain** | **6,402** | **53% of the limit** |
> | **5,000 quotes + the business brain** | **6,416** | **53% of the limit** |
>
> **A 25× increase in row count costs 14 characters.** **The existing 8,000-char regression floor in `jakeAssets.test.js` is UNCHANGED and still passes** — the pre-authorised raise was not needed and was not used; that file is untouched by this PR.
>
> ⚠️ **THE APPROVING BRIEF CARRIED 7,487 / 7,501 FOR THOSE TWO FIGURES AND THEY ARE NOT REPRODUCIBLE.** The measurement was run twice from a clean tree and reproduced 6,402 / 6,416 exactly. The **deltas are identical** (+698 for the section, +14 for 200→5,000), so the brief's numbers appear to be the same measurement on a heavier baseline. **The reproducible values are recorded here rather than the ones that could not be verified**, and the discrepancy is stated in PR #212's body rather than silently reconciled.
>
> **TWO ABSENCES THAT WERE PREVIOUSLY COLLAPSED.** "no quotes at all" and "quotes exist, none pending" both rendered as `0`, so Jake could not tell a business that has **never written a quote** from one that **closed everything it sent**. They are now two different sentences, and the ambiguous numeric `0 ממתינות` form is pinned against by a test.
>
> **AN UNAVAILABLE VALUE IS STATED, NEVER DEFAULTED.** A quote with **no `items` array** has no computable total and says `סכום לא זמין`; a quote with an **empty** `items` array is a real zero and says `₪0`. The discriminator is structural (array vs not-an-array). A `clientId` that resolves to nothing is `לקוח שאינו זמין כרגע`, **never** `ללא לקוח` — the third-outcome rule from `assetCampaignClause`, because a quote whose client cannot be *named* still *has* one.
>
> **THE CAP, THE ORDER, AND THE EXCLUSIONS — the frozen decisions.** `QUOTE_DETAIL_CAP = 6`. Ordering is **pending-first, then the existing array order** (`created_at DESC` as `fetchAll` returned it), and deliberately **NOT re-sorted by `q.date`**, a user-entered field that may be blank or malformed and would make the selection non-deterministic. **EXCLUDED and never printed: `notes`, line-item descriptions, `id`, `vatRate`, `validDays`.** The exclusion of `items` and `notes` is the load-bearing guard, exactly as `meta.prompt` is for assets — both are unbounded free text and `items` is unbounded in count as well. Jake gets the **total**, never the lines. `vatRate` is **consumed** (it is inside the total) but never surfaced as a field.
>
> **ANTI-DRIFT BY EXECUTION, NOT BY IMPORT.** The pending predicate (`draft | sent | viewed`) is duplicated rather than shared with `calc.js`, and parity is proven by **executing both against `dashboardKpis` over 40 status mixes**. A source-level import could be quietly re-inlined by a later refactor; an executed control cannot. `calc.js` was not edited.
>
> ⚠️ **ONE SPEC DEVIATION, AND ONE PRE-EXISTING DEFECT RECORDED RATHER THAN FIXED.** The approved spec called for a "null quote row is skipped" test. `quoteLines` does `filter(Boolean)` and handles it, **but nothing can observe that**: `artValueContext` calls `dashboardKpis` first, and **`src/lib/calc.js:131` dereferences `q.status` unguarded**, so a null row throws before this builder is reached. Rather than pin a crash as intended behaviour the test was dropped and replaced by a documented comment. **The `calc.js` fragility is UNREACHABLE via `fetchAll`** — `rowToQuote` cannot yield `null`, unlike `charges`/`payments` which are normalized **and** `filter(Boolean)`-ed — so hardening shared KPI code for a state no data path produces was ruled outside this slice's boundary. **It is an open owner decision, not a closed item.**
>
> **DELIBERATE NON-GOALS, each a separate slice if wanted:** any **Jake quote write op** (create / edit / send / status / delete — none exists and none was added); **line items and notes in context**; the **`tasks.campaign_id` → Jake join**; **transaction detail**; and any **Assistant UI change** — including the **animated SVG state system** (idle / waking / active / complete / error), which was assessed as a candidate during the consultation and **declined for now**: it closes no tracked gap, has **no truth contract to QA against**, sits adjacent to frozen Jake surfaces, and an "active calculation" state that animates while nothing is running would be the exact false-signal class S0A onward removed. **It becomes a good slice the day its states are bound to real machine state that already exists** (the seam read-states), at which point it stops being decoration and becomes a truthfulness display.
> ### (HISTORICAL — previous release; its behaviour is still LIVE, carried forward in the bundle running today) JAKE PRE-SETTLE TRUTHFULNESS — **CLOSED; SUPERSEDED IN PRODUCTION 2026-08-06** (consultation → owner-approved spec → one merge → Preview + owner QA on the isolated account → a non-empty asset QA patch → promotion of the same artifact)
>
> **WHAT SHIPPED.** During the ≤4,000 ms between a cloud Jake panel open and the campaigns/assets seam reads settling, Jake is no longer told those two modules are **"not connected to this account"**. Frontend only — **no migration, no bucket, no Storage policy, no grant, no Edge, no package, no secret, and `src/lib/api.js` UNTOUCHED**.
>
> | | |
> |---|---|
> | Repository `main` | **`a48020f23d0947a92df0b6da729bdc52924e00cb`** |
> | Merge | `a48020f` — PR [#210](https://github.com/natanMeT/ArtValue20/pull/210), head-gated `8854cc7c8ed869181b2e8360183b0ba26c1643e0`, **9 files / +429 −32**, one commit. Merge-commit parents `27c6a5a` + `8854cc7` are the head-gating proof |
> | Files | `src/lib/jakePack.js` (+38) · `src/lib/campaignReadState.js` (+30 −8) · `src/lib/assetReadState.js` (+28 −7) · `src/components/ai/Assistant.jsx` (+31 −6) · `jakeCampaigns.test.js` (+176) · `jakeAssets.test.js` (+78) · `assetReadState.test.js` (+38 −11) · `campaignReadState.test.js` (+31) · `jakeContextTruthfulness.test.js` (+11) |
> | **Migration** | **NONE — no database object, no bucket, no policy, no API, no Edge, no package, no secret.** Migrations remain **16 applied of 16 tracked, NONE PENDING**; `ai-gateway` v36 (ACTIVE at that release) / `verify_jwt=true`, untouched |
> | ✅ Preview | **`14be2bfb`** (branch `jake-presettle-preview-a48020f`, source `a48020f`, bundle `index-FzGUTjjq.js`, **797,585 B**, SHA256 `0BDFFF59…6F5258E0`) — https://14be2bfb.artvalue-product.pages.dev, **12/12 byte-identical** to local `dist/`, 17/17 smoke requests `200`, every one a `GET`, 0 writes |
> | ✅ Served-artifact proof | Extracted from the **deployed** bundle, not inferred: `function Id(t,s){return`${t}: עדיין אין לי את הנתונים — אל תסיק מכך מסקנה. אל תאמר שאין ${s} ואל תדווח על אפס; אמור בכנות שהנתונים עדיין נטענים.`}` plus both call sites showing the full precedence chain — `:t.campaignsPending?[Id("קמפיינים","קמפיינים")]:[Ia("קמפיינים")]` and `:t.assetsPending?[Id("נכסים (ספריית התמונות)","נכסים")]:[Ia("נכסים")]`. The wording is **parameterised at runtime**, not two hardcoded literals |
> | ✅ Nothing else changed, at ARTIFACT level | Production vs Preview bundle: **1,615 pure Hebrew prose literals identical, 0 removed, 0 added**; Hebrew template literals **exactly 1 added** (the pending wording). The only other delta is one fragment differing by a renamed minifier identifier (`_t(k)`→`Et(k)`) |
> | ✅ Owner QA | **PASSED — isolated QA account ONLY** (`natanturgeman5@gmail.com`, uid `54281dc5-a1ea-4328-81df-937681778d37`, identity read **from the JWT before anything ran**). Nathan's real account was never signed in |
> | ✅ Production | **`024c0aa7`** / `index-FzGUTjjq.js` / **797,585 B** / SHA256 `0BDFFF59…6F5258E0`. Promoted **UNCHANGED** — `wrangler` reported **"Uploaded 0 files (12 already uploaded)"**, canonical↔local `dist/` **12/12 byte-identical**, smoke **16/16 HTTP 200, GET only, zero writes** |
> | ✅ Tests | **155 files / 4,407 passed / 0 failed** (was 155 / 4,375 → **+32 tests**, no new file); production build green |
> | ✅ Mutation controls | **8/8 bite.** M1 default pending TRUE → **14 fail**. M2 drop campaigns branch → 2. M3 drop assets branch → 2. M4 init pending FALSE in cloud → 1. M5 order pending before error → 1. M6 wire the flags into the briefing gate → 1. M7 reuse `notConnectedLine` → 2. M8 `settled:false` on TIMED_OUT → 1. Plus M4b (hardcode `false`) and M4c (drop the key from `jakeData()`) → 1 each |
> | ✅ Build control | A control build from clean `main` reproduced the OUTGOING Production artifact `index-B3o3MS7k.js` at **797,044 B byte-for-byte**, so the **+541 B** delta is measured, not inferred |
>
> **⚠️ A MUTATION SURVIVED, AND THAT IS THE DURABLE LESSON OF THIS SLICE.** M4 — initialising both pending flags to `false` in cloud — **passed the entire 4,375-test suite**. The pure builders were fully covered, but **nothing asserted the seam's INITIAL VALUE**, which is the only thing that makes the window truthful in the shipped app. Three source-level wiring guards were added (initial value, cleared-from-module-return, reaches `jakeData()`), after which M4/M4b/M4c all bite. ⚠️ **They are source-pinned, not behavioural** — this repo has no jsdom and no rendered-component tests. Stated, not glossed.
>
> **THE QA WAS MADE NON-VACUOUS ON PURPOSE, TWICE.** The QA account began **empty**, which would have made a wrong *"אין קמפיינים"* **coincidentally true**. One disposable campaign was created first, so the forbidden zero-claim would have been demonstrably FALSE. The same weakness was then closed for assets by a follow-up patch: a real 70 B PNG with valid magic bytes was uploaded through the **shipped** control, and the pending case re-run against a genuinely non-empty library (2 assets).
>
> **CASES PROVEN (10/10 + the asset patch).** Campaigns pre-settle → pending wording, no `אינו מחובר`, no digit, no failure wording. Assets pre-settle → same. **Both delayed at once** → both pending, distinct labels. **Independence proven in BOTH directions**, not asserted: in one run campaigns were pending while assets settled normally; in the other assets were pending while campaigns settled fully and named the real row. Settled → `1 סה״כ — 1 פעילים` and the real campaign named; with the asset patch, `2 סה״כ — 2 הועלו` with both real assets described and **no signed URL leaked**. Forced failure → failure wording wins, no pending, no count, no leaked row. Calendar → `אין לי גישה ליומן כרגע` intact, using neither the not-connected nor the new pending line. Briefing → appeared at 3 s while both reads were still delayed 8 s, carrying no campaign, asset or pending line.
>
> **⚠️ ONE CASE FIRST READ AS A FAILURE AND WAS NOT REPORTED AS ONE — the method matters more than the result.** The forced-failure case initially returned PENDING instead of the failure wording. Instead of booking a defect, the sequence was timestamped: reads rejected at t=12 ms, context built at t=366 ms, **but a second pair of reads fired at t=1028 ms** — the component had remounted, so the first instance's `alive` guard correctly discarded its own failure and the context measured belonged to a NEW instance that genuinely had not read yet. Re-asked after the surviving instance's read settled, the failure wording won cleanly. **The first reading was a measurement artifact, not a product defect.** This is the same discipline as the hidden-tab trap: prove the stimulus reached the thing you are measuring before reading a result as negative.
>
> **⚠️ ONE "WRITE" WAS CHASED DOWN RATHER THAN WAVED THROUGH.** The QA log recorded a POST during a passing case. It was `POST /storage/v1/object/sign/assets` with `{"expiresIn":300,"paths":[…]}` — **signed-URL minting, a read-only operation**, and the already-documented behaviour where `listAssets()` signs URLs this lane discards. **Zero data mutations occurred during the QA cases.**
>
> **QA METHOD, DISCLOSED RATHER THAN HIDDEN.** The ≤4 s window is too short to type into, so `listCampaigns`/`listAssets` were **delayed at the browser `fetch` boundary** and the assertions were made on `payload.context.summary` — **the exact string sent to the Gateway** — rather than on the model's reply, which is non-deterministic. The shipped bundle ran end-to-end; nothing was stubbed or rewritten. Each pending case used a **fresh page load**, because D2 means pending arms once per mount.
>
> **WRITES AND RESIDUE.** Three writes total, all on one disposable campaign (create → activate → delete, each through the product's own control, the delete via its confirm dialog which **named the exact campaign**), plus one disposable asset upload and its deletion. Verified against the **DATABASE and Storage**, not the UI: `campaigns 0 · my QA asset row 0 · my QA storage object 0 · rows_without_object 0 · objects_without_row 0`.
>
> ⚠️ **LIMITATIONS ACCEPTED AT RELEASE — recorded verbatim so a later session does not mistake them for untested behaviour.** (1) **No owner QA was run on Production** — the Production gate was an unauthenticated non-mutating smoke only (16/16 GET, 0 writes). (2) **QA timing was INJECTED at the fetch boundary**; the real-world window is whatever the network gives. (3) **The Assistant wiring remains SOURCE-PINNED** (no jsdom in this repo), so the owner QA cases are the only behavioural proof of the initial value. (4) **The QA account retains ONE asset that Claude did not create** — `62c5e57a-…`, 1,726,411 B, `source='upload'`, uploaded 18:32:54 while the previous report was being read, three minutes before Claude's own 70 B upload and after a verified `assets_total: 0` baseline. It was **deliberately left untouched by owner instruction**; Claude does not delete data it did not create. It is **not a release blocker**. (5) **Re-open staleness is unchanged and pre-existing** (D2): after the first settle, re-opening the panel shows last-known rows until the new read settles — the same behaviour already recorded for the calendar.
>
> **WHAT THIS SLICE DELIBERATELY DID NOT DO, each a separate slice if wanted:** the calendar's pre-settle wording (proven not false, only mildly pessimistic); re-arming `pending` on every panel open (D2 option b); **Jake quote detail** (still a pending-quote COUNT only, while `quotes` are fully hydrated by `fetchAll` — the cheapest remaining Jake slice, zero new IO); the `tasks.campaign_id` → Jake join; any Jake **write** op; and asset CONTENT understanding.

> ### ✅ ASSET LIBRARY SLICE 4 — USER FILE UPLOAD — **CLOSED / LIVE IN PRODUCTION** (diagnosis → owner-approved spec → one merge → Preview + owner QA on the isolated account → promotion of the same artifact)
>
> **WHAT SHIPPED.** A user can upload **one** of their own images into the durable cloud gallery. Frontend only — **no migration, no bucket, no Storage policy, no grant, no Edge, no package, no secret, no new IO function, and `src/lib/api.js` UNTOUCHED** (`createAsset` was reused exactly as slices 1–3 left it).
>
> | | |
> |---|---|
> | Repository `main` | **`45243b53e7d5d0b1c09d6cccc2e616ce15ee8534`** |
> | Merge | `45243b53` — PR [#204](https://github.com/natanMeT/ArtValue20/pull/204), head-gated `dd8efedb`, **6 files / +716 −4**, one commit |
> | Files | `src/lib/assetLibrary.js` (+94) · `src/pages/ImageStudio.jsx` (+172 −2) · `src/lib/__tests__/assetUpload.test.js` (NEW +140) · `src/pages/__tests__/assetUploadWiring.test.js` (NEW +285) · `assetLibraryGalleryWiring.test.js` (+11) · `a11yScan.test.js` (+14 −2) |
> | **Migration** | **NONE — no database object, no bucket, no policy, no API, no Edge, no package, no secret.** Migrations remain **16 applied of 16 tracked, NONE PENDING**; `ai-gateway` v36 (ACTIVE at that release) / `verify_jwt=true`, untouched |
> | ✅ Preview | **`197b2fe3`** (branch `asset-upload-preview-45243b5`, source `45243b5`, bundle `index-BiwDxmw5.js`) — https://197b2fe3.artvalue-product.pages.dev, **12/12 byte-identical** to local `dist/`, 9/9 smoke requests `200`, every one a `GET`, 0 writes |
> | ✅ Served-artifact proof | Extracted from the **deployed** bundle, not inferred: `const _m="upload",km=bm.join(","),Ec=12,…Sm=Object.freeze({"image/png":t=>Jn(t,0,[137,80,78,71,13,10,26,10]),"image/jpeg":t=>Jn(t,0,[255,216,255]),"image/webp":t=>Jn(t,0,[82,73,70,70])&&Jn(t,8,[87,69,66,80])})` and `function Em(){return{source:_m}}`. The `accept` value is **derived at runtime** (`bm.join(",")`), not a literal; the two `image/*` strings in the bundle are MockupStudio's pre-existing inputs, not this one |
> | ✅ Owner QA | **PASSED — isolated QA account ONLY** (`natanturgeman5@gmail.com`, uid `54281dc5-a1ea-4328-81df-937681778d37`, identity read **from the JWT before any write**). Nathan's real account was never signed in. Baseline was **0 assets / 0 objects / 0 campaigns project-wide** |
> | ✅ Uploads land in REAL Storage | PNG `7b736a9b` (165 B) · JPEG `0190a206` (151 B) · WebP `863f9d69` (46 B) — each with `source='upload'`, `engine`/`prompt`/`preset` **NULL**, `kind='image'`, path under the owner prefix, and **`object_present=1`**. First time an upload has ever been executed against real Storage |
> | ✅ Rows ↔ objects symmetric | **5/5, then 40/40, then 0/0** at every checkpoint — zero orphaned objects, zero dangling rows, across the whole run |
> | ✅ Refusals cost nothing | Oversize (`11.0MB`), wrong type (`application/pdf`) and at-quota all refused with **0 writes**. **The spoof control made ZERO Supabase requests at all** — a text file renamed `.png` and declared `image/png` was refused by the signature guard before the handler reached the network |
> | ✅ Double-submit | Two `change` events **in the same tick** → tiles 4→5, **delta exactly 1**. The synchronous `busyRef` holds |
> | ✅ Quota | Filled to **40** (40 rows / 40 objects); the 41st refused with `הגלריה מלאה (40 פריטים). מחק פריט קיים כדי לשמור חדש.` |
> | ✅ Regressions | Favourite an uploaded asset → `is_favorite=true` in the DB and **`מועדפים (1)` after a full page reload**. Campaign link → `campaign_id` set; unlink → **NULL**; a forged non-owned uuid → `הקישור לקמפיין נכשל`, **0 forged links**. Generation still saves `source='text-to-image'`, `engine='gateway'` — **not** `'upload'` |
> | ✅ QA residue | Every asset deleted through the shipped control; the campaign through the real confirm dialog, which **named `QA-TMP asset link`** before it was accepted. Verified against the **DATABASE**: `assets 0 · objects 0 (incl. the QA storage prefix) · source='upload' 0 · favorites 0 · linked 0 · campaigns 0 · QA-TMP 0`; `tasks`/`appointments` untouched. 0 console errors |
> | ✅ Production | **`378115bc`** / `index-BiwDxmw5.js` / **794,709 B** / SHA256 `12948DE2…D89D0D83`. Promoted **UNCHANGED** — `wrangler` reported **"Uploaded 0 files (12 already uploaded)"**, canonical↔local `dist/` **12/12 byte-identical**, propagation correct on the **first** no-cache poll, unauthenticated non-mutating smoke **9/9 HTTP 200, zero writes** |
> | ✅ Tests | **153 files / 4,335 passed / 0 failed** (was 151 / 4,283 → +2 files, +52 tests); production build green |
> | ✅ Mutation controls | **Seven, all biting.** M1 hardcode `accept="image/*"` → **2 fail**. M2 gate on the mode instead of `durable` → **2 fail**. M3 restore the count-only panel gate → **2 fail**. M4 remove the synchronous busy ref → **1 fails**. M5 delete the dangling row in the catch → **5 fail**. M6 make `'unreadable'` refuse → **10 fail**. M7 add `engine:'local'` to `uploadMeta` → **3 fail** |
> | ✅ Build control | A build from clean `main` reproduced the OUTGOING Production artifact `index-CqSrIXWY.js` / **792,585 B byte-for-byte**, so the **+2,124 B** delta is measured, not inferred |
>
> **THE ONE STRUCTURAL BLOCKER WAS THE PANEL GATE, AND IT NEARLY MADE THE FEATURE UNREACHABLE.** The gallery panel was gated on `gallery.length > 0`, so a new cloud account saw **no gallery at all** — an upload control inside it would have been invisible to exactly the user who needs the first upload. It is now `(gallery.length > 0 || canUpload)`. At zero items the **tab row is suppressed**: four `(0)` counts plus the `וידאו · בקרוב` notice as a new account's first impression is the same structurally-always-zero count the Video Tab Truthfulness slice removed. **The DEVICE backing is unchanged** — `canUpload` is false there, so an empty local gallery still renders nothing.
>
> **THE MAGIC-BYTE GUARD IS UX, NOT SECURITY, AND THE DISTINCTION IS LOAD-BEARING.** `assetSignatureVerdict` returns **three** outcomes: `'match'` proceeds, `'mismatch'` refuses before any network call, and **`'unreadable'` PROCEEDS**. An advisory check that failed *closed* would become a gate the server never agreed to and would refuse valid files on any `File.slice()` quirk. ⚠️ **It does NOT close declared limitation L3** — the server validates the Content-Type the client **declares**, not byte signatures, and a caller that does not go through this screen is entirely unaffected. It exists so a user who picks a mislabelled file is told what is wrong instead of getting a tile that silently fails to render. **L1 (quota concurrency: two creates at 39 can still reach 41) also remains open and unaddressed.**
>
> **UPLOAD METADATA IS DELIBERATELY ONLY `source`.** Not `engine` — the generation path defaults it to `'local'`, and writing that for a file the user chose would record a **false provenance** in a durable column. Not `prompt`/`preset` — the user typed no prompt, and carrying whatever text sits in the studio box would attribute a prompt to an image it never produced. Not `kind` — the cloud row hardcodes `kind='image'` and the server CHECKs it; `meta.kind` is read only by the device store.
>
> **A FAILURE AFTER THE ROW LANDS IS NOT CLEANED UP, AND THAT IS THE POINT.** `api.js` writes the row first and deliberately keeps it when the byte upload fails, so the item is **visible and deletable** rather than an invisible orphan holding quota. The handler therefore re-reads on failure too, so the dangling tile appears at once; calling `deleteAsset` there would invert the rule, and mutation control M5 fails if anyone tries.
>
> **DUPLICATES ARE ALLOWED BY DESIGN.** A fresh uuid per call means the same file picked twice yields **two rows**, not an `assets_storage_path_unique` violation. Measured in QA, not assumed.
>
> **ACCESSIBILITY — the hidden-file-input count moved 5 → 6, and the sixth is NOT an open gap.** The visible `<button>` carries the accessible name; the input is `hidden` (out of the accessibility tree) and `tabIndex={-1}` (off the tab order), so the keyboard path is the button. `aria-hidden` is deliberately **not** added — it is redundant with `hidden`. The scanner classifies by **structure** and cannot see the sibling that names it, so `a11yScan.test.js` was updated by hand. **The other five (`MockupStudio` ×2, `ProjectDetail` ×2, `Settings` ×1) remain undecided and are still the open candidates**, and `totals()` excludes this bucket from `totalRemainingGaps`, so **no published accessibility gap count changed**.
>
> ⚠️ **TWO LIMITATIONS THE QA DID NOT CLOSE, STATED RATHER THAN GLOSSED.** (a) **The OS file-picker hand-off was not directly exercised.** The browser tool that drives a native picker failed on every attempt (three tries, `paths: undefined`), and the native dialog needs access that could not be granted in that session. Each file was instead a **genuine `File` of the real bytes**, assigned via `DataTransfer` to the **shipped input** and delivered by a **real `change` event** — the identical DOM entry point a picker uses, with no application code bypassed or rewritten. Everything downstream of `input.files` is proven against real Storage; the picker → `input.files` step is browser-native behaviour and remains untested. **A two-minute manual pick on Production would close it.** (b) **The cross-account campaign leg used a non-existent uuid, not a live foreign row** — `campaigns` was empty project-wide and manufacturing a row under another account would have meant writing to Nathan's account. The same composite-FK refusal path was exercised; a *live* other-account id remains untested. (c) **Control 14 (local/demo → no upload control) is NOT EXERCISABLE on a deployed artifact** — the product has been cloud-only since 2026-07-27, so the device backing has no reachable entry point. It is covered by the unit test (device adapter → `durable:false`) and by the served-bundle gate `canUpload=Boolean(galleryStore.durable)`.
>
> **WHAT THIS SLICE DELIBERATELY DID NOT DO, each a separate slice if wanted:** multi-file select, drag-and-drop, replacing or editing an existing asset's bytes, **logo / business profile image**, inventory or product images, video, and **assets reaching Jake** — `public.assets` is still durable, live and **invisible to Jake**, which remains the sharpest open Jake gap.

> ### ✅ JAKE CAMPAIGNS CONTEXT — **CLOSED / LIVE IN PRODUCTION** (diagnosis → spec with a measured budget → one merge + owner QA on the isolated account + DB residue proof + promotion of the same artifact)
>
> **WHAT SHIPPED.** The account's durable business campaigns (`public.campaigns`) reach Jake as **read-only context**. Frontend only — **no migration, no API change, no DB, no Edge, no Storage, no package, no secret, no new Jake op, and `src/lib/api.js` untouched** (`listCampaigns()` was reused exactly as it already existed).
>
> | | |
> |---|---|
> | Repository `main` | **`521b7a0b9ea0852cc38b7267542fc46cbfdbe577`** |
> | Merge | `521b7a0b` — PR [#202](https://github.com/natanMeT/ArtValue20/pull/202), head-gated `b53ff1db`, **7 files / +653 −7**, one commit |
> | Files | `src/lib/campaignReadState.js` (NEW +53) · `src/lib/jakePack.js` (+82) · `src/components/ai/Assistant.jsx` (+59 −2) · `src/lib/__tests__/campaignReadState.test.js` (NEW +106) · `src/lib/__tests__/jakeCampaigns.test.js` (NEW +328) · `jakeCalendar.test.js` (+6 −2) · `jakeContextTruthfulness.test.js` (+19 −3) |
> | **Migration** | **NONE — no database object, no API, no Edge, no Storage, no package, no secret.** Migrations remain **16 applied of 16 tracked, NONE PENDING**; `ai-gateway` v36 (ACTIVE at that release) / `verify_jwt=true`, untouched |
> | ✅ Preview | **`12e0a8e4`** (branch `jake-campaigns-preview-521b7a0b`, source `521b7a0b`, bundle `index-CqSrIXWY.js`) — https://12e0a8e4.artvalue-product.pages.dev, **12/12 byte-identical** to local `dist/`, 8/8 smoke requests `200`, every one a `GET`, **0 REST calls** |
> | ✅ Served-artifact proof | The predicate was **extracted from the deployed bundle**, not inferred: `function Uf(t,s){return t===pa.LOADED&&Array.isArray(s)?{campaigns:s,error:!1}:{campaigns:void 0,error:!0}}` — LOADED+array keeps the rows, **every other outcome clears them**. The calendar's separate predicate (`{appointments,error,settled}`) is also present and unchanged |
> | ✅ Owner QA | **PASSED — isolated QA account ONLY** (`natanturgeman5@gmail.com`, uid `54281dc5-a1ea-4328-81df-937681778d37`, identity read **from the session**). 7 `QA-TMP` campaigns created **through the real form** and activated through the real `פעיל` button. **Nathan's real account was never touched**; `public.campaigns` was **empty project-wide** at baseline |
> | ✅ Loaded state | *"יש לך 3 קמפיינים סך הכל: 2 פעילים ו-1 בטיוטות"* — **active campaigns listed, the draft COUNTED BUT NOT LISTED**, Hebrew status labels correct. The dated campaign printed `1.8.2026 עד 30.9.2026`; the undated one printed **no window at all** |
> | ✅ The cap | With 6 actives Jake listed **exactly 5** and then said **`ועוד אחד ששמו אינו מפורט כאן`** — 6 − 5 = 1, the correct overflow. The roll-up still counted **all** of them |
> | ✅ Failed read | With **6 real actives in the database** and the campaigns read blocked (**16 blocked attempts counted, so the stimulus is proven, not assumed**): *"לא הצלחתי לטעון את הקמפיינים. אין לי עליהם נתונים כלל."* — **never `אין קמפיינים`, never a zero** |
> | ✅ **Success → failed read: NO STALE ROWS** | Run **within one page session**, because the seam state only lives there: successful read (3 reads, 7 rows held) → panel closed via its real `סגירה` button → block armed → panel reopened (**16 blocked seam reads**) → chat verified empty of any `QA-TMP` text → **`אין לי גישה לקמפיינים כרגע ואין לי עליהם נתונים כלל.`** The failed read cleared the previously-loaded rows |
> | ✅ Briefing | With 6 actives successfully loaded the morning briefing contained **no campaign line at all** — owner decision D1 honoured live |
> | ✅ Calendar regression | The calendar/briefing lane still matched and answered; timing unchanged |
> | ✅ Empty state | After deleting all 7: **`אין קמפיינים בחשבון הזה.`** — and **not** the unavailable notice |
> | ✅ QA residue | Deleted through the real confirm dialog, each guarded by an assertion that both the row and the dialog name `QA-TMP` — **deleted, never cancelled** (`completed`/`cancelled` are terminal). Final state verified against the **DATABASE**, not the UI: **0 campaigns total, 0 `QA-TMP`, 0 rows on any other account**, and `clients`/`tasks`/`appointments` all untouched |
> | ✅ Production | **`af65f5fe`** / `index-CqSrIXWY.js` / **792,585 B** / SHA256 `2535BEFC…2824`. Promoted **UNCHANGED** — `wrangler` reported **"Uploaded 0 files (12 already uploaded)"**, canonical↔local `dist/` **12/12 byte-identical**, propagation correct on the first no-cache poll, unauthenticated non-mutating smoke **9/9 HTTP 200, zero writes** |
> | ✅ Tests | **151 files / 4,283 passed / 0 failed** (was 148 / 4,216 → +3 files, +67 tests); production build green |
> | ✅ Mutation controls | **Four, all biting.** M1 FAILED returns `[]` instead of `undefined` → **5 tests fail**. M2 drop the `Array.isArray` guard → **1 fails**. M3 raise the cap 5 → 200 → **4 fail**, including the budget test. M4 collapse the two absences → **2 fail** |
> | ✅ Build control | A build from clean `main` reproduced the outgoing Production artifact `index-CBCh_jpO.js` / **790,830 B byte-for-byte**, so the **+1,755 B** delta is measured, not inferred |
>
> **THE BUDGET IS THE RISK, AND IT WAS MEASURED RATHER THAN ESTIMATED.** `MAX_CONTEXT_CHARS = 12000`, enforced on `String.length` (**not bytes** — Hebrew is 1 char but ~1.7 UTF-8 bytes, so byte counts overstate the risk by ~65%), and the Gateway **REJECTS** an over-limit context instead of truncating it. Heavy-account baseline **4,945** chars; capped worst case **5,905 (49.2%)**; campaign cost bounded at **+799 chars**. **Uncapped: 25,253 (2.1x); with `objective` bodies: 65,853 (5.5x)** — both rejected. The 200-row quota makes that worst case reachable by a real account, so the cap of 5 and the exclusion of `objective` are each independently necessary, and T12/T13/M3 guard them.
>
> **TWO PRE-EXISTING GUARDS WERE NARROWED, BOTH DELIBERATELY AND BOTH DOCUMENTED IN PLACE.** (a) `jakeCalendar.test.js` asserted `not.toContain('קמפיינים')` — written by the calendar slice to pin campaigns **and** assets out of **its own** scope. Campaigns are in scope here by owner approval; **the assets half is kept exactly as it was**, and campaigns are now pinned by `jakeCampaigns.test.js`. (b) `jakeContextTruthfulness.test.js` asserted that local/demo emits **no** not-connected wording, on the premise that every collection is present there. Campaigns break that premise **honestly** — the module is cloud-only and `Campaigns.jsx` returns an unavailable state in local mode, pinned by `campaignsContainment.test.js`. Rather than delete the assertion it was **scoped and strengthened**: inventory, projects and activity are each still asserted individually, and it now additionally requires that campaigns is the **only** not-connected line, so a future stray one cannot hide behind the exception. The same file's exact import allowlist gained `./campaigns.js`, a pure module, preserving the property the allowlist protects.
>
> **⚠️ A MEASUREMENT TRAP THAT ALMOST PRODUCED A FALSE DEFECT REPORT — RECORDED SO IT IS NOT REDISCOVERED.** With chat history restored, Jake **repeated the previous list of 6 campaigns verbatim while the reads were blocked**. That looks exactly like the stale-row defect and is **not** one: the identical page state with the chat cleared produced the unavailable notice. **The repeat was the MODEL ECHOING ITS OWN EARLIER MESSAGE from restored chat history, not stale seam state.** This is a pre-existing property of chat history plus an LLM — it would affect clients or tasks identically — and is **not introduced by this slice**. The practical consequence worth knowing: *after data changes, an old answer already in the chat can be repeated until the conversation moves on.* The discriminator that settled it was a controlled comparison (same state, chat cleared vs restored), because the Gateway request body could **not** be intercepted — `supabase-js` captures `fetch` at client construction, so a later `window.fetch` override never sees the `functions/v1/ai-gateway` call.
>
> **⚠️ THE HIDDEN-TAB TRAP FIRED AGAIN AND WAS CAUGHT.** Several intermediate readings reported `panelClosed: false` at `opacity 0.879` with `visibilityState: 'hidden'` — a **frozen exit animation**, not a panel that failed to close; a screenshot showed it visually closed. Every reported QA result was re-measured with the tab **visible and focused**, and **every reload was proven** by setting `window.__qa = 1` and requiring `window.__qa === undefined` afterwards. A hash navigation is not a reload.
>
> **WHAT THIS SLICE DELIBERATELY DID NOT DO, each a separate slice if wanted:** **assets remain OUT OF SCOPE and still invisible to Jake** — `public.assets` is durable and live and, like campaigns before this slice, is **not even declared absent to him**; the **`tasks.campaign_id` → Jake join remains out of scope**, so Jake knows campaign titles but cannot say which tasks belong to them (the grounding rules make him admit that rather than guess); and **Jake still cannot write a campaign** — no create, update, delete or status transition, and no Jake op exists.
>
> **WHAT THE QA DOES NOT COVER, stated rather than glossed.** The block was a page-level `fetch` interceptor, not transport-level blocking — faithful to the client failure path, and proven by counters, but not identical. The **200-campaign quota was not exercised live**; budget headroom rests on measurement plus tests. The QA account held **no appointments**, so the calendar regression confirms the lane is untouched but did not exercise a populated calendar.

> ### ✅ TRANSIENT ACTION CARDS PERSISTENCE — **CLOSED / LIVE IN PRODUCTION** (diagnosis → deferral decision → spec → one merge + owner QA on a real gate + DB verification + promotion of the same artifact)
>
> **WHAT SHIPPED.** An executable action card — `gate` (the code-gated bulk-delete picker), `confirm` (a single pending delete) and `preview` (a batch awaiting `אשר ובצע`) — is no longer persisted or restored. Frontend only, **one pure module** — **no migration, no API, no DB, no Edge, no Storage, no package, no secret, no UI, no copy, and `Assistant.jsx` untouched.**
>
> | | |
> |---|---|
> | Repository `main` | **`c95f46a8a873a6a604b11065a8bbff7a59a4867c`** |
> | Merge | `c95f46a8` — PR [#197](https://github.com/natanMeT/ArtValue20/pull/197), head-gated `52c9a0b2`, **2 files / +169 −3**, one commit |
> | Files | `src/components/ai/chatPersistence.js` (+30 −3) · `src/components/ai/__tests__/chatPersistence.test.js` (+139, T1–T10) |
> | **Migration** | **NONE — no database object, no API, no Edge, no Storage, no package, no secret.** Migrations remain **16 applied of 16 tracked, NONE PENDING**; `ai-gateway` v36 (ACTIVE at that release) / `verify_jwt=true`, untouched |
> | ✅ Preview | **`47597e7d`** (branch `transient-action-cards-preview-c95f46a`, source `c95f46a8`, bundle `index-D2hzBpSc.js`) — **12/12 byte-identical** to local `dist/`, **0 console messages**, 8/8 requests `200` and every one a `GET`, **0 REST calls** |
> | ✅ Served-artifact proof | The predicate was **extracted from the deployed bundle**, not inferred: `function _y(t){return!!(t&&(t.productionProgress\|\|…\|\|t.handoff\|\|t.gate\|\|t.confirm\|\|t.preview))}`. Production's outgoing copy ends at `t.handoff` |
> | ✅ Owner QA | **PASSED — isolated QA account ONLY** (`natanturgeman5@gmail.com`, uid `54281dc5…`, identity read **from the session**). A **real live gate card** was raised over a real row: `🔒 מחיקת כל הלקוחות (1)`, 1 code field. **No code was ever entered and no bulk delete was ever executed.** |
> | ✅ The measurement | **Before reload:** 13 messages in storage, **`storedHasGate: false`**, only `text` / `system+text` shapes — the card was on screen and **never written to disk**. **After a VERIFIED `location.reload()`:** gate cards **0**, code fields **0**, delete controls **0**, history restored **13/13**, the lead-in `🔒` sentence remained, and the completed-action record `✓ נוסף לקוח: QA197_CLIENT_GATE_TEST` survived |
> | ✅ No delete occurred | QA row **`9384fdab-bc63-4c24-88ce-8204f4b1f608`** still existed **after** the reload — verified against the **DATABASE**, not the UI |
> | ✅ QA residue | Cleanup by a **targeted single delete** (never the bulk path), guarded by an assertion that the confirm card named the QA row. Final DB state: **0 QA clients, 0 `QA197%` rows anywhere**, other-account control **4 → 4 unchanged** |
> | ✅ Tests | **148 files / 4,216 passed / 0 failed**; production build green |
> | ✅ Mutation controls | **Two mutations measured, not assumed** — removing `\|\| m.gate` fails **6** tests; removing `\|\| m.confirm \|\| m.preview` fails **6**. The guard cannot silently rot |
>
> **THE ONE RISK, AND WHY IT IS CLOSED.** `isTransientChatMessage` runs on **every message on every save and every hydration**, so a clause matching a broader shape would delete real chat history **silently**. Matching is **property-presence only**, and T5/T5b pin it — including a message that merely *mentions* a card, and one carrying falsy `gate: null` / `confirm: undefined` / `preview: false`. Both persist.
>
> **DECIDED ≠ LOST.** `confirmAction` / `cancelAction` / `approvePreview` / `cancelPreview` replace the card **in place** with a plain `system` text, and those persist exactly as before. Only an **undecided** card disappears — proven live by the surviving `✓ נוסף לקוח…` record.
>
> **ACCEPTED RESIDUE, RECORDED SO IT IS NOT REDISCOVERED AS A BUG.** A gate is emitted as a **pair** — a lead-in `system` text (`…נדרש קוד אישור 🔒`) then the card. After a reload the sentence remains and the card is gone. That is **correct**: the sentence is a truthful, past-tense record of something Jake said. Suppressing it would need a render change and a coupling between two messages. Documented in the module header.
>
> **SCOPE DECISION, OWNER-MADE.** `campaignSelect` and `productionOffer` were **deliberately excluded** — they are not executable destructive affordances and must keep persisting (pinned by T5).
>
> **⚠️ MEASUREMENT DISCIPLINE — A HASH NAVIGATION IS NOT A RELOAD.** The first QA attempt navigated to `#/outreach` and the card appeared to survive. It had not: the SPA never reloaded, revealed by the chat panel still being open. Every subsequent reload was proven with a `window` marker that must disappear. **A reload that did not happen manufactures a defect that does not exist** — the same class as the hidden-tab animation trap.
>
> **⚠️ WHAT THE QA DOES NOT COVER.** One entity (`clients`) and one row; the other three durable entities ride the identical code path. No negative control on Production — the session is per-origin and passwords are never entered — so the negative evidence is the two mutations plus the extracted bundle predicate.
>
> ---
>
> ### ✅ STALE-LIST REFETCH RACE — **CLOSED / LIVE IN PRODUCTION** (diagnosis → spec → one merge + cleanup + scoped owner QA + DB verification + promotion of the same artifact)
>
> **WHAT SHIPPED.** A failed write no longer restores rows a sibling delete already removed. Frontend only — **no migration, no API, no DB, no Edge, no Storage, no package, no secret, no UI, no copy.**
>
> ### ⚠️ THE LESSON OF THIS RELEASE — ONE ABSOLUTE WRITE AMONG FUNCTIONAL ONES IS A LOST UPDATE WAITING FOR CONCURRENCY
>
> `refetch()` was the *only* writer in the store that did `setData(fresh)` instead of `setData(d => reducer(d, action))`. That difference is invisible until two writes overlap, and then it silently discards whatever landed after the fetch was issued.
>
> **The generalisation:** in a store where every mutation composes, a single last-writer-wins path is not a style inconsistency — it is a correctness hole that only concurrency reveals.
>
> ### ⚠️ THE BUG DID NOT APPEAR — IT BECAME OBSERVABLE
>
> Before the bulk-delete fix, the stale list was **consistent** with the false `✓ נמחקו 4` claim: the screen showed four rows and the message said four were deleted, so nothing looked wrong. Making the message truthful is what exposed the list.
>
> **A fix that makes one surface honest will surface every other surface that was quietly agreeing with the lie.** Budget for that.
>
> ### ⚠️ TWO DEFECTS THE TESTS THEMSELVES CAUGHT, NEITHER FOUND BY READING THE CODE
>
> | | what happened |
> |---|---|
> | **A negative control PASSED** | The first anti-deadlock mutation returned **23/23 green**. It was *semantically equivalent* — `r.then(release, release)` still runs before a later-attached handler. **A control that passes has not tested anything.** Re-run with a genuinely deferred decrement, which fails 3 |
> | **A generation-guard test failed** | And it was the test that was wrong, not the code — revealing that coalescing makes overlapping reconciles **impossible**, so the guard could never fire. It was **removed** rather than shipped as dead code that reads as load-bearing |
>
> ### ⚠️ THE HEADLINE — THE SCREEN AND THE DATABASE AGREED, WITHOUT A RELOAD
>
> Six `QA195_` clients, **two of six DELETEs faulted**, one run covering the partial outcome and coalescing:
>
> | | claimed by the UI | measured in the DATABASE |
> |---|---|---|
> | deleted | 4 | **4** |
> | survived | 2 — `QA195_CLIENT_A`, `B` | **2 — the same two, the faulted rows** |
>
> **With no reload.** Pre-fix the identical scenario showed **6 on screen while the database held 2**.
>
> **The network timeline carries both properties at once:** six `DELETE clients` at **0–8 ms** (parallel, unchanged), then **ONE** `fetchAll` burst at **1228 ms** — one reconcile for **two** failures, issued **after** the successful deletes completed rather than at ~10 ms racing them.
>
> | | |
> |---|---|
> | Repository `main` | **`54dbcb0a0bd1af1671a78d43964820cf0595ce45`** |
> | Merge | `54dbcb0a` — PR [#195](https://github.com/natanMeT/ArtValue20/pull/195), head-gated `d72a04cf`, 7 files / +583 −23, two commits (fix + dead-code cleanup) |
> | Files | `src/lib/reconcileScheduler.js` (+164, new) · `src/lib/__tests__/reconcileScheduler.test.js` (+325, new) · `src/store/store.jsx` (+47 −10) · four pin migrations |
> | **Migration** | **NONE — no database object, no API, no Edge, no Storage, no package, no secret.** Migrations remain **16 applied of 16 tracked, NONE PENDING**; `ai-gateway` v36 (ACTIVE at that release) / `verify_jwt=true`, untouched |
> | ✅ Preview | **`942a62d7`** (branch `stale-list-race-preview-54dbcb0a`, source `54dbcb0`, bundle `index-LPwu_nUL.js`) — **12/12 byte-identical** to local `dist/`, **0 console messages**, 9/9 requests `200` and every one a `GET`, **0 REST calls** |
> | ✅ Owner QA | **PASSED — isolated QA account ONLY** (`natanturgeman5@gmail.com`, identity from the JWT). Partial failure: **UI == DB with no reload**, 2 survivors = the 2 faulted rows, **one coalesced reconcile after quiesce**, no deadlock, gate usable, all-success path still `✓ נמחקו 2 הלקוחות (הכל).` ⚠️ The fault injector was **validated with a positive control before being trusted** |
> | ✅ QA residue | **All QA entities 0**, **0** rows carrying a `QA195` label anywhere, other-account control **4 → 4 unchanged** — service-role, RLS-independent, **verified against the DATABASE, never the UI** |
> | Deploy | **Promoted unchanged, NO rebuild.** **Production `fe7baa5e`** (Environment Production, branch `main`, source `54dbcb0`) — `wrangler` reported **"Uploaded 0 files (12 already uploaded)"**. Entry bundle **`index-LPwu_nUL.js`**, SHA256 `56C61486E5654E333E472498BFB9C529145D5FD1BE0AD0EE8C2A4E36F98DA17D`, **788,614 B** (+876 vs the outgoing 787,738). `index-Ddanmefb.css` unchanged, so **this release provably ships no CSS** |
> | ⚠️ Edge-cache propagation | **The first canonical parity run reported 10/12 and it was NOT a failed promotion** — canonical still served the old bundle and the new asset URL returned **1,285 B, the SPA `index.html` fallback**. Polled with `Cache-Control: no-cache`, it flipped on **attempt 4 (~18 s)**; parity then re-ran **12/12** and propagation measured **24/24 across three rounds**, none serving the old bundle |
> | Rollback | **Frontend target `0fa5c3ea` / `index-04boVitS.js`** (source `6635f3d6`, Bulk Delete False-Success), HTTP 200 confirmed after the deploy; **`006761b9` / `index-BhEAq546.js` demoted to historical fallback**, retained, not deleted. **Git tag `pre-stale-list-race-promotion` @ `6635f3d6f44b430b2d32c95b620d7ec02307761d` — created AND pushed**, verified on `origin` (annotated object `05576e7d`). ⚠️ **A rollback here is COMPLETE** — no database object was added. ⚠️ It **restores the stale-list race** |
> | Tests | Full suite **148 files / 4,205 passed / 0 failed**, from a **MEASURED** baseline of **147 / 4,181**. **Four byte-safe negative controls, all biting**: quiesce **3** · coalescing **2** · decrement order **3** · raw-error leak **3**. ⚠️ **A fifth control PASSED and was therefore VOID** — see the box above |
> | ⚠️ What NO test here can prove | **There is no jsdom in this project**, so the store wiring is source-pinned while the scheduler logic is executed. The DOM-level and live-race results are the owner's, from the QA above |
>
> ### 🔎 FOLLOW-UPS THIS SLICE DELIBERATELY LEFT OPEN
>
> * **`refetch`/import overlap.** Hydration, `migrateFromLocal` and `importBackup` call `refetch()` directly, **outside** the scheduler, so an import's absolute write can still overlap a reconcile with no ordering between them. Recorded in the module header **and** `store.jsx`. Rare, pre-existing. **Its own slice.**
> * **An unrelated local edit is still erased during a reconcile.** The absolute `setData(fresh)` is unchanged; fixing this in general needs a pending-operations reconciler, far out of proportion here. **Pinned by a test as known/accepted** so no later reader mistakes it for covered.
> * **The `!userId` silent branch.** It reduces optimistically then returns `{ ok: false }` with **no restore at all and no toast** — fully silent. Untouched by explicit decision. 📌 **DIAGNOSED 2026-08-04 and DEFERRED by owner decision — no code written.** It is the ONLY durable path whose guard sits **after** the reducer (`store.jsx:582-583`); tasks, receivables and business-profile all guard **before** it. Measured by executing the real shipped `dispatch` with injected deps: state mutates, **0** api calls, **0** toasts, **0** reconciles, `{ ok: false }` — with a positive control proving the probe was not vacuous. **BUT it is NOT reachable from the UI:** `App.jsx:185` renders `<Login />` whenever the session is null and `onAuthStateChange` clears the data, and no `dispatch` escapes the render tree. Classified **LATENT INVARIANT CLEANUP, not a current user-facing bug**. **Preferred future fix: the smallest in-place fix on that branch only — NOT a global hoist**, which would widen the return shape and require a broad audit of all 102 dispatch call sites.
>
> ---
>
...
> * **Persisted stale gate cards.** `{ role:'assistant', gate }` is not in `isTransientChatMessage`, so a gate card restores from chat history after a reload carrying ids that may no longer exist — **two live gate cards were observed in the DOM at once**. With this fix, acting on a stale gate surfaces failures instead of false success; the persistence itself is unchanged. **Its own slice.** ✅ **CLOSED and RELEASED 2026-08-04 as the Transient Action Cards Persistence slice — and it turned out to be wider than this entry: `confirm` and `preview` carried the identical defect. See the closure box at the top of this file.**
>
> ---
>
...
>
> ### ⚠️ REMAINING **84** — MEASURED, SPLIT BY THE REACHABILITY GATE. **This is the list the next slice must be chosen from.** *(Was 92; **A6 closed `AppointmentModal`'s 8** on 2026-08-06. The `92` in this heading's history is retained only as the pre-A6 figure.)*
>
> | Reachable in cloud mode — **valid candidates** | Unreachable / latent — **NOT valid candidates** |
> |---|---|
> | `ChargeModal` **9** · `QuoteModal` **8** (5 + 3 nameless) · `BusinessContextEditor` **7** · **`OnboardingWizard` 7** · `Intake` **6** · `TransactionModal` 4 · `PosterEditor` 4 *(all nameless)* · `Diagnose` 4 · `Outreach` 3 · `PaymentModal` 2 · `MockupStudio` 2 *(hidden file)* · `ImageStudio` 1 · `BusinessBrainPanel` 1 · `Settings` 1 *(hidden file)* · `ProjectDetail` 2 *(hidden file)* | `ProjectDetail` **16** · `ProjectModal` **12** — the tool now derives this: `/projects/:id` inherits `/projects`' `betaHidden`. `ItemModal` **fixed but latent**. **`AppointmentModal` is DONE (A6) — do not re-select it** |
>
> ⚠️ **`ChargeModal` 9 is the largest reachable candidate, and it is NOT a free repeat of A6.** It lives on `/finance`, where **more than one dialog exists on one route** (`PaymentModal` too), so the A5/A6 singleton question — *can two instances be in the DOM at the same time* — has to be answered there on its own evidence, not inherited from this slice. **And the mount guard must count OCCURRENCES, not files** — see the A6 box for the mutation that proved the file-level form vacuous.
>
...
> | **Migration** | **NONE — no database object, no API, no Edge, no Storage was touched.** Migrations remain **16 applied of 16 tracked, NONE PENDING**; `ai-gateway` v36 (ACTIVE at that release) / `verify_jwt=true`, untouched |
...
> | **Migration** | **NONE — no database object was touched.** Migrations remain **16 applied of 16 tracked, NONE PENDING**. No Edge/Auth/secret/package change; `ai-gateway` v36 (ACTIVE at that release) / `verify_jwt=true`, untouched |
...
## Baseline (current — C1 CAMPAIGN INTERCEPTION CLOSED / LIVE in Production, deployment `a78d3ecc`)
...
> | **Edge** | `ai-gateway` **v36 (ACTIVE at that release), `verify_jwt=true`** — **UNCHANGED, not redeployed** by this slice |
...
- **Active application release code anchor:** **`5dc2a763a35e3d52c4ee0547b8fe5a370c47082c`** (PR #218 merge — C1 campaign interception, creative containment now triggers on creative-action intent instead of on the bare word `קמפיין`; the running Production artifact was built from this commit and carries every earlier slice's behaviour forward). ⚠️ **The 2026-08-05 correction that first fixed this line still stands, and it was needed AGAIN on 2026-08-06 and AGAIN on 2026-08-07:** it had been left at `8882461731c90f06fca0fc7fb44b5f65e8e20873` (PR #191) through several releases, and the Layer A checker reads `live_src` off THIS EXACT PHRASING (`checks/tracker_status.py` rule 6), so a stale value here makes every CURRENT-labelled block downstream look like it names a superseded release. *(Historical anchors, retained only as the sources of older deployments: `cceaf2228c527d7ef511230a90e1e0eb8e9ce946` — Jake Task↔Campaign join, now the **current rollback target's** source; `d5d86b9420b17ae17e8c7a098a9ec8d43ac607b9` — Accessibility A6, **demoted 2026-08-07 to a historical fallback's source**; `5b63f6d10e50daebdaa8481b3d52cdb9b8c04ae8` — Jake Quote Detail, now a historical fallback's source; `a48020f23d0947a92df0b6da729bdc52924e00cb` — Jake Pre-Settle Truthfulness; `3681d77ef51162f53782ff32119538e84130536c` — Jake Asset Library Context; `45243b53` — Asset Library slice 4; `521b7a0b` — Jake Campaigns Context; `e3555eed` — Jake sees the calendar; `8882461731c90f06fca0fc7fb44b5f65e8e20873` — PR #191; `c138f852` — Accessibility slice A5.)*
- **Three DISTINCT anchors — never collapse them.** This tracker records no fixed value for the repository head, because documentation merges advance it continuously and any pinned value would be false within minutes:
  ...
  3. **Deployed Production source:** **`5dc2a763a35e3d52c4ee0547b8fe5a370c47082c`** — the commit the running Production artifact was actually built from (C1 campaign interception, deployment `a78d3ecc`). The prior `cceaf2228c527d7ef511230a90e1e0eb8e9ce946` (Jake Task↔Campaign join, deployment `08f818d3`) is now the **current rollback target's** source. The older `d5d86b9420b17ae17e8c7a098a9ec8d43ac607b9` (Accessibility A6 — AppointmentModal labels, deployment `d7b9469b`) is now a **historical fallback's** source — **DEMOTED 2026-08-07 by this release** — as are `5b63f6d10e50daebdaa8481b3d52cdb9b8c04ae8` (Jake Quote Detail, deployment `47497fdb`), `a48020f23d0947a92df0b6da729bdc52924e00cb` (Jake Pre-Settle Truthfulness, deployment `024c0aa7`) and `3681d77ef51162f53782ff32119538e84130536c` (Jake Asset Library Context, deployment `0a2669bb`). ⚠️ **CORRECTED 2026-08-06 AFTER STANDING FIVE RELEASES STALE.** This line read `45243b53…` / deployment `378115bc` (Asset Library slice 4) from 2026-08-05 through the Jake Campaigns Context, Jake Asset Library Context, Jake Pre-Settle Truthfulness and Jake Quote Detail releases. **Layer A does NOT catch it:** the checker's anchor regexes are `Current Production deploy`, `CURRENT frontend rollback target` and `Active application release code anchor` — this bullet matches none of them, so it passed every automated gate while contradicting the `Current Production deploy` line three lines below it. **It must be re-read BY HAND every release.** ⚠️ **RE-READ 2026-08-07 (J1) — DELIBERATELY UNCHANGED AND CORRECT:** repository `main` advanced to `2bf78bf8ef87a8ac73aa17cdd1efe4d38108ed61` (PR #220, J1 context ordering), but that commit is the source of **Edge v37 only** — the frontend was deliberately NOT rebuilt (control build 12/12 byte-identical to the live artifact), so the deployed FRONTEND source anchor stays `5dc2a763a35e3d52c4ee0547b8fe5a370c47082c`.
...
- **Current Production deploy: `a78d3ecc`** (Environment Production, branch `main`, source **`5dc2a763a35e3d52c4ee0547b8fe5a370c47082c`**, bundle **`index-Bere1YEI.js`**, SHA256 `9A28C36989D55B79808B7ABE269D7C5D89C84355BAC807B61C38BC15C6CBC26E`, **802,041 B**) — **LIVE (C1 campaign interception — S0F.1 creative containment now triggers on creative-ACTION intent instead of the bare word `קמפיין`, so informational campaign/task questions finally reach Jake's context; every earlier slice's behaviour is carried forward in this bundle).** Full deployment id `a78d3ecc-b8af-4748-a9ec-cac38e228805`. Promoted **UNCHANGED** from the owner-QA'd Preview `b8ee1ad8` — `wrangler` reported **"Uploaded 0 files (12 already uploaded)"**, and canonical Production ↔ local `dist/` are **12/12 byte-identical by SHA-256** (the deployment URL independently confirmed on the entry bundle as well). Unauthenticated non-mutating smoke: **10/10 HTTP 200**, every request a `GET`, **0 writes**, no sign-in. **FRONTEND ONLY — 2 files (`src/components/ai/Assistant.jsx` +46 −3, `src/components/ai/__tests__/campaignIntent.test.js` new +198)**, no migration, bucket, Storage policy, grant, Edge, package or secret, and `src/lib/api.js` UNTOUCHED; migrations remain **16 applied of 16 tracked, none pending**. Edge at this frontend release was v36; **superseded 2026-08-07 by `ai-gateway` v37 ACTIVE / `verify_jwt=true` (J1 context ordering — an EDGE-ONLY release; this bundle is unchanged and a control build reproduced it 12/12 byte-identical)** — so **a frontend rollback is COMPLETE** and independent of the Edge version. Delta **+553 B** (801,488 → 802,041). `index-Ddanmefb.css` unchanged and every sibling chunk byte-identical, so **this release provably ships no CSS**. ⚠️ **Propagation was correct on the FIRST no-cache poll this time — the edge-cache/SPA-fallback class that hit the previous seven releases did NOT recur here. That is one clean observation, not evidence the class is gone; keep polling with `no-cache` first.** *(The previous value of this line is preserved in the JAKE TASK↔CAMPAIGN JOIN box below.)*
- *(HISTORICAL — the line below is the superseded description of the previous deploy, retained as history and NOT current: )* **`47497fdb`** (source **`5b63f6d10e50daebdaa8481b3d52cdb9b8c04ae8`**, bundle **`index-cQ2OpZHP.js`**, SHA256 `A4BA90F34DEA4D2DC0A32FBF6128DE8B4446F916B8331F87AD2475348F8A3363`, **799,495 B**) — **was LIVE (Jake Quote Detail — Jake receives capped quote DETAIL (number · client · VAT-inclusive total · status · date) instead of only the scalar `pendingQuotes` count; every earlier slice's behaviour is carried forward in this bundle).** Promoted **UNCHANGED** from the owner-QA'd Preview `91705297` — `wrangler` reported **"Uploaded 0 files (12 already uploaded)"**, and canonical Production ↔ local `dist/` are **12/12 byte-identical by SHA-256** (the deployment URL independently 12/12 as well). Unauthenticated non-mutating smoke **19/19 HTTP 200, every one a `GET`, 0 writes**. **FRONTEND ONLY — `src/lib/api.js` UNTOUCHED** (blob `1555be7331bdd95d6243eefa00cf10f08854b7c3` on both sides of the merge), no migration, bucket, Storage policy, grant, Edge, package or secret; migrations remain **16 applied of 16 tracked, none pending**, `ai-gateway` **v36 (ACTIVE at that release) / `verify_jwt=true`**, untouched — so **a frontend rollback is COMPLETE**. Delta **+1,910 B**. ⚠️ **The first canonical parity fetch reported the OLD entry `index-FzGUTjjq.js` even with `Cache-Control: no-cache` — edge cache, NOT a failed promotion (the deployment URL already served the new bundle). A cache-busting query returned the new entry and an `index.html` SHA-256 identical to local on the first retry. FIFTH consecutive release to show this: settled edge behaviour. Poll with no-cache AND a cache-buster AND `curl -L` — `/index.html` also 308-redirects to `/`, which reads as a 0-byte mismatch without `-L`.**
    - ⚠️ **PROPAGATION TOOK FIVE NO-CACHE POLLS (~16s) — recorded so the next release does not read it as a failure.** Polls 1–4 on the canonical URL still returned the OUTGOING bundle `index-B3o3MS7k.js`; poll 5 returned `index-FzGUTjjq.js` at the correct 797,585 B and SHA-256. This is the documented Cloudflare edge-cache behaviour, not a mismatch, and it was polled through rather than reported as drift.
  - *(SUPERSEDED 2026-08-05 by `024c0aa7`, retained as history and NOT current. ⚠️ It IS the CURRENT ROLLBACK TARGET — see the rollback hierarchy, which is the single authority: )* **`0a2669bb`** (Environment Production, branch `main`, source **`3681d77ef51162f53782ff32119538e84130536c`**, bundle **`index-B3o3MS7k.js`**, SHA256 `8875BB8E8CD9BF17ED7DD62FC1D475001BC624374BBA1F0DE1F563D7F7B8E91B`, **797,044 B**) — **LIVE (Jake Asset Library Context — the durable cloud asset library reaches Jake's context as READ-ONLY, metadata-only data; every earlier slice's behaviour is carried forward in this bundle).** Promoted **UNCHANGED** from the owner-QA'd Preview `3102b003` — `wrangler` reported "Uploaded 0 files (12 already uploaded)", canonical↔local `dist/` **12/12 byte-identical**, and the unauthenticated non-mutating smoke was **15/15 HTTP 200, zero writes**. Deployed 2026-08-05. **No migration, no bucket, no Storage policy, no Edge, no package, no secret, and `src/lib/api.js` untouched (byte-identical SHA `1555be7331bdd95d6243eefa00cf10f08854b7c3`).**
    - ⚠️ **PROPAGATION NEEDED TWO POLLS, AND BOTH KNOWN ARTIFACTS FIRED — recorded so the next release does not read either as a failure.** The first no-cache canonical poll still returned the OUTGOING bundle `index-BiwDxmw5.js`; the second returned `index-B3o3MS7k.js`. Separately, the first bare fetch of the new asset URL returned the **1,285 B SPA fallback**; re-polled with a cache-buster it returned the correct **797,044 B** and the correct SHA-256, after which the plain cached fetch agreed too. Neither is a mismatch — both are the documented edge-cache behaviour, and both were chased to ground rather than reported.
  - *(SUPERSEDED 2026-08-05 by `0a2669bb`, retained as history and NOT current. ⚠️ It IS the CURRENT ROLLBACK TARGET: `378115bc`, Environment Production, branch `main`, source **`45243b53e7d5d0b1c09d6cccc2e616ce15ee8534`**, bundle **`index-BiwDxmw5.js`**, SHA256 `12948DE215DF6E6B0A5AB46392696A330D728324DCC28A640C01D9ECD89D0D83`, **794,709 B** — Asset Library slice 4, a user's own image reaches the durable cloud gallery. Verified HTTP 200 and still serving its own bundle AFTER this promotion. )*
  - *(SUPERSEDED EARLIER, retained as history and NOT current. It was the rollback target until 2026-08-05 and is now a historical fallback one step further back: )* (Environment Production, branch `main`, source **`521b7a0b9ea0852cc38b7267542fc46cbfdbe577`**, bundle **`index-CqSrIXWY.js`**, SHA256 `2535BEFC45935432841974B7C73E5FC5FE89CB5176AAF4AC593A850EE4CE2824`, **792,585 B**) — **LIVE (Jake Campaigns Context — the durable per-account business campaigns reach Jake's context as READ-ONLY data; every earlier slice's behaviour is carried forward in this bundle).** One PR: [#202](https://github.com/natanMeT/ArtValue20/pull/202) (merge `521b7a0b`, head-gated `b53ff1db`, **7 files / +653 −7**). The owner-QA'd Preview `12e0a8e4` artifact was **promoted UNCHANGED**: `wrangler` reported **"Uploaded 0 files (12 already uploaded)"**, canonical↔local `dist/` is **12/12 byte-identical by SHA-256**, and the served entry bundle hashes to the QA'd value above. Unauthenticated non-mutating Production smoke **9/9 HTTP 200, zero writes**. **NO migration, NO Edge, NO schema, NO package, NO secret** — migrations remain **16 applied of 16 tracked, NONE PENDING**; `ai-gateway` v36 (ACTIVE at that release) / `verify_jwt=true`, untouched — so a **frontend rollback is COMPLETE**.
  - *(SUPERSEDED 2026-08-05, retained as history and NOT current. ⚠️ It IS the CURRENT ROLLBACK TARGET: )* **`123ea5aa`** (Environment Production, branch `main`, source **`e3555eedc691ef2f37cf5ad68db10d6cb9f40c56`**, bundle **`index-CBCh_jpO.js`**, SHA256 `95D769548B757F80836AE68C18A2B120E5F5E4622F07358838137871430418DC`, **790,830 B**) — **was LIVE (Jake sees the calendar — the durable appointments reach Jake's context and daily briefing, read-only, and a failed read no longer leaves stale rows).** HTTP 200 confirmed AFTER the Jake Campaigns deploy, still serving its own bundle `index-CBCh_jpO.js` — verified, not assumed.
  - *(SUPERSEDED 2026-08-05, retained as history and NOT current. ⚠️ It was the current rollback target and was **DEMOTED to historical fallback 2026-08-05** by the Jake Campaigns Context release; the current target is `123ea5aa`: )* **`a5ef6d36`** (Environment Production, branch `main`, source **`c95f46a8a873a6a604b11065a8bbff7a59a4867c`**, bundle **`index-D2hzBpSc.js`**, SHA256 `E14E7671F5937F6B038AB77DCE00AE37228585D309FD86AE4CABCF51D8A8212F`, **788,644 B**) — **was LIVE (Transient Action Cards — an executable action card no longer persists or hydrates; every earlier slice's behaviour is carried forward in this bundle).** The owner-QA'd Preview `47597e7d` artifact was **promoted UNCHANGED**: `wrangler` reported **"Uploaded 0 files (12 already uploaded)"**, **12/12 byte-identical local `dist/` ↔ Preview BEFORE the deploy and 12/12 canonical ↔ `dist/` AFTER it**. Propagation was confirmed on the **first** cache-busted poll — the edge-cache trap did not fire this time, and the canonical parity sweep still ran afterwards. `index-Ddanmefb.css` is unchanged, so **this release provably ships no CSS**; the entry bundle moved by **+30 B**, one predicate clause (the header comment compiles away). ⚠️ **The `index.html` parity read requires the ROOT URL** — `/index.html` returns **308** and a naive sweep reports 11/12; it is byte-identical at `/` (1,285 B, SHA256 `5794D6D6…`), and that 1,285 B is also the SPA-fallback size, so the asset URL was checked separately (788,644 B, not the fallback). ⚠️ **This release added NO database object, no API, no Edge, no package and no secret, so a frontend rollback to `fe7baa5e` is COMPLETE — and it restores the persisted-stale-action-card defect.**
  - *(SUPERSEDED 2026-08-04, retained as history and NOT current. ⚠️ It **WAS** the current rollback target and has since been demoted twice — the current target is `123ea5aa` (⚠️ corrected 2026-08-05; this line had gone stale): )* **`fe7baa5e`** (Environment Production, branch `main`, source **`54dbcb0a0bd1af1671a78d43964820cf0595ce45`**, bundle **`index-LPwu_nUL.js`**, SHA256 `56C61486E5654E333E472498BFB9C529145D5FD1BE0AD0EE8C2A4E36F98DA17D`, **788,614 B**) — **was LIVE (Stale-List Refetch Race — a failed write no longer restores rows a sibling delete removed; every earlier slice's behaviour is carried forward in this bundle).** The owner-QA'd Preview `942a62d7` artifact was **promoted UNCHANGED**: `wrangler` reported **"Uploaded 0 files (12 already uploaded)"**, **12/12 byte-identical local `dist/` ↔ Preview BEFORE the deploy and 12/12 canonical ↔ `dist/` AFTER it**; propagation polled cache-busted **24/24 across three rounds**. `index-Ddanmefb.css` is unchanged, so **this release provably ships no CSS**. ⚠️ **The first canonical parity run reported 10/12 and that was edge cache, NOT a failed promotion** — the new asset URL returned the 1,285 B SPA fallback until it propagated; canonical flipped on the 4th cache-busted poll (~18 s). ⚠️ **This release added NO database object, no API, no Edge, no package and no secret, so a frontend rollback to `0fa5c3ea` is COMPLETE — and it restores the stale-list race.**
  - *(SUPERSEDED 2026-08-04, retained as history and NOT current. ⚠️ It was the current rollback target and was **DEMOTED to historical fallback 2026-08-04** by the Transient Action Cards release; the current target is `fe7baa5e`: )* **`0fa5c3ea`** (Environment Production, branch `main`, source **`6635f3d6f44b430b2d32c95b620d7ec02307761d`**, bundle **`index-04boVitS.js`**, SHA256 `46856ACCC03F5DE255E7968F68BD4028414579F5A27EE82B488189B9987A4EB3`, **787,738 B**) — **was LIVE (Bulk Delete False-Success — a bulk delete reports only what the store confirmed; every earlier slice's behaviour is carried forward in this bundle).** The owner-QA'd Preview `caa00b32` artifact was **promoted UNCHANGED**: `wrangler` reported **"Uploaded 0 files (12 already uploaded)"**, **12/12 byte-identical local `dist/` ↔ Preview BEFORE the deploy and 12/12 canonical ↔ `dist/` AFTER it**; propagation polled cache-busted **24/24 across three rounds**. `index-Ddanmefb.css` is unchanged **and all five sibling JS chunks are byte-identical to the outgoing release** — only the entry hash and `index.html` moved — so **this release provably ships no CSS**. ⚠️ **This release added NO database object, no API, no Edge, no package and no secret, so a frontend rollback to `006761b9` is COMPLETE — and it restores the false-success defect: a refused cloud delete would again report `✓ נמחקו N`.**
  - *(SUPERSEDED 2026-08-04, retained as history and NOT current. ⚠️ It was the current rollback target and was **DEMOTED to historical fallback 2026-08-04** by the Stale-List Refetch Race release; the current target is `0fa5c3ea`: )* **`006761b9`** (Environment Production, branch `main`, source **`8882461731c90f06fca0fc7fb44b5f65e8e20873`**, bundle **`index-BhEAq546.js`**, SHA256 `06c1b4d7cc6bd228c70fa718f4fa9c58…`, **786,231 B**) — **was LIVE (Jake Context Truthfulness — Jake no longer reports unhydrated modules as empty, and now sees the F1 receivables; every earlier slice's behaviour is carried forward in this bundle).** The owner-QA'd Preview `25f8f71f` artifact was **promoted UNCHANGED**: `wrangler` reported **"Uploaded 0 files (12 already uploaded)"**, **12/12 byte-identical local `dist/` ↔ Preview BEFORE the deploy, 12/12 canonical ↔ `dist/` AFTER it, and 12/12 canonical ↔ the QA-approved Preview**; propagation polled cache-busted **8/8**. `index-Ddanmefb.css` is unchanged, so **this release provably ships no CSS**. ⚠️ **This release added NO database object, no API, no Edge, no package and no secret, so a frontend rollback to `e4d3abe7` is COMPLETE — and it restores the phantom "מלאי: ריק / פרויקטים פעילים: אין" claims and removes receivables from Jake's context.**
...
- **Rollback hierarchy — exactly ONE deployment may be called the current target. Do not collapse these levels.**
  1. **CURRENT frontend rollback target: `08f818d3`** (source `cceaf2228c527d7ef511230a90e1e0eb8e9ce946`, bundle `index-C53vjfHW.js`, 801,488 B, SHA256 `0A1072BDF524DD38826D7160F74F8E38C788B76EF1F916EB7D71B519BD126C29`, Jake Task↔Campaign join; git tag `pre-c1-campaign-interception` @ `cceaf2228c527d7ef511230a90e1e0eb8e9ce946`, created **before** the deploy AND pushed — confirmed on `origin` as annotated object `46e59eceab7e07b5088c2135e2e9f46dd976621c`, dereferencing to `cceaf222` via `refs/tags/pre-c1-campaign-interception^{}`). **HTTP 200 confirmed AFTER the C1 deploy, still serving its own bundle `index-C53vjfHW.js` at 801,488 B / SHA-256 `0a1072bd…d126c29`** — the bundle was re-downloaded and re-hashed, not assumed. ⚠️ **Rolling back to it reverts ONLY the C1 intent rule**: the bare word `קמפיין` again enters creative containment, so informational campaign and task↔campaign questions stop reaching Jake's context, and the plural containment hole (`תבנה לי קמפיינים חדשים` reaching the model) re-opens. The slice added no database object, changed no schema and wrote nothing, so a frontend rollback is COMPLETE.
     *(Superseded value, retained as history: `d7b9469b` / `index-DryJ81XU.js` / 799,915 B — Accessibility A6 — was the rollback target until this release and is now a **historical fallback**.)*
     - *(HISTORICAL FALLBACK ONLY — **DEMOTED 2026-08-07** by the C1 campaign-interception release, retained and NOT the current target (§22: exactly one current target): )* **`d7b9469b`** (source `d5d86b9420b17ae17e8c7a098a9ec8d43ac607b9`, bundle `index-DryJ81XU.js`, 799,915 B, SHA256 `9E033ED588A9FBE9EB00AE2D381E7AAD5325C22CF1A845F72F543F6E45E4346F`, Accessibility A6 — AppointmentModal labels; git tag `pre-jake-task-campaign` @ `e06cc6986a990285b41ca6d579bfe40a3c5c09b0`, pushed). ⚠️ **Rolling back this far now silently reverts TWO releases** — it loses the C1 intent rule **as well as** the task↔campaign join.
     - *(HISTORICAL FALLBACK ONLY — **DEMOTED 2026-08-06** by the Jake Task↔Campaign join release, retained and NOT the current target: )* **`47497fdb`** (bundle `index-cQ2OpZHP.js`, 799,495 B, Jake Quote Detail).
     - *(HISTORICAL FALLBACK ONLY — **DEMOTED 2026-08-06** by the Accessibility A6 release, retained and NOT the current target (§22: exactly one current target). **HTTP 200 confirmed after that deploy**: )* **`024c0aa7`** (source `a48020f23d0947a92df0b6da729bdc52924e00cb`, bundle `index-FzGUTjjq.js`, 797,585 B, SHA256 `0BDFFF59A16F9D850708C021DCAC0D8699F151B69C1E323BF29BF0CA6F5258E0`, Jake Pre-Settle Truthfulness; git tag `pre-jake-quote-detail` @ `68aeba0b7e1e45be4612b02faf29118fccb02f4e`, pushed). ⚠️ **Rolling back this far now silently reverts TWO releases** — it loses the label association **as well as** the quote detail, so Jake returns to the bare `pendingQuotes` count.
     - *(HISTORICAL FALLBACK ONLY — **DEMOTED 2026-08-06** by the Jake Quote Detail release, retained and NOT the current target (§22: exactly one current target): )* **`0a2669bb`** (source `3681d77ef51162f53782ff32119538e84130536c`, bundle `index-B3o3MS7k.js`, 797,044 B, SHA256 `8875BB8E8CD9BF17ED7DD62FC1D475001BC624374BBA1F0DE1F563D7F7B8E91B`, Jake Asset Library Context; git tag `pre-jake-pre-settle` @ `27c6a5a0248ec9fb17b13b29bca8bf03bea07b20`, pushed). ⚠️ **Rolling back this far now silently reverts TWO releases** — it loses the quote detail **as well as** the pre-settle truthfulness fix, so the ≤4s window would again tell Jake that campaigns and assets are "not connected to this account".
     - *(HISTORICAL FALLBACK ONLY — **DEMOTED 2026-08-05** by the Jake Pre-Settle Truthfulness release, retained and NOT the current target (§22: exactly one current target). **HTTP 200 confirmed after that deploy**: )* **`378115bc`** (source `45243b53e7d5d0b1c09d6cccc2e616ce15ee8534`, bundle `index-BiwDxmw5.js`, 794,709 B, Asset Library slice 4 — user file upload; git tag `pre-jake-asset-context` @ `f5ab300ee9b833e755e2f17ba7095e18abd5c074`, pushed). **HTTP 200 confirmed AFTER the Jake Asset Library Context deploy, still serving its own bundle `index-BiwDxmw5.js`** — verified, not assumed. ⚠️ **Rolling back to it removes the asset library from Jake's context entirely — he returns to having NO asset data AND NO declaration of absence, so an ungrounded answer becomes possible again for "מה יש לי בגלריה". The gallery itself, favourites, campaign association and user upload all keep working; only Jake's awareness of them is lost.** ⚠️ **A frontend rollback here is COMPLETE** — this release added no database object, no API, no Edge, no package and no secret.
     - *(HISTORICAL FALLBACK ONLY — **DEMOTED 2026-08-05** by the Jake Asset Library Context release, retained and NOT the current target (§22: exactly one current target). **HTTP 200 confirmed after that deploy**: )* **`af65f5fe`** (source `521b7a0b9ea0852cc38b7267542fc46cbfdbe577`, bundle `index-CqSrIXWY.js`, 792,585 B, Jake Campaigns Context; git tag `pre-asset-user-upload` @ `df9cfae1ea0546fc035237ceb845ff660cb413b2`, pushed). ⚠️ **Rolling back this far now silently reverts TWO releases** — it loses Jake's asset context **as well as** the user-upload inlet into the durable gallery.
     - *(HISTORICAL FALLBACK ONLY — demoted 2026-08-05, retained and NOT the current target (§22: exactly one current target). **HTTP 200 confirmed, still serving `index-CBCh_jpO.js`**: )* **`123ea5aa`** (source `e3555eedc691ef2f37cf5ad68db10d6cb9f40c56`, bundle `index-CBCh_jpO.js`, 790,830 B, Jake sees the calendar + its stale-read fix; git tag `pre-jake-campaigns` @ `885e8093bf431cdb8d260514962689542ada08bf`, pushed). **HTTP 200 confirmed AFTER the Jake Campaigns deploy, still serving its own bundle `index-CBCh_jpO.js`** — verified, not assumed. ⚠️ **Rolling back to it removes campaigns from Jake's context entirely — he returns to having NO campaign data AND NO declaration of absence, so an ungrounded answer becomes possible again for "מה קורה עם הקמפיינים שלי".** ⚠️ **A frontend rollback here is COMPLETE** — this release added no database object, no API, no Edge, no package and no secret.
     - *(HISTORICAL FALLBACK ONLY — **DEMOTED 2026-08-05** by the Jake Campaigns Context release, retained and NOT the current target (§22: exactly one current target): )* **`a5ef6d36`** (source `c95f46a8a873a6a604b11065a8bbff7a59a4867c`, bundle `index-D2hzBpSc.js`, 788,644 B, Transient Action Cards; git tag `pre-jake-calendar` @ `463f6d37d2e324e8bb8946523aba726ebb5203d4`, pushed). **HTTP 200 confirmed AFTER the Jake Calendar deploy, still serving its own bundle `index-D2hzBpSc.js`** — verified, not assumed. ⚠️ **Rolling back to it removes the calendar from Jake entirely — he goes back to answering `מה יש לי היום` from tasks, quotes and charges with the calendar silently omitted — AND it restores the stale-appointments defect, where a failed read leaves a previous read's rows in place and Jake presents an unverified snapshot as the current calendar.**
     - *(HISTORICAL FALLBACK ONLY — **DEMOTED 2026-08-05**, retained and NOT the current target (§22: exactly one current target): )* **`fe7baa5e`** (source `54dbcb0a0bd1af1671a78d43964820cf0595ce45`, bundle `index-LPwu_nUL.js`, 788,614 B, Stale-List Refetch Race; git tag `pre-transient-action-cards-promotion` @ `54dbcb0a0bd1af1671a78d43964820cf0595ce45`, retained). Still HTTP 200 at the time of the promotion, but **rolling back to it now silently reverts TWO releases** — it restores the persisted-stale-action-card defect **as well as** losing the calendar.
     - *(HISTORICAL FALLBACK ONLY — demoted 2026-08-04, retained and NOT the current target: )* **`0fa5c3ea`** (source `6635f3d6f44b430b2d32c95b620d7ec02307761d`, bundle `index-04boVitS.js`, Bulk Delete False-Success; git tag `pre-stale-list-race-promotion` @ `6635f3d6f44b430b2d32c95b620d7ec02307761d`, retained). Still HTTP 200 at the time of the promotion, but **rolling back to it now silently reverts TWO releases** — it restores the stale-list refetch race **as well as** the persisted-stale-action-card defect.
  1. *(HISTORICAL FALLBACK — demoted 2026-08-04 by the Stale-List Refetch Race release, retained, NOT the current target; §22 requires exactly one: )* **`006761b9`** (source `8882461731c90f06fca0fc7fb44b5f65e8e20873`, bundle `index-BhEAq546.js`, Jake Context Truthfulness; git tag `pre-bulk-delete-false-success-promotion` @ `8882461731c90f06fca0fc7fb44b5f65e8e20873`, pushed). HTTP 200 confirmed after the Stale-List Refetch Race deploy. ⚠️ **Rolling back this far ALSO restores the `runBulkDelete` false-success defect.**
  ...
  3. **Git rollback tag: `pre-jake-calendar` @ `463f6d37d2e324e8bb8946523aba726ebb5203d4` — created AND pushed** (confirmed on `origin`: `* [new tag] pre-jake-calendar`; annotated object `9cced3be4b74f65867a9ed16901cddc699b4e194`). It anchors the last commit before PR #199, i.e. the tree that produced the outgoing `index-D2hzBpSc.js`. *(Previous tag, retained: `pre-transient-action-cards-promotion` @ `54dbcb0a0bd1af1671a78d43964820cf0595ce45`.)* **No Edge rollback applies to this release — `ai-gateway` v36 was NOT redeployed.** **The live release added NO migration**, so there is nothing left applied behind a frontend revert. *(HISTORICAL — the previous release's tag record, retained and NOT the current tag: `pre-stale-list-race-promotion` @ `6635f3d6f44b430b2d32c95b620d7ec02307761d` was created AND pushed, confirmed on `origin` as `refs/tags/pre-stale-list-race-promotion` → annotated object `05576e7d5846b14bf31dafb0f67b76902cd7bdd8`, dereferencing to `6635f3d6f44b430b2d32c95b620d7ec02307761d`.)* *(Previous tags, retained: `pre-bulk-delete-false-success-promotion` @ `8882461731c90f06fca0fc7fb44b5f65e8e20873` — annotated object `7934d98bca1163bf4058aebb6249b030e3099d75`; `pre-jake-context-truthfulness-promotion` @ `d1b695f2` — annotated object `f0fa2870b407721e765c71d6be5c80d66b343c08`.)* **No Edge rollback applies to this release — `ai-gateway` v36 was NOT redeployed.** **The live release added NO migration**, so there is nothing left applied behind a frontend revert. Earlier tags retained: `pre-a11y-a5-promotion` @ `06b43ae7`, `pre-a11y-a3a4-promotion` @ `3e7d7d07`, `pre-a11y-a2-promotion` @ `69193895`, `pre-a11y-a1-promotion` @ `a9f96eeb`, `pre-modal-stacking-promotion` @ `d1dcc64`, `pre-schedule-month-promotion` @ `ba76045`, `pre-monthly-plan-promotion` @ `75a8a167`, `pre-campaign-link-promotion` @ `934722f4`, `pre-ui-polish-action-card` @ `e80a1341`.
...
- **Edge: `ai-gateway` v37 ACTIVE, `verify_jwt=true`** — the only function on the project. **v37 (J1 stale-chat context ordering, deployed 2026-08-07T02:24:17.804Z from `main` @ `2bf78bf8ef87a8ac73aa17cdd1efe4d38108ed61`, PR #220; deployed bundle `ezbr_sha256` `026b9594983eb0a037a473efaa760d29777055d83324c95ef95df539a0e83029`)** folds `context.summary` into the CURRENT (last) user turn for `jake.chat` + `jake.draft_message` only, adds the current-data precedence rule to their server system prompts, and delegates every other action byte-identically to the legacy contract (new pure module `supabase/functions/_shared/aiGatewayContextFold.js`; `aiGatewayContract.js` deliberately untouched so the frontend bundle graph is untouched by construction). **EDGE ROLLBACK CONVENTION: the rollback target is the byte-proven v36 SOURCE at annotated tag `pre-j1-context-ordering` → `66853d4e229e7a5bd548e32f99ca41500cf1486d` (13/13 function files verified SHA-identical, EOL-normalized, against the downloaded deployed v36 source; v36 bundle was `ezbr_sha256` `77e982e0dc4bd279c2bbbfe358270cd9430d4e58bb19ea337ce60dbe0fd83f21`). Rolling back means REDEPLOYING that source as a NEW Edge version (it would become v38) — version numbers never move backward, and no claim that they do may ever be recorded here.** The FRONTEND rollback target is a separate, unchanged anchor: `08f818d3` / `index-C53vjfHW.js`. **v36 history, retained — v36 removed the local providers from the shared contract**: `comfyui` / `ollama` / `fooocus` / `a1111` are gone from `AI_PROVIDERS`, `AI_MODELS` and every routing chain, together with the `LOCAL_PROVIDERS` partition, the `localFirst` ordering option and its response metadata, and the local zero-cost branch; the 20-action cloud vocabulary is unchanged and every action still resolves to a non-empty all-API chain. Verified live: a request naming `provider:'ollama'` with `localFirst` returns `options:{}` and the chain `["gemini","openai","openrouter"]`. *(Older history: **v35** preceded v36 — its only S0F.1 change was the Jake persona text constant and its related comment in `actionProfiles.ts`. ⚠️ **CORRECTED 2026-08-07: v35 is NO LONGER the Edge rollback target** — the convention is now the byte-proven **v36 source** at tag `pre-j1-context-ordering`, redeployed as a NEW version; see this bullet's head.)* Account Business Context continues to be assembled + injected by the **frontend chat/draft seam** before the existing Gateway call; the ImageStudio brand-palette block is likewise composed **frontend-side** into the existing `studio.generate_image` prompt.
...
- **Test evidence for the CURRENT release (Asset Library slice 4 — user file upload, source `45243b53`, deployment `378115bc`, bundle `index-BiwDxmw5.js`, 794,709 B):** full suite **153 files / 4,335 passed / 0 failed** (was 151 / 4,283 → +2 files, +52 tests); production build green; **seven mutation controls all biting** (M1 hardcoded `accept` → 2 fail; M2 gate on the mode → 2 fail; M3 count-only panel gate → 2 fail; M4 no synchronous busy ref → 1 fails; M5 auto-delete the dangling row → 5 fail; M6 `'unreadable'` refuses → 10 fail; M7 false `engine` meta → 3 fail), and each JSX-level scan carries a negative control that mutates the real file so it cannot pass vacuously. **Build control:** a build from clean `main` reproduced the OUTGOING artifact `index-CqSrIXWY.js` / 792,585 B byte-for-byte, so the **+2,124 B** delta is measured. **NO migration, NO DB/schema change, NO bucket, NO Storage policy, NO Edge/Auth/secret/package change, and `src/lib/api.js` untouched** — migrations were **16 applied of 16 tracked, none pending** at this release. Promoted **unchanged** ("Uploaded 0 files (12 already uploaded)"), canonical **12/12 byte-identical** by SHA-256 to the accepted `dist/`.
  - *(HISTORICAL — superseded test evidence, retained as history and NOT current. ⚠️ This line had carried the CURRENT label through several later releases: )* Test evidence for the CURRENT release (Accessibility slices A3 + A4, source `06b43ae7`, deployment `4855a30f`, bundle `index-ClJVE2eR.js`):** full suite **144 files / 4,043 passed / 0 failed** on the released source (baseline before the two slices: 144 / 4,017). **No new test FILE** — the existing parser-backed `src/lib/__tests__/accessibleNames.test.js` grew **16 → 43 tests** across the two slices. **NO migration — no database object, no API, no Edge, no Storage, no package and no secret was touched; 16 applied of 16 tracked, none pending.** Each slice carried an **end-to-end negative control on the real source** (removing one real `htmlFor` failed tests; the file was restored **byte-identical by SHA256**) and a **CRLF re-checkout control** — `ItemModal.jsx` was re-materialised by `git checkout` from LF to **CRLF** and the guard still passed, which is the failure mode this repo has been bitten by before. ⚠️ **THE NEW ASSERTION THAT MATTERS MOST is not about labels at all — it is the REACHABILITY gate** (`Clients.jsx` carries no `BetaUnavailable`, `/clients` carries no `betaHidden`, `'clients'` is absent from `BETA_HIDDEN_MODULES`), added because A3 shipped a correct fix to a modal no cloud user can open. ⚠️ **What the suite still CANNOT prove, unchanged since A1:** there is **no jsdom environment and no `@testing-library`**, so no test computes a rendered accessible NAME. For A4 the load-bearing evidence is the owner's Preview QA; **for A3 there is none and there cannot be, until Inventory becomes durable.** ⚠️ **The 86 / 107 bucket counts are a measured scan result, NOT an invariant this file enforces** — `accessibleNames.test.js:8` carries them as a hand-edited comment.
...
## Status ledger
- **Jake Pre-Settle Truthfulness (campaigns + assets stop reading as "not connected" while their seam reads settle)** — **CLOSED / RELEASED / LIVE VERIFIED in Production (2026-08-05).**
  - **Chain:** PR [#210](https://github.com/natanMeT/ArtValue20/pull/210) → merge `a48020f` (head-gated `8854cc7`) → one build → Preview `14be2bfb` → **owner QA 10/10 PASS** on the isolated QA account → a **non-empty asset QA patch** → Production **`024c0aa7` / `index-FzGUTjjq.js`**, promoted unchanged ("Uploaded 0 files (12 already uploaded)").
  - **The gap:** `{ rows, error }` encodes TWO absences while the product has THREE. The third — "the read has not come back yet" — was collapsed into "this module does not exist here", so for ≤4 s after every cloud panel open Jake was handed a FALSE claim about two live, durable, account-owned modules.
  - **Delivered:** a `pending` flag consumed ONLY inside the existing unhydrated block, with precedence hydrated > error > pending > not-connected. **12 lines of behavioural diff.**
  - **The load-bearing decision:** `pending` defaults FALSY, so an absent key keeps today's wording and local/demo can never announce that a module it does not have is "loading". Mutation M1 → 14 failures.
  - **The calendar was excluded on measured evidence** — its wording asserts no disconnection and no zero, so there was no falsehood to fix. This NARROWED the previously recorded "one slice across all three seams" to two.
  - **Budget measured:** hydrated 5,085 / 12,000 stays the binding constraint; the pre-settle state is 3,947 — **43 chars cheaper than the wording it replaces**.
  - **Frontend only:** no migration, bucket, Storage policy, grant, Edge, package or secret, and `src/lib/api.js` byte-identical on both sides. **A frontend rollback is COMPLETE.**
  - ⚠️ **A mutation SURVIVED (M4) and was not accepted** — nothing asserted the seam's initial value; three source-level wiring guards were added. ⚠️ **A QA case first read as a failure and was correctly diagnosed as a measurement artifact** (component remount + `alive` guard) rather than booked as a defect.
  - ⚠️ **Open limits:** no Production owner QA; QA timing was injected at the fetch boundary; the Assistant wiring stays source-pinned; the QA account retains one asset Claude did not create, left untouched by owner instruction.
- **Asset Library slice 4 — user file upload into the durable gallery** — **CLOSED / RELEASED / LIVE VERIFIED in Production (2026-08-05).**
  - **Chain:** PR [#204](https://github.com/natanMeT/ArtValue20/pull/204) → merge `45243b53` (head-gated `dd8efedb`) → Preview `197b2fe3` → owner QA on the isolated account → Production `378115bc`, the same artifact promoted unchanged.
  - **Delivered:** one file per pick, cloud/durable backing only, into the existing `assets` bucket and `public.assets` table. `source='upload'`; `engine`/`prompt`/`preset`/`kind` deliberately absent.
  - **The gap:** `createAsset()` always accepted any `Blob`, but the only blob-producing call site was `srcToBlob(r.src)` in the generation branch and `ImageStudio.jsx` had **no file input at all** — an image could enter the durable library only by being generated.
  - **Frontend only:** **no migration, no bucket, no Storage policy, no grant, no Edge, no package, no secret, no new IO function, and `src/lib/api.js` UNTOUCHED.** Every server rule an upload needs was already live.
  - **Ordering rules reused verbatim:** row-then-bytes on create, object-then-row on delete, `upsert:false`, persist-first with no optimistic item, and **a failure after the row lands is left visible rather than cleaned up**.
  - **Measured in QA, not assumed:** rows↔objects symmetric at 5/5, 40/40 and 0/0; the spoof control made **zero Supabase requests**; two `change` events in one tick produced **exactly one row**; the 41st upload at quota was refused; favourites, campaign link/unlink and generation all regressed clean; residue zero in the database including the storage prefix.
  - ⚠️ **Open limits:** the **OS picker hand-off** was not directly exercised; the **client magic-byte guard is UX only and does NOT close L3**; **L1 quota concurrency** is unchanged; the cross-account campaign leg used a non-existent uuid rather than a live foreign row; and control 14 (local/demo) is **not exercisable** on a cloud-only deployed artifact.
- **Jake sees the calendar (durable appointments → Jake's context + daily briefing, read-only) + its stale-read fix** — **CLOSED / RELEASED / LIVE VERIFIED in Production (2026-08-05).**
  - **Chain:** PR [#199](https://github.com/natanMeT/ArtValue20/pull/199) → merge `2adfbb4c` (head-gated `cf378251`) → PR [#200](https://github.com/natanMeT/ArtValue20/pull/200) → merge `e3555eed` (head-gated `2b12ba65`) → one build → Preview `033987aa` → **owner QA 7a/7b PASS** on the isolated QA account → Production **`123ea5aa` / `index-CBCh_jpO.js`**, promoted unchanged ("Uploaded 0 files (12 already uploaded)", 12/12 canonical parity) → non-mutating smoke 29/29, all GET.
  - **Delivered:** Jake's context gains today's agenda (up to **5** entries, then `ועוד N`) plus a **count only** for the next seven days, and the daily briefing gains a calendar line that **leads `למעקב`**. Agenda semantics are **inherited** from the pure `schedule.js`, not redefined: `cancelled` / `completed` / `no_show` are history, not agenda, and the day window is half-open. **`schedule.js` was not modified** — it was already clock-free by contract, which is why this slice wrote no new time logic.
  - **The read is the JAKE SEAM, not `fetchAll`:** one `listAppointments()` per panel open, guarded by `isSupabaseConfigured`, with an `alive` unmount/account-switch guard. The store's whole-object replacement had already produced two shipped defects, and appointments have no local reducer, seed or `localStorage` fallback to classify.
  - **Read-only:** no new Jake op, no write path, no `answerFromData` branch, no new Gateway call or `actionType`. **No migration, no Edge, no package, no secret.**
  - **The cap of 5 is a budget guard, not cosmetics.** The context ships inside the existing `context.summary`, validated against `MAX_CONTEXT_CHARS = 12000`, which **rejects over-limit rather than truncating** — an uncapped list would stop Jake, not degrade him. Measured on the real outbound payload: **2,140 chars**; the test asserts against the *imported* limit.
  - **⚠️ Owner QA found a real defect in the first PR, and that is the durable lesson.** A failed read set the error flag but **kept the rows a previous successful read had stored**, so `isHydrated` stayed true and Jake presented an unverified snapshot as the current calendar. **First-open failure was always correct** — `appointments` had simply never been set — so every shipped test passed; **only a reopen after a success exposed it**. The spec's behaviour table modelled "not hydrated" and silently assumed failure ⇒ absent. **A behaviour table must enumerate the ORDER of outcomes, not just the outcomes.**
  - **Fixed in PR #200** by extracting one decision to the pure `src/lib/calendarReadState.js` so tests **execute** it rather than grep the component (the `reconcileScheduler.js` precedent): only a successful read may leave rows; a rejection or timeout returns `appointments` to `undefined`, which is what re-enables the notice. A `LOADED` outcome carrying a non-array is a **failure**, not `[]` — `[]` asserts "nothing scheduled".
  - **QA evidence (Preview `033987aa`, isolated QA account), every reading gated on `visible` + `hasFocus` + rAF advancing:** **7a** — first open, read blocked (4 blocked, 0 successful) → `📅 לא הצלחתי לטעון את היומן`, never `אין רשומות`, never `הכל רגוע`, no `HH:MM` leaked, marker written once *after* the briefing. **7b** — read succeeds and lists the row, then a reopen with the read blocked → the **notice**, stale row **absent**. Earlier full QA on Preview `8a54751a` covered briefing order, exclusions, the 5 + `ועוד 2` cap **measured on the wire**, the week count, the **3,919 ms** bounded wait on a genuinely hung request, and **0 unintended writes**. Cleanup verified **against the database**: zero residue, controls unchanged.
  - **⚠️ Two limitations recorded, not closed.** (1) The marker-ordering and bounded-wait guards are **source-pinned, not behavioural** — this repo has no jsdom, no testing-library and zero rendered-component tests, so QA 7a/7b are the only behavioural proof. (2) **The calendar snapshot is taken at panel open**: rows created while Jake is open are not reflected until it is reopened. That is the declared design (one read per open, not per message), observed live during QA, and it is a **known trade, not a defect**.
  - **⚠️ A phantom was raised and dismissed under measurement.** A modal appeared stuck on `שומר…` — measured in a **hidden** browser pane where `requestAnimationFrame` never fires. Re-tested visible, it closed cleanly. This re-confirms the existing `Modal Polish (b)` finding: **never conclude anything about transient UI from a hidden pane.**
...
- ✅ **Asset Library (gallery images)** — **durable in cloud (slices 1 + 2 + 3 + 4). CLOSED / LIVE.** Private `assets` bucket + `public.assets`, owner-only, signed-URL access, symmetric 40 quota (slice 1), favourites (slice 2), the optional campaign link (slice 3), and **USER FILE UPLOAD (slice 4, LIVE 2026-08-05)** — a user's own PNG/JPEG/WebP up to 10 MiB now enters the library, not only generated images. Scope still **open**: approve/reject, usage rights, versions, cross-account sharing, **logo / business profile image**, inventory or product images, replacing an existing asset's bytes, multi-file upload, drag-and-drop and video are **NOT** included.
...
- [x] **Jake sees the calendar (durable appointments → Jake's context + daily briefing, read-only): DONE, CLOSED / LIVE, OWNER-QA PASSED** *(first released as Production `123ea5aa` / `index-CBCh_jpO.js`, source `e3555eed`, PRs [#199](https://github.com/natanMeT/ArtValue20/pull/199) + [#200](https://github.com/natanMeT/ArtValue20/pull/200))*; that deployment is **now the current rollback target**, and the calendar behaviour is carried forward in the live bundle. **Do not re-select it.** Two things it deliberately did NOT do, each a separate slice if wanted: **Jake writing an appointment** (create / reschedule / cancel — no Jake op exists and none was added), and **campaigns or assets reaching Jake's context**, which stayed out of scope so a single panel-open read could not become a three-way fan-out with three partial-failure states. ⚠️ *(HISTORICAL — this was true when written and is NOT current: it said `public.campaigns` and `public.assets` were durable, live and still invisible to Jake, neither even declared absent to him. **BOTH ARE NOW CLOSED AND LIVE** — campaigns by PR [#202](https://github.com/natanMeT/ArtValue20/pull/202) and assets by PR [#208](https://github.com/natanMeT/ArtValue20/pull/208).)*
- [x] **Jake Campaigns Context (durable business campaigns → Jake's context, read-only): DONE, CLOSED / LIVE, OWNER-QA PASSED** *(Production `af65f5fe` / `index-CqSrIXWY.js`, source `521b7a0b`, PR [#202](https://github.com/natanMeT/ArtValue20/pull/202))*. **Do not re-select it.** Three things it deliberately did NOT do, each a separate slice if wanted: **Jake writing a campaign** (create / update / delete / status — no Jake op exists and none was added), the **`tasks.campaign_id` → Jake join** (so Jake knows campaign titles but not which tasks belong to them), and **assets**. ⚠️ *(HISTORICAL — this was true when written and is NOT current: it said `public.assets` was durable, live and still invisible to Jake, not even declared absent to him. **CLOSED AND LIVE 2026-08-05** by the Jake Asset Library Context slice, PR [#208](https://github.com/natanMeT/ArtValue20/pull/208), Production `0a2669bb` / `index-B3o3MS7k.js`.)*
- [x] **Jake Asset Library Context (the durable cloud asset library → Jake's context, read-only, metadata-only): DONE, CLOSED / LIVE, OWNER-QA PASSED** *(Production `0a2669bb` / `index-B3o3MS7k.js`, source `3681d77`, PR [#208](https://github.com/natanMeT/ArtValue20/pull/208))*. **Do not re-select it.** Five things it deliberately did NOT do, each a separate slice if wanted: **Jake writing an asset** (upload / delete / favourite / campaign link — no Jake op exists and none was added), **asset CONTENT understanding** (Jake sees metadata, never the image), **quote detail** (Jake still gets only a pending-quote COUNT while `quotes` are fully hydrated by `fetchAll`), the **`tasks.campaign_id` → Jake join** (Jake knows campaign titles and task titles, not which tasks belong to which campaign), and **transaction detail** (aggregated into the monthly KPI roll-up only). ⚠️ **The pre-settle window is now true of THREE seams** (calendar, campaigns, assets) and closing it would be one slice across all three — see the limitations box at the top of this file.
- [x] **Asset Library slice 4 (user file upload into the durable gallery): DONE, CLOSED / LIVE, OWNER-QA PASSED** *(Production `378115bc` / `index-BiwDxmw5.js` — **now the CURRENT ROLLBACK TARGET, no longer live**, source `45243b53`, PR [#204](https://github.com/natanMeT/ArtValue20/pull/204))*. **Do not re-select it.** Six things it deliberately did NOT do, each a separate slice if wanted: **multi-file select**, **drag-and-drop**, **replacing or editing an existing asset's bytes**, **logo / business profile image** (⚠️ that one is NOT the cheap follow-on it looks like — `business_profile` has **no logo column**, there is **no logo bucket**, and `quoteIssuer.test.js` deliberately pins QuotePrint as logo-free to stop one account's brand leaking into another's document; it needs a migration, a storage decision and an owner decision to reopen that invariant), **inventory / product images**, and **video**. ⚠️ **Two QA limits carried forward rather than closed:** the **OS picker → `input.files` hand-off was never directly exercised** (a two-minute manual pick on Production would close it), and the **client magic-byte guard is UX/truthfulness only — server-side L3 (declared MIME) and L1 (quota concurrency) both remain open.**
- [x] **Jake Pre-Settle Truthfulness (campaigns + assets stop claiming "not connected" during the ≤4s seam-read window): DONE, CLOSED / LIVE, OWNER-QA PASSED** *(Production `024c0aa7` / `index-FzGUTjjq.js`, source `a48020f`, PR [#210](https://github.com/natanMeT/ArtValue20/pull/210))*. **Do not re-select it.** Five things it deliberately did NOT do, each a separate slice if wanted: **the calendar's pre-settle wording** (proven not false — it asserts no disconnection and no zero — so the three-seam estimate this file previously carried was NARROWED to two); **re-arming `pending` on every panel re-open** (owner decision D2 chose the conservative option; re-open staleness is unchanged and pre-existing); **Jake quote detail** (⚠️ the cheapest remaining Jake slice by a wide margin — `quotes` are ALREADY fully hydrated by `api.fetchAll()`, so it needs **zero new IO, no new lane, no timer and no pre-settle exposure**; it is a `jakePack.js`-only context-shaping change plus a measured cap); the **`tasks.campaign_id` → Jake join**; and any **Jake write op**. ⚠️ **The QA account retains one asset Claude did not create** (`62c5e57a-…`, 1.7 MB), left untouched by owner instruction — not a blocker.
- [x] **Jake Quote Detail (durable quotes → Jake's context as capped DETAIL, read-only): DONE, CLOSED / LIVE, OWNER-QA PASSED** *(Production `47497fdb` / `assets/index-cQ2OpZHP.js` / 799,495 B — **that DEPLOYMENT is now the CURRENT ROLLBACK TARGET and no longer the live one; the behaviour is carried forward in `d7b9469b`**, source `5b63f6d1`, PR [#212](https://github.com/natanMeT/ArtValue20/pull/212))*. **Do not re-select it.** It closed what the Jake Asset Library Context box called the cheapest remaining Jake slice: `quotes` were ALREADY fully hydrated by `api.fetchAll()` (rows AND items), so it needed **zero new IO, no new lane, no timer and no pre-settle exposure** — a `jakePack.js`-only context-shaping change plus a measured cap. **Five things it deliberately did NOT do, each a separate slice if wanted:** any **Jake quote write op** (create / edit / send / status / delete — none exists and none was added); **line items and notes in context** (unbounded free text, the `meta.prompt` exclusion class); the **`tasks.campaign_id` → Jake join** (not delivered by this slice, and the obvious next Jake step); **transaction detail** (still aggregated into the monthly KPI roll-up only); and any **Assistant UI change**, including the **animated SVG state system**, which was assessed and declined for now — see the ✅ box for the reasoning and the condition under which it becomes a good slice. ⚠️ **TWO QA BRANCHES WERE NOT EXERCISABLE THROUGH THE UI and are NOT recorded as passes:** a quote with **no client** (the form's client field is required) and a **₪0 quote** (the form refuses a zero total). Both are covered by the executed-artifact controls only. ⚠️ **One pre-existing defect booked, not fixed:** `src/lib/calc.js:131` dereferences `q.status` unguarded, so a null quote row throws in `dashboardKpis`. **Unreachable via `fetchAll`** (`rowToQuote` cannot yield null, unlike `charges`/`payments`), so it is an owner decision rather than a carried release item.
- [x] **Accessibility A6 (AppointmentModal's eight `.field` labels → `id`/`htmlFor`): DONE, CLOSED / LIVE, OWNER-QA PASSED 8/8 with ZERO writes** *(Production `d7b9469b` / `assets/index-DryJ81XU.js` / 799,915 B — **that DEPLOYMENT is now the CURRENT ROLLBACK TARGET and no longer the live one; the behaviour is carried forward in `08f818d3`**, source `d5d86b94`, PR [#214](https://github.com/natanMeT/ArtValue20/pull/214))*. **Do not re-select it.** ⚠️ **The bucket is now 74, not 82** (TOTAL 84, not 92), and the hidden-file class is **6, not 5**. ⚠️ **The shared-`Field`-component decision was NOT taken here** — hand edits continued by owner direction, and it stays open for whoever takes the next `.field` slice.
- [x] **Jake Task↔Campaign join (which open tasks belong to which active campaign → Jake's context, read-only): DONE, CLOSED; its behaviour is still LIVE, carried forward in `a78d3ecc`** *(first released as Production `08f818d3` / `assets/index-C53vjfHW.js` / 801,488 B — **that DEPLOYMENT is now the CURRENT ROLLBACK TARGET and no longer the live one**, source `cceaf222`, PR [#216](https://github.com/natanMeT/ArtValue20/pull/216))*. **Do not re-select it.** It closed what the Jake Quote Detail box called "the obvious next Jake step". **ZERO NEW IO** — `api.fetchAll()` already maps `campaign_id → campaignId` (`api.js:172`) and campaigns already ride the Jake seam, so it is a `jakePack.js`-only context-shaping change: no new lane, no timer, no fetch, and **no new absence state** (it inherits the campaigns lane's existing three). **ONE DIRECTION ONLY — campaign → its open tasks, never task → its campaign**, because tasks are not individually listed anywhere in the context and a per-task clause would mean creating a new listing surface over a collection with no quota and no title bound. `TASKS_PER_CAMPAIGN_CAP = 3`, titles sliced at 60, store array order, no comparator. **Five things it deliberately did NOT do, each a separate slice if wanted:** any **Jake task or campaign write op** (none exists and none was added); **task notes in context** (unbounded free text, the `meta.prompt` exclusion class); **transaction detail**; **per-task campaign attribution**; and any **Assistant UI change**. ⚠️ **THE `קמפיין` INTERCEPTION IS OPEN AND PRE-EXISTING** — see the carried item in the ✅ box. ⚠️ **TWO BRANCHES ARE NOT UI-TESTABLE and are NOT recorded as passes:** a task with an **empty title** (the form requires one) and a **truthy non-array `campaigns`** value; both covered by executed-artifact controls only.
- [x] **C1 Campaign Interception (creative containment triggers on creative-ACTION intent, not on the word `קמפיין`): DONE, CLOSED / LIVE, OWNER-QA PASSED** *(Production `a78d3ecc` / `assets/index-Bere1YEI.js` / 802,041 B, source `5dc2a763`, PR [#218](https://github.com/natanMeT/ArtValue20/pull/218))*. **Do not re-select it.** It closed the item the Jake Task↔Campaign release carried as "the campaign side of Jake is hard to reach conversationally until this is addressed". **Rule shipped:** contain when the campaign noun is present AND (a build verb is present OR no informational / CRM-lead / drafting frame is) — build intent is absolute, and a bare or ambiguous noun **fails closed** into containment. **ONE rule for cloud and local/demo** — the predicate never reads the mode; the only divergence stays at the existing `isSupabaseConfigured` gate. **The containment message is byte-identical** and lane 4.5 keeps its position. **Frontend only, `src/lib/api.js` and `supabase/functions/**` untouched.** ⚠️ **A REAL, PRE-EXISTING S0F.1 CONTAINMENT HOLE WAS FOUND AND CLOSED HERE:** `/קמפיי?ן/` accepted only the FINAL nun `ן` (U+05DF), while the Hebrew plural spells that consonant with the MEDIAL nun `נ` (U+05E0) — so **no plural form was ever a campaign**, and `תבנה לי קמפיינים חדשים` reached the model while the singular form of the same request was contained. Fixed with `/קמפיי?[ןנ]/`. It also meant three approved QA controls were passing **vacuously**; each gained a singular twin. **Seven mutation controls** each turn the suite red, including one that restores the singular-only noun. **Four things it deliberately did NOT do, each a separate slice if wanted:** the `systemCapabilities()` truthfulness follow-up; any **Jake campaign write op**; the **re-open / pre-settle wording** item (not reproducible on a clean settled load); and the two accepted fail-closed residuals (a build verb winning inside a question, and English `campaign` now being matched at all).
- [ ] **Slice after those — PENDING NATHAN DECISION.** Do NOT begin/design/invent the next slice until Nathan selects one and approves a spec. Candidate open items: **account-aware Growth data model** (the last of the three Growth reopening prerequisites); **Asset Library beyond gallery images** (approve/reject, usage rights, versions, logo/brand assets — **favourites is DONE and LIVE (slice 2), and CAMPAIGN ASSOCIATION is DONE and LIVE (slice 3, deployment `be068244`). Do not re-select either**); **video generation** (a GENERATION-lane slice, not a storage one — animated WebP is already an allowed MIME; what blocks it is that the cloud lane cannot produce video at all and no managed provider has been chosen. **UNSTARTED — nothing about this has begun.** ⚠️ Do NOT read the 2026-08-01 Video Tab Truthfulness release as progress here: it only stopped the UI *claiming* the capability. The tab now reads `וידאו · בקרוב`, so the visible symptom is gone while the capability gap is entirely unchanged); **accessibility cleanup — slices A1, A2, A3, A4, A5 and A6 are DONE and LIVE; the REST is still a candidate.** ⚠️ **THE FIGURES IN THE REST OF THIS BULLET ARE PRE-A5/A6 AND ARE NOT CURRENT — the live, tool-measured list is the "REMAINING 84" table at the top of this file, and it is the one a next slice must be chosen from.** Current measurement: `visible-label-not-associated` **74**, `no-visible-label` **10**, **TOTAL 84**, hidden `input[type=file]` **6** (not 5 — slice 4 moved it). A5 closed `TaskModal`'s 9 and A6 closed `AppointmentModal`'s 8; **do not re-select either.** Retained below as written** (⚠️ **the "~156" recorded here on 2026-08-02 was a crude count and is CORRECTED: a parser-backed scan found 157 controls, 140 real gaps before A1, 130 after A1, 128 after A2 and 107 after A3+A4.** A1 shipped the 10 nameless to everyone; A2 the two Login controls; A3 ItemModal's 9; A4 ClientModal's 12 — do not re-select any of those 33. **What remains splits into three buckets needing three different fixes: 86 `.field` "visible label, not associated" (no shared `Field` component exists — a shared-component-vs-hand-edits decision that has STILL NOT been taken), 21 other nameless controls, and **6** hidden `input[type=file]` behind styled buttons (⚠️ **was 5; slice 4 added the gallery upload input 2026-08-05 — but its fix class was DECIDED there** (name on the visible trigger button, input `hidden` + `tabIndex={-1}`), so the open set is still the same **five**: `MockupStudio` ×2, `ProjectDetail` ×2, `Settings` ×1. This bucket is excluded from `totalRemainingGaps`, so no published gap count moved) (`aria-label` vs `aria-hidden`+`tabIndex`, decided per case).** ⚠️ **THE NEXT SLICE MUST BE CHOSEN FROM THE REACHABLE COLUMN — this is the A3 lesson and it is now a CI-asserted gate, not advice. REACHABLE candidates *(⚠️ AS WRITTEN THEN — `TaskModal` was closed by A5 and `AppointmentModal` by A6; the current list is the "REMAINING 84" table at the top)*: ~~`TaskModal` 9~~ · `ChargeModal` 9 · ~~`AppointmentModal` 8~~ · `Intake` 6 · `QuoteModal` 5 · `TransactionModal` 4 · `Diagnose` 4 · `Outreach` 3 · `PaymentModal` 2. UNREACHABLE / NOT valid candidates: `ProjectDetail` 16 and `ProjectModal` 12, both behind `<BetaUnavailable/>` — they are the two LARGEST remaining files and they are still not next. `ItemModal` is now fixed but LATENT.** ⚠️ **ALSO BINDING: classify each site as singleton-or-repeated. A literal `id` is safe only where the screen is PROVEN to render at most once; a repeated component must derive its `id` from stable domain data, or N rows share one `id` and every label binds to the first control.** ⚠️ **The 86 is a measured scan result, not a CI-enforced invariant, and `src/lib/__tests__/accessibleNames.test.js:8` carries it as a hand-edited comment that must be updated by hand each slice.** Scoping it to "the 5 the DevTools panel showed" would have no verifiable definition of done. The Monthly Plan slice added none — its five inputs are `id`+`htmlFor` paired, pinned by a test. Originally recorded as: NEW, non-blocking, surfaced by the owner's DevTools Issues panel during the 2026-08-01 Production QA: *"a form field element should have an id or name attribute"* ×1 and *"no label associated with a form field"* ×4. **Pre-existing and unrelated to Favorites or the video tab** — recorded so they do not evaporate. ⚠️ Also seen in that panel and **investigated and dismissed**: *"CSP blocks the use of 'eval'"* — **not ours**: the site sends **no CSP header**, has no CSP meta tag, and `eval(`/`new Function(` appear **0 times** in both `src/` and the served bundle, so it originates in a browser extension); **Products / Projects / Inventory / Templates / Activity durability** (⚠️ Projects are the largest of these: they live ONLY in the local store — `src/store/store.jsx` reduces `ADD_PROJECT`/`UPDATE_PROJECT`/`DELETE_PROJECT` and `api.js` has no projects route at all, so this is a new table + RLS + composite FKs + a UI rewrite, not one slice); **organization boundaries**; **credits / cost controls**; **Website Scanner** (per the section above); Jake conversation-refresh UX. **NOT candidates — already done, and selecting either would mean re-implementing merged work:**
...
## Next action
**Nothing is mid-flight. As of 2026-08-07 there is no undeployed slice and no release gate awaiting an owner.**
Production is `a78d3ecc` / `assets/index-Bere1YEI.js` / **802,041 B** (source `5dc2a763a35e3d52c4ee0547b8fe5a370c47082c`, C1 campaign interception — the frontend is UNCHANGED by J1), migrations are **16 applied of 16 tracked, none pending**, Edge `ai-gateway` v37 is ACTIVE / `verify_jwt=true` (J1 context ordering, deployed 2026-08-07; Edge rollback = the v36 source at tag `pre-j1-context-ordering` → `66853d4e229e7a5bd548e32f99ca41500cf1486d`, redeployed as a NEW version if needed), and the current FRONTEND rollback target is **`08f818d3`** / `index-C53vjfHW.js` (with `d7b9469b` / `index-DryJ81XU.js` demoted to historical fallback). Rollback tag `pre-c1-campaign-interception` @ `cceaf2228c527d7ef511230a90e1e0eb8e9ce946`, created before the deploy and pushed.

**THE APPROVED ROADMAP (owner decision, 2026-08-07): J1 stale-chat context ordering (CLOSED / LIVE, this release) → J3 `systemCapabilities()` truthfulness → J2 Jake Receivables Detail; Calendar Write is DEFERRED to fourth position. J3 IS NOT STARTED — execution remains PENDING AN EXPLICIT NATHAN GATE. Do not design or begin it without that gate.**

**THREE ITEMS CARRIED FROM THE C1 RELEASE, none a blocker and none a C1 defect:**
1. ✅ **Persisted-chat stale echo — CLOSED / LIVE 2026-08-07 by J1 (Edge v37): the context now folds into the CURRENT user turn with an explicit precedence rule, and the real-data QA reproduced this exact scenario WITHOUT clearing chat — Jake answered from the fresh data.** The QA hygiene advice below is retained for historical reads of older sessions, but clearing the chat is NO LONGER REQUIRED for correctness. *(As written at the C1 release, retained: "Persisted Jake chat can echo stale prior data and looks exactly like a broken context lane.")* During the C1 real-data QA, a campaign that had just been created and activated was reported by Jake as absent — because the persisted conversation still held the earlier empty-account answers. Clearing the chat key and reloading produced the correct data-grounded answer immediately. **Any future Jake QA must clear the chat first, or it will manufacture defects.** Reporting the first reading would have booked a phantom.
2. ⚠️ **The re-open / pre-settle wording observation (`המודול אינו מחובר` where the module is connected and merely empty) was NOT reproducible on any clean, settled load.** Every clean full-page load produced the correct wording with the seam read observed returning `GET 200`. Carried as a **separate pre-existing seam-timing item** — D2 deliberately does not re-arm `pending` on a later panel open — **not attributed to C1 and not expanded into it. Untouched by J1 (which changed context ORDERING, not seam timing) — STILL CARRIED and unresolved.**
3. **Two accepted C1 residuals, both failing CLOSED and both pinned by tests:** a build verb wins inside a question (`מי בנה את הקמפיין הזה?`, `how did the campaign run?` are contained although informational), and English `campaign` is now matched where the old bare-noun matcher never matched it, so `build a campaign for me` is contained where it previously was not. Containment is stricter in English — a widening, not only a narrowing.

**Also still carried, and untouched by C1:** the `systemCapabilities()` truthfulness follow-up (it still enumerates every live workflow regardless of what the runtime can execute) is **more exposed now**, because informational campaign traffic reaches the chat lane. **Pre-existing, not caused by C1.** ✳️ **SELECTED 2026-08-07 as the NEXT slice (J3) by the approved roadmap — NOT STARTED; design and execution remain PENDING AN EXPLICIT NATHAN GATE.**

**SIX ITEMS CARRIED FROM THE JAKE TASK↔CAMPAIGN RELEASE, none a blocker:**
1. ✅ **Jake's `קמפיין` interception — CLOSED / LIVE 2026-08-07 by the C1 slice, deployment `a78d3ecc`.** *(As written at the task↔campaign release, retained: "PRE-EXISTING and STILL OPEN. The bare word `קמפיין` is caught by the S0F.1 creative-containment lane, which answers "בניית קמפיין קריאייטיבי עדיין אינה זמינה בגרסת הבטא בענן" instead of reaching Jake's context. It fired on **3 of the 5 scripted QA questions**; rephrasing without the word reached the context correctly every time. Not caused by the task↔campaign slice and not fixed by it.")* **It was selected, specified, implemented and released as C1** — containment keeps its precedence, its position and its message, and now triggers on creative-ACTION intent instead of the word alone. See the C1 box in the Baseline.
2. ⚠️ **The deletion-test expectation is CORRECTED, not carried as a defect.** `tasks.campaign_id` uses **`on delete set null (campaign_id)`** (migration `20260729120000`), and the campaign-delete confirm dialog states it in the UI. So after a full reload the ex-linked tasks are genuinely unlinked in the database and **"not linked" is the TRUTHFUL answer**. The third bucket ("linked to a campaign whose name is unavailable") is reachable only while the task snapshot is stale against a refreshed campaigns read — that is where it was exercised, and it passed.
3. **Two task↔campaign branches are NOT UI-testable and are NOT recorded as passes:** a task with an **empty title** (the form requires one) and a **truthy non-array `campaigns`** value. Both are covered by executed-artifact controls only. **A control that never fired has not passed.**
4. **No authenticated Production QA was run.** Preview QA passed on the isolated QA account with the byte-identical artifact, so behaviour is established; Production behaviour for a signed-in user is not independently observed. Same posture as several prior releases.
5. **The shared-`Field`-component decision is STILL untaken** for the remaining 74 `.field` gaps. A6 continued hand edits by owner direction; the choice is deferred, not made. Untouched by the task↔campaign slice.
6. **`src/lib/calc.js:131` null-row fragility** — still booked, unreachable via `fetchAll`, an owner decision. Untouched by A6 and by the task↔campaign slice. *(The a11y bucket figures also still live in a hand-edited comment in `accessibleNames.test.js`, not a CI invariant — edit by hand each slice and re-run the tool.)*

*(⚠️ CARRIED FORWARD from the Jake Quote Detail release and still true: two quote branches remain unexercised through a UI — a quote with no client, and a ₪0 quote. The form blocks both; covered by executed-artifact controls only and **not** recorded as passes.)*

*(⚠️ CORRECTED 2026-08-06 — HISTORICAL quotation, superseded and NOT current: this paragraph previously read "As of 2026-08-05 … Production is `378115bc` / `index-BiwDxmw5.js` … rollback target `af65f5fe`", i.e. Asset Library slice 4 — **four releases behind**. Same stale-block class this file keeps catching; Layer A does NOT catch it, because the checker keys off the `Current Production deploy`, `CURRENT frontend rollback target` and `Active application release code anchor` anchors alone. A stale sentence here passes every automated gate and has to be re-read by hand each release.)*
Production is `378115bc` / `index-BiwDxmw5.js` / **794,709 B** (source `45243b53`, Asset Library slice 4 — user file upload),
migrations are **16 applied of 16 tracked, none pending**, Edge `ai-gateway` v36 is ACTIVE / `verify_jwt=true`, and
the current rollback target is `af65f5fe` / `index-CqSrIXWY.js` (with `123ea5aa` / `index-CBCh_jpO.js` demoted to
historical fallback). Rollback tag `pre-asset-user-upload` @ `df9cfae1ea0546fc035237ceb845ff660cb413b2`, pushed.

**ONE OPEN ITEM carried from this release, and it is a two-minute job, not a blocker:** the **OS file-picker → `input.files`
hand-off was never directly exercised** during QA — the tool that drives a native picker failed, so each file was delivered
to the shipped input as a genuine `File` on a real `change` event instead. Everything downstream is proven against real
Storage. A single manual pick on Production (`/#/studio` → `העלאת תמונה` → choose an image) closes it.

*(⚠️ CORRECTED 2026-08-05 — HISTORICAL quotation, superseded and NOT current: this paragraph previously read
"Production is `af65f5fe` / `index-CqSrIXWY.js` … rollback target `123ea5aa`". Both identifiers have moved one step back.
Same stale-block class this file keeps catching; Layer A does NOT catch it, because the checker keys off the
`Current Production deploy` and `CURRENT frontend rollback target` anchors alone — a stale sentence here passes every
automated gate and has to be re-read by hand each release.)*

*(⚠️ CORRECTED 2026-08-05 — HISTORICAL quotation, superseded and NOT current state: this paragraph had stood at
"As of 2026-08-03 … Production is `4855a30f` … rollback target `7be49ef5`" through **five** later releases; both of
those identifiers are historical fallbacks now. Same stale-block class this file keeps catching, and Layer A
does NOT catch it: the checker keys off the `Current Production deploy` and `CURRENT frontend rollback target`
anchors alone, so a stale sentence here passes every automated gate. It has to be re-read by hand each release.)* ⚠️ **One caveat carried forward rather than closed: A3 / ItemModal is live but LATENT — `Inventory.jsx`
...
## Change log
- **2026-08-07** — **J1 Stale-Chat Context Ordering: RELEASED / LIVE — the FIRST EDGE-ONLY release. `ai-gateway` v37 (context folds into the CURRENT user turn for `jake.chat` + `jake.draft_message`; server-side precedence rule; every other action byte-identical). Frontend UNCHANGED at `a78d3ecc` — control build 12/12 byte-identical. PR #220 merged as `2bf78bf8ef87a8ac73aa17cdd1efe4d38108ed61`; pre-deploy gates (byte-identity, local rehearsal with zero external calls, v36 capture + tag `pre-j1-context-ordering`) all passed; separately gated deploy; immediate smoke 401/200/200/200; real-data owner QA passed the exact C1 stale-echo scenario WITHOUT clearing chat; cleanup zero-residue; Jake zero writes. Edge rollback = v36 source redeployed as a NEW version. Carried stale-echo item CLOSED; pre-settle item still carried. Approved roadmap: J3 `systemCapabilities()` next (PENDING EXPLICIT NATHAN GATE), then J2 Receivables Detail; Calendar Write deferred.**
- **2026-08-06** — **Jake Task↔Campaign join: RELEASED / LIVE IN PRODUCTION. Jake stops knowing campaign titles and task titles without knowing which belongs to which.** PR [#216](https://github.com/natanMeT/ArtValue20/pull/216) merged as `cceaf222` (head-gated `7c32902d`, **3 files / +700 −5**); Preview `62595b33` owner-QA'd on the isolated QA account, then the same artifact promoted unchanged as Production **`08f818d3`** / `assets/index-C53vjfHW.js` / **801,488 B** / SHA256 `0A1072BD…D126C29`. Frontend only — no migration, no bucket, no Storage policy, no grant, no Edge, no package, no secret, **`src/lib/api.js` and `src/lib/assetLibrary.js` untouched**, `jakeAgent.js` untouched and pinned by a test. **ZERO NEW IO:** tasks ride the store (`api.js:172` already maps `campaign_id → campaignId`), campaigns ride the Jake seam — both were already in `data`. **THE CROSS-LIFECYCLE RULE IS THE WHOLE SLICE:** the two collections settle independently, so a naive join reports *"not linked to a campaign"* at exactly the moment the truth is *"I could not read the campaigns"*. The split is made on what each source can answer — **WHETHER** a task carries a campaign id is knowable from `tasks` alone and is stated in **every** campaigns state; **WHICH** campaign it is needs the campaigns read and is emitted **only when campaigns are hydrated**. It therefore adds **zero new absence states** and leaves `campaignLines()` byte-identical. **THREE BUCKETS, NOT TWO** — a task whose `campaignId` resolves to nothing is **still linked** and is reported as linked-but-unnameable without asserting a cause; `campaignLabelForAsset` is reused as-is. `TASKS_PER_CAMPAIGN_CAP = 3`, titles sliced at **60**, store array order, no comparator; **EXCLUDED: `notes`, `id`, `clientId`, `projectId`, `assignee`, `linkRef`, `priority`, `deadline`.** **THE BUDGET IS ARGUED LIKE THE QUOTE SLICE'S, BUT THE CASE IS WORSE:** `public.tasks` has **no row quota**, `title` is `text not null` with **no length check**, and `TaskModal`'s title input has **no `maxLength`** — unbounded from BOTH sides, so the hard slice is the only bound that exists. Substitute property MEASURED: this section costs **1,986 chars at 5,000 tasks and 1,994 at 50,000 — a 10× row increase costs EIGHT CHARACTERS**; full worst-case context **5,067 of the 12,000 the Gateway REJECTS rather than truncates**. The 8,000-char floor in `jakeAssets.test.js` was **measured first and did not need to move**. ⚠️ **THE APPROVED SPEC'S BOUND WAS WRONG AND IS CORRECTED:** it published `5 × 3 × 60 ≈ 0.9KB`, **omitting the campaign-title term**; the real bound is `CAMPAIGN_ACTIVE_CAP × (CAMPAIGN_LIMITS.title + 3 × 60)` ≈ 1.8KB — still every term a constant and no term a row count, but the published figure was understated. **Owner QA on the isolated QA account (`natanturgeman5@gmail.com`, uid `54281dc5-a1ea-4328-81df-937681778d37`), executed by Claude in the owner's already-authenticated browser — no credential entered, no account created, the real account never signed in; identity read from the JWT BEFORE the first write, and Production never opened.** DB-backed baseline 0 campaigns / 0 tasks. **EIGHT CASES PASSED**, including *"המשימות הפתוחות המשויכות לקמפיין QA-TASK-CAMP-A הן: QA-משימה-4 · QA-משימה-3 · QA-משימה-2 · ועוד משימה אחת נוספת שאינה מפורטת"* (cap + exact overflow, the `done` task absent), *"5 מתוך 6 המשימות הפתוחות משויכות לקמפיין, ומשימה אחת אינה משויכת"* (the task on the **completed** campaign correctly counted linked), and **the third bucket live: after deleting campaign A, *"מתוך המשויכות, 4 משויכות לקמפיין ששמו אינו זמין לי כרגע — אל תאמר שהן אינן משויכות לקמפיין ואל תנחש את שמו"*.** Two questions where the task was beyond the cap drew *"אין לי מידע על המשימה … בנתונים שסופקו"* — **truthful refusal, no fabrication**. **THE LEAK EVIDENCE IS THE ACTUAL GATEWAY PAYLOAD, NOT THE PROSE:** the `POST /functions/v1/ai-gateway` body was intercepted and the `context` inspected directly — 2,609 chars, **positive controls fire** (`QA-משימה-4`, `QA-TASK-CAMP-A` present) while the notes marker, the capped-out task, the `done` task, the unlinked task's name and the completed-campaign task's name are **all absent**. ⚠️ **One measurement error caught and corrected mid-QA:** the first leak check searched the WHOLE payload and reported the marker present — it was **Claude's own typed question** in `messages`, not the context. **Jake performed ZERO writes.** Cleanup verified **zero residue DB-backed** (0 tasks, 0 campaigns, 0 QA-named rows, 0 notes markers). ⚠️ **THREE OF THE FIVE SCRIPTED QA QUESTIONS COULD NOT BE ASKED VERBATIM:** the bare word `קמפיין` is intercepted by the pre-existing **S0F.1 creative-containment lane**, which answers *"בניית קמפיין קריאייטיבי עדיין אינה זמינה בגרסת הבטא בענן"* instead of reaching the context — 3/3 times. **Pre-existing routing, NOT caused by this slice**, rephrasing avoided it, and it is carried as an open follow-up. ⚠️ **THE SCRIPTED DELETION EXPECTATION WAS WRONG AND IS CORRECTED:** the composite FK is `on delete set null (campaign_id)` (migration `20260729120000`) and the UI's own confirm dialog says so, so **after a full reload "unlinked" is the TRUTHFUL answer** — the third bucket is reachable only in the stale-snapshot window, which is where it was exercised and passed. **Served-artifact proof was EXECUTION, not grep:** eight spans of minified logic were sliced out of the bundle downloaded off the Preview URL (SHA matched first, harness refuses otherwise) and run in a VM — **37/37 controls passed**, every absence check paired with a positive control. **Mutation controls 9 of 10 bite**; ⚠️ **M5a SURVIVED and is an EQUIVALENT MUTANT, not a coverage hole** — the campaigns guard is doubled, so removing either half alone changes nothing observable, while removing **both** crashes the suite (`TypeError: .filter is not a function`), which M5b pins. ⚠️ **M7 initially SURVIVED and that was a REAL GAP IN THE TEST, not the code** — every fixture task had a truthy title, so a predicate forked to `status !== 'done' && t.title` was indistinguishable; a task with an **empty** title was added to the parity fixture and M7 now bites. ⚠️ **THE MUTATION HARNESS LEFT TWO MUTATIONS IN THE WORKING TREE while reporting a matching SHA** — caught by a direct source read, both guards restored, the whole diff re-verified line-by-line against `origin/main`, and every figure re-measured from the restored state; the harness was rewritten to keep the pristine copy outside the repo and assert SHA-256 after every mutation. Full suite **157 files / 4,512 passed / 0 failed** (from 156 / 4,459). Promotion reported **"Uploaded 0 files (12 already uploaded)"**; canonical ↔ local **12/12 byte-identical** and the deployment URL independently 12/12; smoke **19/19 HTTP 200, GET only, zero writes**. ⚠️ **The first canonical parity fetch mismatched again — SEVENTH consecutive release — this time the NEW asset URL returned the 1,285 B SPA fallback rather than the 801,488 B bundle. Edge cache/fallback on a brand-new asset path, NOT a deploy defect: the deployment URL was already 12/12 at that same moment, and a no-cache re-poll returned the correct bytes and SHA on the FIRST retry.** **Current rollback target advances to `d7b9469b`** / `index-DryJ81XU.js` / 799,915 B, **re-downloaded and re-hashed HTTP 200 after the deploy, still serving its own bundle**; `47497fdb` / `index-cQ2OpZHP.js` demoted to historical fallback; tag `pre-jake-task-campaign` @ `e06cc698` created **before** the deploy and pushed, confirmed on `origin` and dereferenced. **No authenticated Production QA was run.**
- **2026-08-06** — **Accessibility A6 (AppointmentModal labels): RELEASED / LIVE IN PRODUCTION. Eight visible Hebrew labels that named nothing now name their controls.** PR [#214](https://github.com/natanMeT/ArtValue20/pull/214) merged as `d5d86b94` (head-gated `bdd48292`, **2 files / +284 −13**); Preview `0470d5b0` owner-QA'd on the isolated QA account, then the same artifact promoted unchanged as Production **`d7b9469b`** / `assets/index-DryJ81XU.js` / **799,915 B** / SHA256 `9E033ED5…45E4346F`. Frontend only — no migration, no bucket, no Storage policy, no grant, no Edge, no package, no secret, `src/lib/api.js` untouched. **Additive `id`/`htmlFor` only**: no CSS, no layout, no `useId`, no wrapping label, **no shared `Field` component** (that decision remains open and untaken), no behaviour change. Ids `appointment-kind · -title · -date · -start-time · -end-time · -client · -task · -notes`, **all eight unconditional**. **Literal ids are justified by a singleton proof, not assumed** — one mount site, one route, `mode="wait"` on the page transition, `{open && …}` in `Modal.jsx`, no host-declared `appointment-*` id — each half pinned by a test. **OWNER QA: 8/8 label-click → `document.activeElement` is the expected control, with Chrome's own 8 forwarded clicks as independent corroboration; `[id^="appointment-"]` = 8, duplicates 0, every `label.control` the same node as `getElementById(htmlFor)`; 0 before opening and 0 after closing. ZERO WRITES — 3 requests, all GET, all inline `data:` URIs, no REST/Auth/Storage call at all, no appointment created, no residue.** Non-regression confirmed live: `maxLength` 160/2000 intact, the `סוג` select changed by a real keypress, date/time keep `type` and `dir="ltr"`, and an empty submit produced the existing errors *"צריך למלא כותרת."* / *"צריך למלא תאריך ושעת התחלה."*. **Served-artifact proof was EXECUTION, not grep** — the component was extracted from the bundle downloaded off the Preview URL (SHA matched first) and run under a stub JSX runtime: 8 labels, 8 controls, 0 duplicates, 0 unpaired, with positive controls, a fabricated-id negative control, and the live Production bundle showing **0** occurrences of `appointment-`. **A11Y COUNT, measured with the tool: `visible-label-not-associated` 82 → 74, `no-visible-label` 10 unchanged, TOTAL 92 → 84, hidden `input[type=file]` 6 unchanged**; gate `--expect 74,10,6` matches. ⚠️ **THE HIDDEN-FILE COUNT IS 6, NOT THE STALE 5** the approved spec inherited — slice 4 moved it, A6 did not touch that bucket, and the correction is now in `accessibleNames.test.js` too. ⚠️ **THE DURABLE LESSON: a guard I wrote FAILED ITS OWN MUTATION.** The first MOUNT SITES test counted *files containing a mount* (A5's shape) and a second `<AppointmentModal>` **inside the same host survived it** — the exact state that would put two sets of literal ids in one document. It now counts **occurrences**; both same-file and cross-file mutations bite, plus a new `SINGLE OPEN FLAG` test. *"How many mount sites" was still a proxy, one level further in than A5 found it.* **Mutation controls 11/11 bite**, every mutated file restored byte-identical by SHA-256. Full suite **156 files / 4,459 passed / 0 failed** (from 156 / 4,439). **A control build from clean `main` reproduced the OUTGOING artifact byte-for-byte, so the +420 B delta is measured, not inferred.** Promotion reported **"Uploaded 0 files (12 already uploaded)"**; canonical ↔ local **12/12 byte-identical** and the deployment URL independently 12/12; smoke GET only, zero writes, no sign-in. ⚠️ **The first canonical parity fetch again mismatched on `index.html` — same 1,285 B length, different SHA, one differing line (the old bundle reference). Edge cache, SIXTH consecutive release; a cache-busting re-poll cleared it on the first retry.** ⚠️ **One measurement artifact recorded rather than booked as a defect:** the QA tab reported `visibilityState: "hidden"`, so after `ביטול` the overlay node still measured present (frozen exit-animation clock) and read 0 once a paint occurred — the documented hidden-tab trap, not a "modal does not close" defect. **Current rollback target advances to `47497fdb`** / `index-cQ2OpZHP.js` / 799,495 B, **re-downloaded and re-hashed HTTP 200 after the deploy**; `024c0aa7` demoted to historical fallback; tag `pre-a11y-a6-appointment` @ `91f26a88` created **before** the deploy and pushed. **No authenticated Production QA was run.**
- **2026-08-06** — **Jake Quote Detail: RELEASED / LIVE IN PRODUCTION. Jake stops answering quote questions from a bare count.** PR [#212](https://github.com/natanMeT/ArtValue20/pull/212) merged as `5b63f6d1` (head-gated `cc97bcbd`, 2 files / +553 −1); Preview `91705297` owner-QA'd on the isolated QA account, then the same artifact promoted unchanged as Production **`47497fdb`** / `assets/index-cQ2OpZHP.js` / **799,495 B** / SHA256 `A4BA90F3…48F8A3363`. Frontend only — **`src/lib/api.js` untouched** (blob `1555be73…` identical on both sides), no migration, no bucket, no Storage policy, no Edge, no package, no secret. **The whole slice is one new `quoteLines(data)` in `jakePack.js` plus its tests**: `quotes` were already hydrated WITH their line items by `api.fetchAll()`, so it added **zero IO**. **NOT A SEAM — that removed four states rather than one**: quotes ride the store, so there is no `quotesPending`, no `quotesError` and no ≤4s pre-settle window, and inventing a "not connected" wording for a collection that cannot disconnect would have been its own untruth. **`QUOTE_DETAIL_CAP = 6`**, pending-first then array order, never re-sorted by the user-entered `date`. Fields: number · client · VAT-inclusive total · status · date. **EXCLUDED: `notes`, item descriptions, `id`, `vatRate`, `validDays`.** **THE BUDGET IS ARGUED DIFFERENTLY HERE BECAUSE `public.quotes` HAS NO ROW QUOTA** — `quotes_own` carries ownership and nothing else, so the "fits at the quota ceiling" proof used by `jakeAssets.test.js` is unavailable: there is no ceiling to build. The substitute property is that the section is **O(1) in row count**, and it was MEASURED: 200 max-length quotes + the S0D business brain = **6,402 chars**, 5,000 quotes = **6,416** — **a 25× row increase costs 14 characters**, 53% of the 12,000 limit the Gateway REJECTS rather than truncates. **The 8,000-char regression floor in `jakeAssets.test.js` is UNCHANGED and still passes**; the pre-authorised raise was neither needed nor used. ⚠️ **The approving brief's figures (7,487 / 7,501) were NOT reproducible** — measured twice from a clean tree as 6,402 / 6,416 with identical deltas; the reproducible values are published and the discrepancy is stated in the PR body rather than silently reconciled. **Owner QA on the isolated QA account (`natanturgeman5@gmail.com`, uid `54281dc5-a1ea-4328-81df-937681778d37`), executed by Claude in the owner's already-authenticated browser at his explicit request — no credential entered, no account created, the real account never signed in.** The account began empty, so disposable rows were created first to make a wrong "אין הצעות" demonstrably false. **Seven cases PASSED**, including *"הצעת מחיר אחת ממתינה: AV-1041 ללקוח "QA בדיקה זמנית" בסך ₪2,360, נשלחה ב-5.8.2026"* (all five fields correct, the accepted quote correctly absent), the amount of an **accepted** quote (₪590, exercising D4), and **the leak test: `סודי-בדיקה-QA-7b2c` planted in the notes and `עיצוב לוגו` in the items — Jake answered *"אין לי גישה לפרטי הערות או תיאור פריטים ספציפיים... אני יכול לראות רק את מספר ההצעה, הלקוח, הסכום והסטטוס"*, zero leakage.** Cleanup verified **zero residue** after a full reload. ⚠️ **TWO CASES ARE NOT UI-TESTABLE AND ARE NOT RECORDED AS PASSES:** a quote with no client (required field) and a ₪0 quote (the form refuses it); both covered by executed-artifact controls only. **A control that never fired has not passed.** **Served-artifact proof was EXECUTION, not grep:** the builder was extracted from the bundle downloaded off the Preview URL and run with 24/24 controls — unique markers proved every excluded field absent, and **five positive controls proved those absences were not a dead check**. **Mutation controls 6/6 bite**, including the `dashboardKpis` parity control that catches a forked pending predicate. Full suite **156 files / 4,439 passed / 0 failed** (from 155 / 4,407). Promotion reported **"Uploaded 0 files (12 already uploaded)"**; canonical ↔ local **12/12 byte-identical** and the deployment URL independently 12/12; smoke **19/19 HTTP 200, GET only, zero writes**. ⚠️ **The first canonical parity fetch returned the OLD bundle — edge cache, NOT a failed promotion, the FIFTH consecutive release to show it**; a cache-busting query fixed it on the first retry. `/index.html` also **308-redirects to `/`**, which reads as a 0-byte mismatch without `curl -L` — that one is newly recorded. **Current rollback target advances to `024c0aa7`** / `index-FzGUTjjq.js` / 797,585 B, **HTTP 200 verified after the deploy still serving its own bundle**; `0a2669bb` demoted to historical fallback; tag `pre-jake-quote-detail` @ `68aeba0b` created **before** the deploy and pushed, confirmed on `origin` and dereferenced. ⚠️ **One spec deviation and one pre-existing defect booked:** the null-row test was dropped because **`src/lib/calc.js:131` dereferences `q.status` unguarded** and throws in `dashboardKpis` before `quoteLines` is reached — unreachable via `fetchAll`, so recorded as an owner decision rather than fixed inside this slice. **No authenticated Production QA was run.** **ALSO FIXED IN THIS TRACKER PR: the `Deployed Production source` anchor, which had stood FIVE releases stale** at `45243b53` / `378115bc` (Asset Library slice 4) — Layer A does not key off that phrasing, so it passed every automated gate while contradicting the `Current Production deploy` line three lines below it.
- **2026-08-05** — **Asset Library slice 4 (user file upload): RELEASED / LIVE IN PRODUCTION. The first time an upload has ever been executed against real Storage.** PR [#204](https://github.com/natanMeT/ArtValue20/pull/204) merged as `45243b53` (head-gated `dd8efedb`, 6 files / +716 −4); Preview `197b2fe3` owner-QA'd on the isolated QA account, then the same artifact promoted unchanged as Production **`378115bc`** / `index-BiwDxmw5.js` / 794,709 B. Frontend only — **`src/lib/api.js` untouched**, no migration, no bucket, no Storage policy, no Edge, no package. Rows and objects stayed symmetric (5/5, 40/40, 0/0) through the whole QA and residue was verified **zero in the database including the storage prefix**. Rollback target advanced to `af65f5fe`; `123ea5aa` demoted to historical fallback; tag `pre-asset-user-upload` @ `df9cfae1` pushed. ⚠️ Two limits carried, not closed: the **OS picker → `input.files` hand-off was not directly exercised** (the tool that drives a native picker failed; a genuine `File` was delivered to the shipped input by a real `change` event instead), and the **client magic-byte guard is UX/truthfulness only — L3 and L1 remain open on the server**.
- **2026-07-31** — **Finance Charge Safe Delete: the last open item CLOSED. Stale-tab refusal exercised; the error contract is now an executed test, not a source-text pin.** Two changes, neither touching production code. **(1) The stale-tab path was run and PASSED** — owner-driven, two tabs on the isolated QA account `natanturgeman5@gmail.com`: the same charge open twice, a payment recorded in tab B, then the **row trash icon 🗑️** clicked in tab A (**not** the cancel ✕). The server **refused**, the **charge stayed visible**, the **payment stayed recorded**, there was **no success toast**, and the **specific** message **"לחיוב הזה רשומים תשלומים…"** rendered rather than the generic fallback. That is the one path proving `23514` survives the **PostgREST → `supabase-js`** hop as `error.code` — the only link in the chain that had never been measured for this code (it had been measured for `42501` via an `anon` call). Cleanup through the UI in the only order the rule permits — payment first, then charge — and **zero residue verified read-only** as the owner role (`charges_total = 0`, `payments_total = 0`). The gap recorded on 2026-07-30 as an **accepted cosmetic gap** is therefore **closed after one day**, not carried. **(2) PR [#152](https://github.com/natanMeT/ArtValue20/pull/152) → merge `67a53f801053c00ddb0c42f32bc91ee64993f7e9`**, one new file `src/lib/__tests__/chargeDeleteErrorContract.test.js` (+151 / −0), **test-only**. The existing tests pinned `api.deleteCharge` by **source text** and never executed it, so nothing could say what a user actually sees when the server refuses. This one runs the shipped function against a stubbed transport and follows the value through the **real** `userFacingError` with the **real** fallback string read out of `store.jsx`: the RPC call **including the argument name** (PostgREST resolves by name, so a rename would 404 live while every source-text assertion still passed), `23514` → the payments-exist message, `P0002` → the not-found message **plus** an assertion that it leaks nothing about ownership (the migration raises one error for "no such charge" and "not your charge"), unknown or absent code → the generic fallback, success → `true`. The stub throws if `deleteCharge` ever reaches for `.from()` again — the cascade path this slice removed. **Two mutations measured, not assumed:** `engineError` → `throw error` fails the `23514` test, and `{ p_charge_id }` → `{ chargeId }` fails the call test; `api.js` restored byte-identical. Full suite **129 files / 3,748 passed / 0 failed** (from 128 / 3,738). **NO production code, NO migration, NO build, NO deployment, NO DB change, NO new slice:** Production remains `86d5cca9` / `index-o0xZrfkL.js`, migrations **13 applied of 13 tracked, none pending**, Edge `ai-gateway` v36 (ACTIVE at that release) / `verify_jwt=true`, untouched. **One open item remains on this slice:** no authenticated Production smoke was run for the release, unlike the previous three.
- **2026-07-30** — **Finance Charge Safe Delete: RELEASED / LIVE IN PRODUCTION. Migration applied on the first attempt; owner Preview UI QA PASSED.** PR [#150](https://github.com/natanMeT/ArtValue20/pull/150) → merge **`1eb7b2abd44249da83f1c4891d4a5d3fcd44a88c`** (parents `2c9bec42` + the owner-approved head `0db561a8`, `--match-head-commit`); rollback tag `pre-charge-safe-delete` @ `2c9bec42`. **Closes F1 gap (a):** a charge can be deleted from the UI, and **only if no payment row belongs to it**. `deleteCharge` already existed end-to-end but nothing called it — and exposing it as-is would have been a defect, because `payments_charge_same_owner_fk` is **ON DELETE CASCADE** and the old direct delete silently destroyed every payment attached to the charge. **A UI gate could not enforce this**, which is why the slice carries a migration: `received === 0` is a SUM over rows the client happens to hold and reads 0 while a payment row exists (dropped by `normalizePaymentRow`, recorded after the last `fetchAll`, or landing between the confirm click and the DELETE). The `payments_amount_positive` CHECK rules out the *arithmetic* cases (amount `0`, negative, net-zero) but **none of those three**. Adds **`public.delete_charge_if_unpaid(uuid)`** — `SECURITY DEFINER`, `set search_path = ''`, ownership from `auth.uid()` — which locks the charge `FOR UPDATE`, refuses on payment-row **EXISTENCE** (not a sum), then deletes with ownership repeated on the statement. **`SECURITY DEFINER` is load-bearing:** under INVOKER the existence check would be RLS-filtered and could miss the very row it exists to find. **Cross-account non-disclosure:** not-found and not-owned raise the identical `P0002` from a **single raise site**, ownership checked before payments — proven on both code and message text. **Race measured, not asserted:** `FOR UPDATE` conflicts with the `FOR KEY SHARE` a concurrent payment INSERT takes via the FK — the INSERT blocked **5.97 s** against a held lock vs **0.01 s** with no holder, on real PostgreSQL 17.6. **Rejected alternatives recorded in the migration header:** FK `CASCADE`→`RESTRICT` (checked immediately; could break account deletion mid-cascade) and a `BEFORE DELETE` trigger (fires on that cascade too); neither closes the race. **Migration `20260802120000` APPLIED FIRST ATTEMPT** — **13 applied of 13 tracked**, `db push --dry-run --linked` = "Remote database is up to date" — **because the exact file was executed against a real PostgreSQL 17.6 before the PR was opened**. That rehearsal **failed on its own postflight** and caught a real defect: it asserted `proconfig @> array['search_path=']`, but PostgreSQL stores `set search_path = ''` as **`search_path=""` with quotes**, so the assertion was false for a correctly hardened function; corrected and pinned by a test. **19/19 DB acceptance controls PASS** on disposable records inside a rolled-back transaction — unpaid charge deleted; charge with a payment refused **`23514`** with **both charge and payment surviving**; cross-account and nonexistent both **`P0002`** with byte-identical messages; `anon` false / `authenticated` true / `PUBLIC` false; FK still `confdeltype='c'`. **HTTP-path control:** an `anon` call returned **401 / `42501`**, proving both that the revoke holds end-to-end and that **PostgREST passes the raw SQLSTATE through verbatim as `code`**. **Owner Preview UI QA PASSED** on the isolated QA account: control shown for an unpaid charge, **withdrawn** once a payment exists, **restored** when the payment is deleted, delete works after confirm, a cancelled+unpaid charge is still deletable, a charge with a payment offers cancel/reopen but not delete, and the payment still counts after cancel/reopen. **One build**, Preview **`5a9e7277`**, then Production **`86d5cca9-88e2-40db-9869-664cfc1567e8`** by promoting the identical `dist/` ("Uploaded 0 files (12 already uploaded)"; local ↔ Preview ↔ Production **12/12 byte-identical**), entry **`index-o0xZrfkL.js`**. **Current rollback target is now `c45518fb` / `index-B21Es_EZ.js`**; `478e4d62` demoted to a historical fallback, retained. **A frontend rollback does NOT remove the function** — the migration is additive and stays applied, and no pre-release code calls it. **Gap 2 UNCHANGED and reclassified as no-change-needed:** payments on a cancelled charge still count in "הכנסה בפועל" — correct accounting, pinned by a test, and now stated on screen. ⚠️ **Two things NOT done, recorded rather than implied:** the **stale-tab refusal message** was never exercised (owner-accepted cosmetic gap — the refusal is proven server-side, only the toast text is unverified) — *(**SINCE CLOSED**: exercised and PASSED 2026-07-31, see the entry directly above; this clause is scoped to 2026-07-30)* — and **no authenticated Production smoke** was run, unlike the previous three releases *(still open)*. **One approved allowlist deviation:** `scheduleMigration.test.js` asserted its own migration is the last file in the directory forever, so any new migration failed an unrelated slice's test; changed to assert what it meant. Full suite **128 files / 3,738 passed / 0 failed**. **NO table/column/constraint/policy/data change, NO rebuild, NO second Preview, NO Edge/Auth/secret/package change, NO new slice:** Edge `ai-gateway` v36 (ACTIVE at that release) / `verify_jwt=true`, untouched.

...
- **Three DISTINCT anchors — never collapse them.** This tracker records no fixed value for the repository head, because documentation merges advance it continuously and any pinned value would be false within minutes:
  ...
  2. **Historical application-code merge anchor:** `29cccddda52e1c546b4ae46be052285ec24d2116` — the **PR #114 merge commit** (a historical anchor, not a head). Its code is **now RELEASED** (see anchor 3); it is kept only as the point at which the Studio containment code entered `main`.
  3. **Deployed Production source:** `03c23c2` — the commit the running Production artifact was actually built from (Studio local-engine containment). The prior `7e30199` (P1) is retained as the rollback deployment's source.
...
- Current Production deploy: **`247ef9ec-ad3a-4c15-8b16-25afa1c47f2b`** (Environment Production, branch `main`, source `03c23c2`, bundle **`index-BZ3B-0yd.js`**) — **LIVE (Studio / Local-Engine UI Containment)**. Deployed by reusing the exact Preview-accepted `dist/` — NOT rebuilt (wrangler "Uploaded 0 files (12 already uploaded)"). **Served-bytes proof: 12/12 files fetched from the canonical URL are byte-identical (SHA256) to the accepted artifact**; entry `index-BZ3B-0yd.js` = `422daf3aedb4f0512ede6dd1fc192ca45414c5d08f0b7ac1e7fc19d497d6f34c` (810,499 bytes), `index.html` = `8a737b72fd7771b3af291470c1b51b8ff6fdcb7688db607f98a917b9e7e9f46e`; every asset HTTP 200. **Recorded nuance:** the first sweep briefly returned the SPA `index.html` fallback for the new entry asset on one edge node (propagation race); a clean refetch and a full re-sweep were 12/12 with 0 mismatches. Previous Production **`476830a2-f8ea-45dc-b0ce-a71876bc48dd`** (source `7e30199`, bundle `index-BrR14XIC.js`, P1) is the retained **rollback target** — verified post-deploy HTTP 200, still serving its own bundle (819,230 bytes) with `index.html` SHA256 `205080b2…` matching this document's recorded value. Deployed by reusing the exact Preview-accepted `dist/` — NOT rebuilt (wrangler "Uploaded 0 files (12 already uploaded)"). **Served-bytes proof: all 12/12 files fetched from the canonical URL are byte-identical (`cmp`) to the accepted artifact**; entry `index-BrR14XIC.js` = `ec4a865a12c4a90f81063e3df366610f26bc45c3c37e9ad1792e24df29c64745` (819,230 bytes), `index.html` = `205080b22aff81bf0a930753b18c41a94537a81faa9443cdf8c6e016574e3d24`; every asset HTTP 200; zero console errors on load.
...
- Preview (retained): **`ec239e3b-2748-4d87-9122-11e11de2a3a1`** (branch `studio-containment-preview-03c23c2`, source `03c23c2`, bundle `index-BZ3B-0yd.js`) — authenticated QA acceptance PASSED; **this is the artifact promoted to Production unchanged**. The P1 Preview `c999988e` (source `7e30199`) is retained historically. The S0F.1 Preview `0760f00e` is retained historically.
...
- **Rollback:** `476830a2` / `index-BrR14XIC.js` (source `7e30199`) retained, verified healthy post-deploy.
...
5. ✅ Production `247ef9ec` by reusing the accepted artifact, rollback `476830a2` retained.
...
- **STILL NOT PROVEN: the authenticated Production smoke — attempted 2026-07-26 alongside the Studio containment release, and deliberately NOT closed.** With a live Account A session on Production (released bundle `index-BZ3B-0yd.js`), `/quotes` and `/finance` were inspected read-only. **What this DOES establish:** both pages render and hydrate on the released bundle with **no error banner, 0 console errors and 0 mutating requests**, and authenticated Production data access is healthy in general (the Clients page showed Account A's **3 real clients**). **What it does NOT establish, and why P1 stays open:** Account A currently holds **0 quotes and 0 transactions**, so *existing quote-data visibility could not be observed at all*, and `save_quote_atomic` was **not exercised** — proving it would require creating or editing a quote, which was explicitly out of scope. An empty-state render is **not** evidence that atomic quote persistence works in Production. P1's atomic behaviour therefore still rests on Preview acceptance + failure-injection (13/13) with the byte-identical artifact. **To close this honestly, a future authorised session must exercise one real quote save on Production (or inspect an account that already holds quote data).**
...
non-mutating Account A smoke PASS**. Rollback **`476830a2` / `index-BrR14XIC.js`** (source `7e30199`) retained and
verified healthy. No migration; Edge `ai-gateway` **v35** unchanged.

**Carried-forward open items (neither blocks anything):**
1. **P1 authenticated Production smoke — still NOT proven.** Account A holds 0 quotes and 0 transactions, so existing
   quote-data visibility could not be observed and `save_quote_atomic` was not exercised. Closing it honestly needs one
   authorised real quote save on Production, or an account that already holds quote data.

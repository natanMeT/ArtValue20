...
> ### ✅ VIDEO TAB TRUTHFULNESS — **CLOSED / LIVE IN PRODUCTION** (merge + owner Preview QA + promotion of the same artifact)
>
...
- **Active application release code anchor:** **`934722f4fecb70d814acf4662b4bf98787ba97fb`** (PR #166 merge — Video Tab Truthfulness; the running Production artifact was built from this commit). *(⚠️ This anchor read `97b4229c` (PR #127) for many releases; corrected 2026-07-31 together with the Production line, the rollback line, the migration count and the deployed-source anchor — five stale present-tense claims of one class, found by the Layer A checker only after the first was fixed and stopped masking the rest.)* *(Historical anchors, retained only as the sources of older deployments: `87fed4b` — Asset Library slice 1, now the rollback target's source; `2c8b1df` — complete local-engine retirement, now the **current rollback target's** source; `03c23c2` — Studio containment; `7e30199` — P1; `983f4899` — S0F.1.)*
- **Three DISTINCT anchors — never collapse them.** This tracker records no fixed value for the repository head, because documentation merges advance it continuously and any pinned value would be false within minutes:
  ...
  3. **Deployed Production source:** **`934722f4`** — the commit the running Production artifact was actually built from (Video Tab Truthfulness, deployment `6f0a20df`). The prior `272bc5ec` (Asset Library slice 2 — Favorites, deployment `19eeb73d`) is retained as the **current rollback deployment's** source. *(⚠️ This anchor read `87fed4b` — Asset Library slice 1 — for many releases; corrected 2026-07-31. Same stale-block class as the Production line and the migration count.)*
...
- **Rollback hierarchy — exactly ONE deployment may be called the current target. Do not collapse these levels.**
  1. **CURRENT frontend rollback target: `19eeb73d-1d86-4c59-b2b7-caf3837e2b16`** (source `272bc5ec`, bundle `index-BvcRPLhg.js`, Asset Library slice 2 — Favorites) — the immediately previous Production deployment, promoted to current target by the Video Tab Truthfulness release and **verified HTTP 200 post-deploy on 2026-08-01, still serving its own bundle**. **This is the deployment to roll back to.** A rollback undoes the video-tab wording **completely** — that slice added no database object — but it restores the misleading `וידאו (0)` tab. *(`c2bb560e` / `index-CZQwY28c.js` (Campaign Delete Safety) was the current target until this release and is now a **historical fallback only** — retained, NOT deleted.)* ⚠️ **Rolling back to `c2bb560e` or earlier does NOT remove the Favorites database objects** — `20260803120000` is additive and stays applied: the `is_favorite` column, the `assets_update_favorite_own` policy and the `grant update (is_favorite)` all survive a frontend rollback, and a pre-release bundle simply never writes the column. **Slice 2 is not rollback-able in isolation** — undoing it would need its own migration dropping the grant, the policy and the column, in that order, which was deliberately not written. *(`bc0aa2a2` / `index-BL6kQf-e.js` (Campaigns slice 3) was the current target until this release and is now a **historical fallback only** — retained, NOT deleted. Older still: `c45518fb` / `index-B21Es_EZ.js` (Client Profile slice 1) and `478e4d62` / `index-PcQFaAu-.js` (UI Polish + Dashboard action card), likewise retained as progressively further-behind fallbacks. ⚠️ Rolling back that far also does NOT remove `public.delete_charge_if_unpaid` — `20260802120000` is additive too.)*
...
- [ ] **Slice after this one — PENDING NATHAN DECISION.** Do NOT begin/design/invent the next slice until Nathan selects one and approves a spec. Candidate open items: **account-aware Growth data model** (the last of the three Growth reopening prerequisites); **Asset Library beyond gallery images** (approve/reject, usage rights, versions, campaign association, logo/brand assets — **favourites is DONE and LIVE, do not re-select it**); **video generation** (a GENERATION-lane slice, not a storage one — animated WebP is already an allowed MIME; what blocks it is that the cloud lane cannot produce video at all and no managed provider has been chosen. **UNSTARTED — nothing about this has begun.** ⚠️ Do NOT read the 2026-08-01 Video Tab Truthfulness release as progress here: it only stopped the UI *claiming* the capability. The tab now reads `וידאו · בקרוב`, so the visible symptom is gone while the capability gap is entirely unchanged); **the authenticated Production smoke for Asset Library slice 2** (**still never run** — needs an owner sign-in; the unauthenticated smoke passed and Preview QA covered the behaviour on byte-identical bytes, so this is a coverage gap, not a suspected defect); **basic accessibility cleanup** (NEW, non-blocking, surfaced by the owner's DevTools Issues panel during the 2026-08-01 Production QA: *"a form field element should have an id or name attribute"* ×1 and *"no label associated with a form field"* ×4. **Pre-existing and unrelated to Favorites or the video tab** — recorded so they do not evaporate. ⚠️ Also seen in that panel and **investigated and dismissed**: *"CSP blocks the use of 'eval'"* — **not ours**: the site sends **no CSP header**, has no CSP meta tag, and `eval(`/`new Function(` appear **0 times** in both `src/` and the served bundle, so it originates in a browser extension); **Products / Projects / Inventory / Templates / Activity durability**; **organization boundaries**; **credits / cost controls**; **Website Scanner** (per the section above); Jake conversation-refresh UX. **NOT candidates — already done, and selecting either would mean re-implementing merged work:**

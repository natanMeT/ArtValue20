...
> ### ✅ JAKE QUOTE DETAIL — **CLOSED / LIVE IN PRODUCTION** (consultation → owner-approved spec → one merge → Preview + owner QA on the isolated account → promotion of the same artifact)
>
...
## Baseline (current — JAKE QUOTE DETAIL CLOSED / LIVE in Production, deployment `47497fdb`)
...
- **Three DISTINCT anchors — never collapse them.** This tracker records no fixed value for the repository head, because documentation merges advance it continuously and any pinned value would be false within minutes:
  ...
  3. **Deployed Production source:** **`5b63f6d10e50daebdaa8481b3d52cdb9b8c04ae8`** — the commit the running Production artifact was actually built from (Jake Quote Detail, deployment `47497fdb`). The prior `a48020f23d0947a92df0b6da729bdc52924e00cb` (Jake Pre-Settle Truthfulness, deployment `024c0aa7`) is now the **current rollback target's** source, and `3681d77ef51162f53782ff32119538e84130536c` (Jake Asset Library Context, deployment `0a2669bb`) is now a **historical fallback's** source. ⚠️ **CORRECTED 2026-08-06 AFTER STANDING FIVE RELEASES STALE.** This line read `45243b53…` / deployment `378115bc` (Asset Library slice 4) from 2026-08-05 through the Jake Campaigns Context, Jake Asset Library Context, Jake Pre-Settle Truthfulness and Jake Quote Detail releases. **Layer A does NOT catch it:** the checker's anchor regexes are `Current Production deploy`, `CURRENT frontend rollback target` and `Active application release code anchor` — this bullet matches none of them, so it passed every automated gate while contradicting the `Current Production deploy` line three lines below it. **It must be re-read BY HAND every release.**
...
- **Rollback hierarchy — exactly ONE deployment may be called the current target. Do not collapse these levels.**
  1. **CURRENT frontend rollback target: `024c0aa7`** (source `a48020f23d0947a92df0b6da729bdc52924e00cb`, bundle `index-FzGUTjjq.js`, 797,585 B, SHA256 `0BDFFF59A16F9D850708C021DCAC0D8699F151B69C1E323BF29BF0CA6F5258E0`, Jake Pre-Settle Truthfulness; git tag `pre-jake-quote-detail` @ `68aeba0b7e1e45be4612b02faf29118fccb02f4e`, created **before** the deploy AND pushed — confirmed on `origin` as annotated object `0a4f37ffb331dbd913899c2f5ff9c89e1ba911ae`, dereferencing to `68aeba0b` via `refs/tags/pre-jake-quote-detail^{}`). **HTTP 200 confirmed AFTER the Jake Quote Detail deploy, still serving its own bundle `index-FzGUTjjq.js` at 797,585 B / SHA-256 `0bdfff59…5258e0`** — verified, not assumed. ⚠️ **Rolling back to it reverts ONLY the quote detail**: Jake returns to seeing the bare `pendingQuotes` count with no number, client, amount or date. The slice added no database object, so a frontend rollback is COMPLETE.
...
Production is `47497fdb` / `assets/index-cQ2OpZHP.js` / **799,495 B** (source `5b63f6d10e50daebdaa8481b3d52cdb9b8c04ae8`, Jake Quote Detail), migrations are **16 applied of 16 tracked, none pending**, Edge `ai-gateway` v36 is ACTIVE / `verify_jwt=true`, and the current rollback target is **`024c0aa7`** / `index-FzGUTjjq.js` (with `0a2669bb` / `index-B3o3MS7k.js` demoted to historical fallback). Rollback tag `pre-jake-quote-detail` @ `68aeba0b7e1e45be4612b02faf29118fccb02f4e`, created before the deploy and pushed.

**THREE ITEMS CARRIED FROM THIS RELEASE, none a blocker:**
...
2. **Two branches remain unexercised through a UI** — a quote with no client, and a ₪0 quote. The form blocks both. Covered by executed-artifact controls only, and **not** recorded as passes.
3. **`src/lib/calc.js:131` null-row fragility** — booked, unreachable via `fetchAll`, an owner decision.

...
> | Deploy | **Preview ONLY.** Preview **`10dbbf8d-d02d-4fcb-8255-6d83a5bff70b`** (Environment Preview, branch `f1-preview-110bb1e`, source **`110bb1e`**) — https://10dbbf8d.artvalue-product.pages.dev. Built once from merged `main`; entry bundle **`index-BLR2aev7.js`**. **NO Production deploy, NO Edge/Auth/secret/package change.** Production remains `90a7dc15` / `index-9FYipeQ9.js` (source `97b4229`), verified still serving that bundle after the Preview went up |
...
> | ⏳ Authenticated Preview UI acceptance | **NOT DONE — this is the open gate.** Everything above is either catalog-level, artifact-level or signed-out. **Nobody has yet seen the Finance receivables screen render, create a charge, record a payment, or show a derived balance in a browser.** That requires an owner-driven signed-in session on the Preview URL and is the next step |
>
...
> 5. ⏳ **Authenticated Preview UI acceptance — THE OPEN GATE, owner-driven.** Nobody has yet rendered the Finance
>    receivables screen signed in, created a charge, recorded a payment or seen a derived balance in a browser.
> 6. ⏳ Production deploy — **NOT taken**, and not on the table until gate 5 passes.
>
...
- **F1 Core Receivables slice 1 (charges, payments, five same-owner composite keys)** — **MIGRATION APPLIED / PART A + PART B VERIFIED LIVE / NOT RELEASED (2026-07-30).**
  ...
  - **✅ The database now has all of it.** Migration applied and verified live 2026-07-30 (11 of 11, dry-run clean); PART A matched exactly and all nine PART B controls were decisive on disposable QA records, with cleanup verified zero per-account and RLS-independently. **⚠️ Still NOT released:** the frontend is built and on **Preview `10dbbf8d`** only — Production is unchanged at `90a7dc15`, so no user can reach it. **The authenticated Preview UI acceptance has NOT been done**; the Preview smoke was unauthenticated. See the ✅ box at the top for the full evidence and for the two failed applies.

...
- **Rollback hierarchy — exactly ONE deployment may be called the current target. Do not collapse these levels.**
  1. **CURRENT frontend rollback target: `fe7baa5e`** (source `54dbcb0a0bd1af1671a78d43964820cf0595ce45`, bundle `index-LPwu_nUL.js`, 788,614 B, Stale-List Refetch Race; git tag `pre-transient-action-cards-promotion` @ `54dbcb0a0bd1af1671a78d43964820cf0595ce45`, pushed). **HTTP 200 confirmed AFTER the Transient Action Cards deploy, still serving its own bundle `index-LPwu_nUL.js`** — verified, not assumed. ⚠️ **Rolling back to it restores the persisted-stale-action-card defect: an undecided `gate` / `confirm` / `preview` card would again be written to `localStorage` and restore after a reload, offering a destructive action the user never requested that session.**
     - *(HISTORICAL FALLBACK ONLY — **DEMOTED 2026-08-04**, retained and NOT the current target (§22: exactly one current target): )* **`0fa5c3ea`** (source `6635f3d6f44b430b2d32c95b620d7ec02307761d`, bundle `index-04boVitS.js`, Bulk Delete False-Success; git tag `pre-stale-list-race-promotion` @ `6635f3d6f44b430b2d32c95b620d7ec02307761d`, retained). Still HTTP 200 at the time of the promotion, but **rolling back to it now silently reverts TWO releases** — it restores the stale-list refetch race **as well as** the persisted-stale-action-card defect.

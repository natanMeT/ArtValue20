...
- **Three DISTINCT anchors — never collapse them.** This tracker records no fixed value for the repository head, because documentation merges advance it continuously and any pinned value would be false within minutes:
  ...
  3. **Deployed Production source:** **`521b7a0b9ea0852cc38b7267542fc46cbfdbe577`** — the commit the running Production artifact was actually built from (Jake Campaigns Context, deployment `af65f5fe`). The prior `e3555eedc691ef2f37cf5ad68db10d6cb9f40c56` (Jake sees the calendar + the stale-read fix, deployment `123ea5aa`) is now the **current rollback target's** source, and `c95f46a8a873a6a604b11065a8bbff7a59a4867c` (Transient Action Cards, deployment `a5ef6d36`) the historical fallback's.
...
- [x] **Jake Campaigns Context (durable business campaigns → Jake's context, read-only): DONE, CLOSED / LIVE, OWNER-QA PASSED** *(Production `af65f5fe` / `index-CqSrIXWY.js`, source `521b7a0b`, PR [#202](https://github.com/natanMeT/ArtValue20/pull/202))*. **Do not re-select it.** Three things it deliberately did NOT do, each a separate slice if wanted: **Jake writing a campaign** (create / update / delete / status — no Jake op exists and none was added), the **`tasks.campaign_id` → Jake join** (so Jake knows campaign titles but not which tasks belong to them), and **assets**. ⚠️ **Still open and now the sharpest remaining Jake gap: `public.assets` is durable, live and STILL INVISIBLE to Jake** — the same class campaigns just closed, and like campaigns before this slice it is **not even declared absent to him**.
...
Production is `af65f5fe` / `index-CqSrIXWY.js` (source `521b7a0b`, Jake Campaigns Context),
migrations are **16 applied of 16 tracked, none pending**, Edge `ai-gateway` v36 is ACTIVE / `verify_jwt=true`, and
the current rollback target is `123ea5aa` / `index-CBCh_jpO.js` (with `a5ef6d36` / `index-D2hzBpSc.js` demoted to
historical fallback).

...
> ### ✅ ACCESSIBILITY SLICE A1 — THE NAMELESS CONTROLS NOW HAVE AN ACCESSIBLE NAME — **CLOSED / LIVE IN PRODUCTION** (diagnosis + merge + owner Preview QA + promotion of the same artifact)
>
...
## Baseline (current — ACCESSIBILITY SLICE A1 CLOSED / LIVE in Production, deployment `268e3f83`)
...
- **Active application release code anchor:** **`69193895247b559ba85eaa3f14c88c2e60eb3342`** (PR #180 merge — Accessibility slice A1; the running Production artifact was built from this commit. The slice itself entered `main` at `05ac884b`). *(Historical anchors, retained only as the sources of older deployments: `a9f96eeb` — Modal Stacking Fix, now the **current rollback target's** source; `d1dcc64b` — Schedule slice 2, now the historical fallback's source; `ba760451` — Monthly Plan; `75a8a167` — Asset Library slice 3, now the **current rollback target's** source; `934722f4` — Video Tab Truthfulness, now the historical fallback's source; `87fed4b` — Asset Library slice 1; `2c8b1df` — complete local-engine retirement; `03c23c2` — Studio containment; `7e30199` — P1; `983f4899` — S0F.1.)*
- **Three DISTINCT anchors — never collapse them.** This tracker records no fixed value for the repository head, because documentation merges advance it continuously and any pinned value would be false within minutes:
  ...
  3. **Deployed Production source:** **`69193895`** — the commit the running Production artifact was actually built from (Accessibility slice A1, deployment `268e3f83`). The prior `a9f96eeb` (Modal Stacking Fix, deployment `56d9cd56`) is now the **current rollback target's** source, and `d1dcc64b` (Schedule slice 2, deployment `4294aba9`) the historical fallback's.
...
- **Current Production deploy: `268e3f83`** (Environment Production, branch `main`, source **`69193895`**, bundle **`index-CM2Rtz83.js`**, **781,984 B**) — **LIVE (Accessibility slice A1 — the 10 nameless form controls now carry an `aria-label`).** The owner-QA'd Preview `c567eaa4` artifact was **promoted UNCHANGED**: `wrangler` reported **"Uploaded 0 files (12 already uploaded)"**, **12/12 byte-identical BEFORE the deploy and 12/12 on canonical AFTER it**, and the canonical URL was polled cache-busted to **30/30 across three rounds**. ⚠️ **This release added NO database object, no API, no Edge, no package and no secret, so a frontend rollback to `56d9cd56` is COMPLETE — and it also restores the 10 nameless controls.**
  - *(SUPERSEDED 2026-08-03, retained as history and NOT current. ⚠️ It IS the CURRENT ROLLBACK TARGET: )* **`56d9cd56-8517-4293-8a62-a576dee0b75e`** (Environment Production, branch `main`, source **`a9f96eeb`**, bundle **`index-CR-yRHJq.js`**, SHA256 `21e73104121cf251e903fce65d890caa5ba4d92052fc918829976e9b47bf029b`, 781,472 B) — **was LIVE (Modal Stacking Fix — the dialog is portalled to `document.body` and paints above the topbar and the sidebar; that behaviour is carried forward in the live bundle).**
  - *(SUPERSEDED 2026-08-02, retained as history and NOT current. ⚠️ It is the HISTORICAL FALLBACK: )* **`4294aba9-e244-4531-bcbd-1a75522f6515`** (Environment Production, branch `main`, source **`d1dcc64b`**, bundle **`index-vlblhC-w.js`**, SHA256 `a7c36219841cebacc8e74db22dbdf145c9ad8865b4109a0db8257773cd569cdc`, 781,441 B) — **was LIVE (Schedule slice 2 — the read-only month grid on `/schedule`). Promoted UNCHANGED from the owner-QA'd Preview `df4e95af`: "Uploaded 0 files (12 already uploaded)", 12/12 byte-identical before AND after**, and the canonical URL was polled cache-busted until **10/10** consecutive requests served the new bundle.
...
- **Rollback hierarchy — exactly ONE deployment may be called the current target. Do not collapse these levels.**
  1. **CURRENT frontend rollback target: `56d9cd56-8517-4293-8a62-a576dee0b75e`** (source `a9f96eeb`, bundle `index-CR-yRHJq.js`, Modal Stacking Fix; git tag `pre-a11y-a1-promotion` @ `a9f96eeb`, pushed). HTTP 200 confirmed after the A1 deploy, still serving its own bundle. ⚠️ **A frontend rollback of the live release is COMPLETE** — Accessibility slice A1 added no database object, no API, no Edge, no package and no secret. ⚠️ It does **NOT** undo migration `20260804120000` (Asset Library slice 3), which is additive and stays applied. ⚠️ **Rolling back also RESTORES the 10 nameless controls** — that is the trade, and it should be a deliberate choice, not a surprise.
  1. *(HISTORICAL FALLBACK — demoted 2026-08-03, retained, NOT the current target; §22 requires exactly one: )* **`4294aba9-e244-4531-bcbd-1a75522f6515`** (source `d1dcc64b`, bundle `index-vlblhC-w.js`, Schedule slice 2 — the read-only month grid; git tag `pre-modal-stacking-promotion` @ `d1dcc64`, pushed). Verified HTTP 200 after the A1 deploy.
  2. **HISTORICAL FALLBACK ONLY — NOT the current target: `90868363-fd44-4f4a-b6c7-d1197676e6cd`** (source `ba760451`, bundle `index-BAajglJn.js`, Monthly Plan `/plan`; git tag `pre-schedule-month-promotion` @ `ba76045`, pushed). Older fallbacks (`be068244`, `6f0a20df`, `19eeb73d`, `c2bb560e`, `88b20584`) are retained further down as history only.
  3. **Git rollback tag: `pre-a11y-a1-promotion` @ `a9f96eeb` — created AND pushed** (confirmed on `origin`: `refs/tags/pre-a11y-a1-promotion^{}` → `a9f96eeb3a6ebdbc88b72d836cd9be7c9518b5fc`). **No Edge rollback applies to this release — `ai-gateway` v36 was NOT redeployed.** **The live release added NO migration**, so there is nothing left applied behind a frontend revert. Earlier tags retained: `pre-modal-stacking-promotion` @ `d1dcc64`, `pre-schedule-month-promotion` @ `ba76045`, `pre-monthly-plan-promotion` @ `75a8a167`, `pre-campaign-link-promotion` @ `934722f4`, `pre-ui-polish-action-card` @ `e80a1341`.
...
- [ ] **A2 CANDIDATE, NOT SELECTED — the Login screen's own email and password inputs have no programmatic accessible name.** `src/pages/Login.jsx:55` and `:64` carry visible `<label>אימייל</label>` / `<label>סיסמה</label>` that are **not associated**; measured live on the served artifact, both inputs report `aria-label: null` and no `id`. It sits in the 109-strong `.field` bucket, so it was **out of A1's scope by construction**. **Strongest next candidate: every user passes through login**, and it is where password managers and assistive tech matter most. Two controls, one file. ⚠️ Fixing it via `id`+`htmlFor` would set the pattern for the other 108 — so decide the pattern deliberately, not incidentally.
...
Production is `268e3f83` / `index-CM2Rtz83.js` (source `69193895`, Accessibility slice A1), migrations are **16 applied
of 16 tracked, none pending**, Edge `ai-gateway` v36 is ACTIVE / `verify_jwt=true`, and the current rollback target is
`56d9cd56` / `index-CR-yRHJq.js`.

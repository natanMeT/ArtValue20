...
> **(1)** Migration counts are always written **"N applied of M tracked"**. A bare `9/9` cannot be read as either one
> and silently contradicts a pending migration. **(2)** No paragraph states that `main` **is** a given SHA. A merge
...
> `17bba0b3` accepted and 6/6 migrations at the time. **`b3708cc2` is today a HISTORICAL FALLBACK ONLY** (see the
...
- Supabase: project `weciwurjfwmqihcyexzj`; **all 9 migrations applied & matching, none pending.**
...
- [x] **Campaigns slice 1 (durable per-account business campaigns): DONE, CLOSED / LIVE** *(Production `90a7dc15`)*. **Slice 2 — `tasks.campaign_id` + composite-FK cross-account ownership enforcement — is now MERGED / NOT APPLIED / NOT RELEASED (PR [#131](https://github.com/natanMeT/ArtValue20/pull/131) merged as `8b6a78a`; migration `20260729120000` NOT applied, project at 9/10); see the box at the top.**

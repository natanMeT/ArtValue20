...
**Last updated:** 2026-08-05 — session: **JAKE PRE-SETTLE TRUTHFULNESS — THE ≤4s WINDOW NO LONGER CLAIMS A LIVE MODULE IS DISCONNECTED — RELEASED / LIVE IN PRODUCTION.** **Production `024c0aa7` / `index-FzGUTjjq.js`**, Environment Production, branch `main`, source **`a48020f23d0947a92df0b6da729bdc52924e00cb`**, 797,044 → **797,585 B** (+541), entry-bundle SHA256 `0BDFFF59A16F9D850708C021DCAC0D8699F151B69C1E323BF29BF0CA6F5258E0`. **ONE PR:** [#210](https://github.com/natanMeT/ArtValue20/pull/210) (merge `a48020f`, head-gated `8854cc7c8ed869181b2e8360183b0ba26c1643e0`, **9 files / +429 −32**). **THE GAP IT CLOSED:** `artValueContext()` discriminates on `Array.isArray` alone, so between a cloud panel open and the campaigns/assets seam reads settling both collections are `undefined` with their error flags `false` and fell through to `notConnectedLine()` — Jake was told *"קמפיינים/נכסים: המודול אינו מחובר לחשבון הזה"* about two modules that ARE connected, durable and owned by the account. The window is reachable: `buildContext(jakeData())` runs on every chat send, draft and force-actions call, all available the instant the panel opens. **ROOT CAUSE, STATED PRECISELY:** the `{ rows, error }` contract encodes **two** absences (failed cloud read, no module at all) while the product has **THREE**; the third was collapsed into the second, and the second is the one that makes a false claim. **THE CALENDAR IS EXCLUDED ON EVIDENCE, NOT BY OMISSION** — `calendarLines()` deliberately does NOT use `notConnectedLine()`; its wording is *"אין לי גישה ליומן כרגע"*, which asserts no disconnection and no zero and stays true during the window. This slice therefore formally NARROWS this file's own earlier "one slice across all three seams" estimate to **two seams**. **THE LOAD-BEARING DECISION IS THE DEFAULT DIRECTION:** the new signal is an explicit `pending` flag that **defaults to FALSY**, not a `settled` flag defaulting to false. An absent key means "not pending", so every existing caller, fixture and test keeps today's behaviour byte-for-byte and an unknown consumer fails toward the pre-existing wording. Inverted, local/demo — which never sets the key and genuinely has neither module — would announce that its campaigns and assets are loading, FOREVER. Mutation M1 proves the guard bites (**14 failures**). **PRECEDENCE, INSIDE THE EXISTING UNHYDRATED BLOCK:** hydrated rows > error > pending > not-connected. A read known to have failed is never softened into "still loading", and a verified list is never hidden behind one. **The behavioural diff is 12 lines**; everything else is comments and tests. **D1:** `campaignReadState.js` / `assetReadState.js` gain a third key `settled` **for WORDING ONLY** — a documented reversal of their own "TWO keys, not three" headers — and the morning briefing stays gated on the CALENDAR alone, pinned by tests (mutation M6 fails if anyone wires the new flags in). **D2:** `pending` is NOT re-armed on a later panel open; after the first settle the seams keep today's last-known-rows behaviour. **D3:** the Hebrew strings were verified **byte-exact** by diffing the shipped builder's output against the owner-approved text. **BUDGET MEASURED, NOT ASSUMED** (200 campaigns at max title, 40 assets, S0D brain, against the imported `MAX_CONTEXT_CHARS = 12000`): hydrated **5,085 (42.4%)** remains the binding constraint, PRE-SETTLE **3,947 (32.9%)**, not-connected 3,990, failed 4,025 — the new state is **43 chars CHEAPER than the wording it replaces** and can never become the worst case. Committed as CI assertions expressed as RELATIONS, not hardcoded numbers. **FRONTEND ONLY — `src/lib/api.js` UNTOUCHED** (blob `1555be7331bdd95d6243eefa00cf10f08854b7c3` on both sides), and no migration, bucket, Storage policy, grant, Edge, package or secret. Migrations remain **16 applied of 16 tracked, none pending**; Edge `ai-gateway` **v36 ACTIVE / `verify_jwt=true`**, untouched — so **a frontend rollback is COMPLETE**. **Current rollback target is now `0a2669bb` / `index-B3o3MS7k.js`**, HTTP 200 verified after this deploy still serving its own bundle; `378115bc` / `index-BiwDxmw5.js` is demoted to historical fallback, retained. **Git rollback tag `pre-jake-pre-settle` @ `27c6a5a0248ec9fb17b13b29bca8bf03bea07b20` created AND pushed** (confirmed on `origin`: `refs/tags/pre-jake-pre-settle` → annotated object `5ab77ffd5e50a62bd5f7761a7a5386b2ef123e33`; target verified as the merge commit's FIRST PARENT). **OWNER QA PASSED ON THE ISOLATED QA ACCOUNT, 10/10 CASES, PLUS A NON-EMPTY ASSET PATCH** — see the box below. Full suite **155 files / 4,407 passed / 0 failed** (was 4,375, **+32**). Previous session: **JAKE ASSET LIBRARY CONTEXT — THE DURABLE CLOUD ASSET LIBRARY REACHES JAKE AS READ-ONLY, METADATA-ONLY DATA — RELEASED / LIVE IN PRODUCTION.** **Production `0a2669bb` / `index-B3o3MS7k.js`**, Environment Production, branch `main`, source **`3681d77ef51162f53782ff32119538e84130536c`**, 794,709 → **797,044 B** (+2,335), entry-bundle SHA256 `8875BB8E8CD9BF17ED7DD62FC1D475001BC624374BBA1F0DE1F563D7F7B8E91B`. **ONE PR:** [#208](https://github.com/natanMeT/ArtValue20/pull/208) (merge `3681d77`, head-gated `ae14f2fa5981716805f00dcd2e84fcf8c55a99a4`, **7 files / +763 −11**). **THE GAP IT CLOSED:** `public.assets` is durable, live and cloud-only, and four shipped slices (gallery images, favourites, campaign association, user upload) built the whole chain — while **nothing in `jakePack.js` mentioned assets at all**, neither data nor a declaration of absence, so an ungrounded answer was the only thing available for `מה יש לי בגלריה`. This file had already named it *"the sharpest remaining Jake gap"*. **THE READ PATH IS THE JAKE SEAM, NOT `fetchAll`:** `Assistant.jsx` calls the EXISTING, UNCHANGED `listAssets()` once per panel open, with its own `alive` guard and its own 4,000 ms timer — **a THIRD independent lane, not a fan-out**. **FRONTEND ONLY — `src/lib/api.js` UNTOUCHED** (byte-identical SHA `1555be7331bdd95d6243eefa00cf10f08854b7c3` on both sides), and no migration, bucket, Storage policy, grant, Edge, package or secret. **READ-ONLY: no Jake asset op exists and none was added** — no upload, delete, favourite or campaign link; `jakeAgent.js` is not in the diff at all. **THE LOAD-BEARING GUARD IS NOT THE CAP — IT IS THE EXCLUSION OF `meta.prompt`.** Measured against `MAX_CONTEXT_CHARS = 12000`, which the Gateway **REJECTS** on rather than truncating: **5** assets carrying maximum-length (2,000-char) prompts is **15,310 chars and FAILS**, while all **40** assets WITHOUT the prompt is **7,615 and passes**; uncapped with prompts is **87,975 (7.3×)**. The prompt is therefore **excluded, not truncated** — a half-sentence prompt is something Jake would quote back as a fact about the user's image. The cap of 5 is a second, cheap defence and keeps the answer readable. Worst case on the shipped code including the S0D business brain: **5,673 / 12,000**; measured **live in Production QA at 3,512 / 12,000** with 40 assets and 200 campaigns. **FIELDS EXPOSED:** total · uploaded/generated split · favourites count · campaign-linked count · and for at most 5 assets: created date, MIME, size in KB, favourite flag, campaign title (truncated at 60). **EXCLUDED:** `meta.prompt`, `meta.preset`, `meta.engine`, the signed `url`, `storagePath`, the asset uuid and `user_id` — `listAssets()` attaches a signed URL to every row and this lane never reads it, because a signed URL is a short-lived bearer credential and putting one in a model prompt hands out the object. **THE CAMPAIGN LABEL HAS THREE OUTCOMES, NOT TWO:** an asset whose link cannot be named — deleted campaign, or a failed campaigns read — is still LINKED and reads *"משויך לקמפיין (השם אינו זמין כרגע)"*; folding that into "no campaign" would contradict the database and inventing a title would be worse. The linked COUNT stays correct in all three states. **Current rollback target is now `378115bc` / `index-BiwDxmw5.js`**, HTTP 200 verified after this deploy still serving its own bundle; `af65f5fe` / `index-CqSrIXWY.js` is demoted to historical fallback, retained. **Git rollback tag `pre-jake-asset-context` @ `f5ab300ee9b833e755e2f17ba7095e18abd5c074` created AND pushed** (confirmed on `origin`: `refs/tags/pre-jake-asset-context` → annotated object `59c9c75e32c7299ac5c61c64d8413e1cb2734d89`). **OWNER QA PASSED ON THE ISOLATED QA ACCOUNT WITH REAL DATA, AND CLEANUP WAS VERIFIED IN THE DATABASE** — see the box below. Full suite **155 files / 4,375 passed / 0 failed** (the previously published `122 files / 3,522` baseline was stale and was re-measured before these counts were reported). Migrations remain **16 applied of 16 tracked, none pending**; Edge `ai-gateway` **v36 ACTIVE / `verify_jwt=true`**, untouched — so **a frontend rollback is COMPLETE**. Previous session: **ASSET LIBRARY SLICE 4 — A USER'S OWN IMAGE REACHES THE DURABLE GALLERY — RELEASED / LIVE IN PRODUCTION.** **Production `378115bc` / `index-BiwDxmw5.js`**, Environment Production, branch `main`, source **`45243b53e7d5d0b1c09d6cccc2e616ce15ee8534`**, 792,585 → **794,709 B** (+2,124), entry-bundle SHA256 `12948DE215DF6E6B0A5AB46392696A330D728324DCC28A640C01D9ECD89D0D83`. **ONE PR:** [#204](https://github.com/natanMeT/ArtValue20/pull/204) (merge `45243b53`, head-gated `dd8efedb`, **6 files / +716 −4**). **THE GAP IT CLOSED:** `createAsset()` had always accepted any `Blob`, but the only blob-producing call site in the product was `srcToBlob(r.src)` inside the generation success branch, and `ImageStudio.jsx` carried **no file input at all** — so an image could enter the durable cloud library **only by being generated by the product**. The pipe existed, proven by three shipped slices, with no inlet. **FRONTEND ONLY — `src/lib/api.js` UNTOUCHED**, and no migration, bucket, Storage policy, grant, Edge, package or secret: every server rule an upload needs was already live (bucket `allowed_mime_types` + `file_size_limit`, the `assets_mime_allowed` / `assets_byte_size_bounded` / `assets_storage_path_matches_owner` CHECKs, owner-only RLS, and BOTH 40 quotas), and `source` is a free text column bounded at 40, so `'upload'` needed no schema change. **Current rollback target is now `af65f5fe` / `index-CqSrIXWY.js`**; `123ea5aa` / `index-CBCh_jpO.js` is demoted to historical fallback, retained. Rollback tag `pre-asset-user-upload` @ `df9cfae1ea0546fc035237ceb845ff660cb413b2`, pushed. Full suite **153 files / 4,335 passed / 0 failed**. Migrations remain **16 applied of 16 tracked, none pending**; Edge `ai-gateway` **v36 ACTIVE / `verify_jwt=true`**, untouched. Previous session: **JAKE CAMPAIGNS CONTEXT — THE DURABLE BUSINESS CAMPAIGNS REACH JAKE AS READ-ONLY CONTEXT, AND A FAILED READ LEAVES NO STALE ROWS — RELEASED / LIVE IN PRODUCTION.** **Production `af65f5fe` / `index-CqSrIXWY.js`**, Environment Production, branch `main`, source **`521b7a0b9ea0852cc38b7267542fc46cbfdbe577`**, 790,830 → **792,585 B** (+1,755), entry-bundle SHA256 `2535BEFC45935432841974B7C73E5FC5FE89CB5176AAF4AC593A850EE4CE2824`. **ONE PR:** [#202](https://github.com/natanMeT/ArtValue20/pull/202) (merge `521b7a0b`, head-gated `b53ff1db`, **7 files / +653 −7**). **THE GAP #202 CLOSED, AND IT WAS WORSE THAN THE CALENDAR'S:** `public.campaigns` is durable, live and cloud-only, and was **invisible to Jake** — `api.fetchAll()` hydrates nine collections and campaigns is not one of them, the Campaigns page owns its own state and re-reads after every write, and the word did not appear in `jakePack.js` at all. Projects and inventory at least get `notConnectedLine()`, so Jake is TOLD he cannot see them; campaigns had **neither data nor a declaration of absence**, so an ungrounded answer was the only thing available for `מה קורה עם הקמפיינים שלי`. **THE READ PATH IS THE JAKE SEAM, NOT `fetchAll`:** `Assistant.jsx` calls the EXISTING, UNCHANGED `listCampaigns()` **once per panel open**, guarded by `isSupabaseConfigured`, with its own `alive` unmount/account-switch guard and its own 4,000 ms timer. Deliberately NOT added to the store — that whole-object replacement has already produced **two shipped defects**, and campaigns have no reducer, no seed and no `localStorage` fallback (a test already forbids all three storage APIs on that page). **A SECOND INDEPENDENT LANE, NOT A FAN-OUT:** separate `alive` flag, separate timer, separate state pair, so neither seam read can observe, delay or corrupt the other — which is why two seam reads are safe where the merged three-way read the calendar slice refused would not have been. **THE TIMER IS FOR TRUTHFULNESS, NOT TO UNBLOCK ANYTHING:** nothing waits on this read, but `supabase-js` sets no timeout of its own and a hang would leave `campaigns` undefined with the error flag false, rendering as *"המודול אינו מחובר לחשבון הזה"* — **false in cloud mode**. **READ-ONLY:** no new Jake op (`JAKE_DURABLE_OPS` untouched), no create/update/delete/status, no `answerFromData` branch, no new Gateway call or `actionType` — the campaign text rides inside the **existing** `context.summary`. **CAMPAIGNS DO NOT GATE THE MORNING BRIEFING** (owner decision D1): the calendar does, because it is the only time-bound item; `campaignReadState.js` therefore returns **two keys, not three** — no `settled` flag — and the briefing code was not touched at all. **THE CAP OF 5 IS A BUDGET GUARD WHOSE WORST CASE IS REACHABLE BY DESIGN:** the account quota is **200 campaigns** and the Gateway **REJECTS** an over-limit context rather than truncating it (`MAX_CONTEXT_CHARS = 12000`, enforced on `String.length`, not bytes). Measured against a heavy account: baseline **4,945** chars, capped worst case (200 campaigns, all active, 120-char titles) **5,905 / 12,000 — 49.2%**, campaign cost bounded at **+799 chars** regardless of account size. **Uncapped the same account is 25,253 chars (2.1×), and with `objective` bodies 65,853 (5.5×)** — Jake would not degrade, he would **stop**. That is why `objective` bodies are excluded and only actives are listed. **FIELDS EXPOSED:** title · Hebrew status label · start/end dates. Excluded: `objective` bodies, the uuid, `user_id`, the raw status enum, `created_at`/`updated_at`. **NO migration, NO Edge, NO schema, NO package, NO secret** — migrations remain **16 applied of 16 tracked, NONE PENDING**; `ai-gateway` v36 ACTIVE / `verify_jwt=true`, untouched. **A frontend rollback is therefore COMPLETE.** **Current rollback target is now `123ea5aa` / `index-CBCh_jpO.js`**, HTTP 200 verified after this deploy still serving its own bundle; `a5ef6d36` / `index-D2hzBpSc.js` **demoted to historical fallback**. **Git rollback tag `pre-jake-campaigns` @ `885e8093bf431cdb8d260514962689542ada08bf` created AND pushed** (confirmed on `origin`: `refs/tags/pre-jake-campaigns` → annotated object `96db388c43293737ce4172e4287ce7f886d68e35`). Full suite **151 files / 4,283 passed / 0 failed**. Previous session: **JAKE SEES THE CALENDAR — RELEASED / LIVE IN PRODUCTION**, Production `123ea5aa` / `index-CBCh_jpO.js`, source `e3555eed`, PRs [#199](https://github.com/natanMeT/ArtValue20/pull/199) + [#200](https://github.com/natanMeT/ArtValue20/pull/200) — see the historical box below.
...
> ### ✅ JAKE PRE-SETTLE TRUTHFULNESS — **CLOSED / LIVE IN PRODUCTION** (consultation → owner-approved spec → one merge → Preview + owner QA on the isolated account → a non-empty asset QA patch → promotion of the same artifact)
>
...
## Baseline (current — JAKE PRE-SETTLE TRUTHFULNESS CLOSED / LIVE in Production, deployment `024c0aa7`)
...
- **Active application release code anchor:** **`3681d77ef51162f53782ff32119538e84130536c`** (PR #208 merge — Jake Asset Library Context, read-only; the running Production artifact was built from this commit and carries every earlier slice's behaviour forward). ⚠️ **The 2026-08-05 correction that first fixed this line still stands: it had been left at `8882461731c90f06fca0fc7fb44b5f65e8e20873` (PR #191, Jake Context Truthfulness) through several later releases — the same stale-anchor class this file keeps catching.** *(Historical anchors, retained only as the sources of older deployments: `45243b53` — Asset Library slice 4, user file upload, now the **current rollback target's** source; `521b7a0b` — Jake Campaigns Context, now a historical fallback's source; `e3555eed` — Jake sees the calendar; `8882461731c90f06fca0fc7fb44b5f65e8e20873` — PR #191; `c138f852` — Accessibility slice A5.)*
- **Three DISTINCT anchors — never collapse them.** This tracker records no fixed value for the repository head, because documentation merges advance it continuously and any pinned value would be false within minutes:
  ...
  3. **Deployed Production source:** **`45243b53e7d5d0b1c09d6cccc2e616ce15ee8534`** — the commit the running Production artifact was actually built from (Asset Library slice 4 — user file upload, deployment `378115bc`). The prior `521b7a0b9ea0852cc38b7267542fc46cbfdbe577` (Jake Campaigns Context, deployment `af65f5fe`) is now the **current rollback target's** source, and `e3555eedc691ef2f37cf5ad68db10d6cb9f40c56` (Jake sees the calendar, deployment `123ea5aa`) is now a **historical fallback's** source.
...
- **Current Production deploy: `024c0aa7`** (Environment Production, branch `main`, source **`a48020f23d0947a92df0b6da729bdc52924e00cb`**, bundle **`index-FzGUTjjq.js`**, SHA256 `0BDFFF59A16F9D850708C021DCAC0D8699F151B69C1E323BF29BF0CA6F5258E0`, **797,585 B**) — **LIVE (Jake Pre-Settle Truthfulness — during the ≤4s window between a cloud panel open and the campaigns/assets seam reads settling, Jake is no longer told those modules are "not connected to this account"; every earlier slice's behaviour is carried forward in this bundle).** Promoted **UNCHANGED** from the owner-QA'd Preview `14be2bfb` — `wrangler` reported **"Uploaded 0 files (12 already uploaded)"**, and canonical Production ↔ local `dist/` are **12/12 byte-identical by SHA-256**. Unauthenticated non-mutating smoke **16/16 HTTP 200, every one a `GET`, 0 writes**. **FRONTEND ONLY — `src/lib/api.js` UNTOUCHED** (blob `1555be7331bdd95d6243eefa00cf10f08854b7c3` on both sides), no migration, bucket, Storage policy, grant, Edge, package or secret; migrations remain **16 applied of 16 tracked, none pending**, `ai-gateway` **v36 ACTIVE / `verify_jwt=true`**, untouched — so **a frontend rollback is COMPLETE**. Delta **+541 B**, measured against a control build from clean `main` that reproduced the outgoing artifact `index-B3o3MS7k.js` at 797,044 B byte-for-byte.
...
- **Rollback hierarchy — exactly ONE deployment may be called the current target. Do not collapse these levels.**
  1. **CURRENT frontend rollback target: `0a2669bb`** (source `3681d77ef51162f53782ff32119538e84130536c`, bundle `index-B3o3MS7k.js`, 797,044 B, SHA256 `8875BB8E8CD9BF17ED7DD62FC1D475001BC624374BBA1F0DE1F563D7F7B8E91B`, Jake Asset Library Context; git tag `pre-jake-pre-settle` @ `27c6a5a0248ec9fb17b13b29bca8bf03bea07b20`, created AND pushed — confirmed on `origin` as annotated object `5ab77ffd5e50a62bd5f7761a7a5386b2ef123e33`). **HTTP 200 confirmed AFTER the Jake Pre-Settle Truthfulness deploy, still serving its own bundle `index-B3o3MS7k.js`** — verified, not assumed. ⚠️ **Rolling back to it reverts ONLY the pre-settle wording**: the slice added no database object, so a frontend rollback is COMPLETE.
...
## Next action
**Nothing is mid-flight. As of 2026-08-05 there is no undeployed slice and no release gate awaiting an owner.**

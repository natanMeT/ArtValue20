...
## Baseline (current — JAKE SEES THE CALENDAR + ITS STALE-READ FIX CLOSED / LIVE in Production, deployment `123ea5aa`)
...
- **Three DISTINCT anchors — never collapse them.** This tracker records no fixed value for the repository head, because documentation merges advance it continuously and any pinned value would be false within minutes:
  ...
  3. **Deployed Production source:** **`e3555eedc691ef2f37cf5ad68db10d6cb9f40c56`** — the commit the running Production artifact was actually built from (Jake sees the calendar + the stale-read fix, deployment `123ea5aa`). The prior `c95f46a8a873a6a604b11065a8bbff7a59a4867c` (Transient Action Cards, deployment `a5ef6d36`) is now the **current rollback target's** source, and `54dbcb0a0bd1af1671a78d43964820cf0595ce45` (Stale-List Refetch Race, deployment `fe7baa5e`) the historical fallback's.
...
- **Current Production deploy: `123ea5aa`** (Environment Production, branch `main`, source **`e3555eedc691ef2f37cf5ad68db10d6cb9f40c56`**, bundle **`index-CBCh_jpO.js`**, SHA256 `95D769548B757F80836AE68C18A2B120E5F5E4622F07358838137871430418DC`, **790,830 B**) — **LIVE (Jake sees the calendar — the durable appointments reach Jake's context and daily briefing, read-only, and a failed read no longer leaves stale rows; every earlier slice's behaviour is carried forward in this bundle).** Two PRs, one release: [#199](https://github.com/natanMeT/ArtValue20/pull/199) (capability) and [#200](https://github.com/natanMeT/ArtValue20/pull/200) (the stale-read fix its own owner QA found). The owner-QA'd Preview `033987aa` artifact was **promoted UNCHANGED**: `wrangler` reported **"Uploaded 0 files (12 already uploaded)"** — zero bytes re-uploaded, which is the mechanical proof the promoted artifact is the QA'd one and not a rebuild — with **12/12 byte-identical local `dist/` ↔ Preview BEFORE the deploy and 12/12 canonical ↔ `dist/` AFTER it**. Propagation was confirmed on the **first** cache-busted poll; the canonical parity sweep still ran afterwards. Non-mutating Production smoke **29/29 requests, all GET, 0 writes**. `index-Ddanmefb.css` is unchanged, so **this release provably ships no CSS**; the entry bundle moved **+2,186 B**. ⚠️ **This release added NO database object, no API, no Edge, no package and no secret, so a frontend rollback to `a5ef6d36` is COMPLETE — and it removes the calendar from Jake entirely AND restores the stale-appointments defect.**
  - *(SUPERSEDED 2026-08-05, retained as history and NOT current. ⚠️ It IS the CURRENT ROLLBACK TARGET: )* **`a5ef6d36`** (Environment Production, branch `main`, source **`c95f46a8a873a6a604b11065a8bbff7a59a4867c`**, bundle **`index-D2hzBpSc.js`**, SHA256 `E14E7671F5937F6B038AB77DCE00AE37228585D309FD86AE4CABCF51D8A8212F`, **788,644 B**) — **was LIVE (Transient Action Cards — an executable action card no longer persists or hydrates; every earlier slice's behaviour is carried forward in this bundle).** The owner-QA'd Preview `47597e7d` artifact was **promoted UNCHANGED**: `wrangler` reported **"Uploaded 0 files (12 already uploaded)"**, **12/12 byte-identical local `dist/` ↔ Preview BEFORE the deploy and 12/12 canonical ↔ `dist/` AFTER it**. Propagation was confirmed on the **first** cache-busted poll — the edge-cache trap did not fire this time, and the canonical parity sweep still ran afterwards. `index-Ddanmefb.css` is unchanged, so **this release provably ships no CSS**; the entry bundle moved by **+30 B**, one predicate clause (the header comment compiles away). ⚠️ **The `index.html` parity read requires the ROOT URL** — `/index.html` returns **308** and a naive sweep reports 11/12; it is byte-identical at `/` (1,285 B, SHA256 `5794D6D6…`), and that 1,285 B is also the SPA-fallback size, so the asset URL was checked separately (788,644 B, not the fallback). ⚠️ **This release added NO database object, no API, no Edge, no package and no secret, so a frontend rollback to `fe7baa5e` is COMPLETE — and it restores the persisted-stale-action-card defect.**
  - *(SUPERSEDED 2026-08-04, retained as history and NOT current. ⚠️ It IS the CURRENT ROLLBACK TARGET: )* **`fe7baa5e`** (Environment Production, branch `main`, source **`54dbcb0a0bd1af1671a78d43964820cf0595ce45`**, bundle **`index-LPwu_nUL.js`**, SHA256 `56C61486E5654E333E472498BFB9C529145D5FD1BE0AD0EE8C2A4E36F98DA17D`, **788,614 B**) — **was LIVE (Stale-List Refetch Race — a failed write no longer restores rows a sibling delete removed; every earlier slice's behaviour is carried forward in this bundle).** The owner-QA'd Preview `942a62d7` artifact was **promoted UNCHANGED**: `wrangler` reported **"Uploaded 0 files (12 already uploaded)"**, **12/12 byte-identical local `dist/` ↔ Preview BEFORE the deploy and 12/12 canonical ↔ `dist/` AFTER it**; propagation polled cache-busted **24/24 across three rounds**. `index-Ddanmefb.css` is unchanged, so **this release provably ships no CSS**. ⚠️ **The first canonical parity run reported 10/12 and that was edge cache, NOT a failed promotion** — the new asset URL returned the 1,285 B SPA fallback until it propagated; canonical flipped on the 4th cache-busted poll (~18 s). ⚠️ **This release added NO database object, no API, no Edge, no package and no secret, so a frontend rollback to `0fa5c3ea` is COMPLETE — and it restores the stale-list race.**
...
- [x] **Jake sees the calendar (durable appointments → Jake's context + daily briefing, read-only): DONE, CLOSED / LIVE, OWNER-QA PASSED** *(Production `123ea5aa` / `index-CBCh_jpO.js`, source `e3555eed`, PRs [#199](https://github.com/natanMeT/ArtValue20/pull/199) + [#200](https://github.com/natanMeT/ArtValue20/pull/200))*. **Do not re-select it.** Two things it deliberately did NOT do, each a separate slice if wanted: **Jake writing an appointment** (create / reschedule / cancel — no Jake op exists and none was added), and **campaigns or assets reaching Jake's context**, which stayed out of scope so a single panel-open read could not become a three-way fan-out with three partial-failure states. ⚠️ **Still open and now the sharpest remaining Jake gap: `public.campaigns` and `public.assets` are durable, live and STILL INVISIBLE to Jake** — the same class the calendar just closed, and neither is even declared absent to him.
...
Production is `123ea5aa` / `index-CBCh_jpO.js` (source `e3555eed`, Jake sees the calendar + its stale-read fix),
migrations are **16 applied of 16 tracked, none pending**, Edge `ai-gateway` v36 is ACTIVE / `verify_jwt=true`, and
the current rollback target is `a5ef6d36` / `index-D2hzBpSc.js` (with `fe7baa5e` / `index-LPwu_nUL.js` demoted to
historical fallback).

...
>
> ⚠️ **STATE — the CODE is merged; the SCHEMA CHANGE IS NOT LIVE, and there is no release.** Migration
> `20260731120000_finance_receivables_slice1.sql` is **NOT APPLIED**: **10 applied of 11 tracked, one pending**
> (verified by `supabase migration list --linked` after the merge — the remote column for `20260731120000` is empty).
> `main` therefore deliberately carries merged database work that does **not** exist in the live database: anyone
> reading `supabase/migrations/**` off `main` sees two tables and five composite keys that are **not there yet**.
> This is a known owner gate, not drift — the same window Campaigns slice 2 held between 2026-07-29 and 2026-07-30.
> **No build, no deployment, no Edge/Auth/secret change**, and none is implied by the merge; the Production release,
...
> | Migration | `20260731120000_finance_receivables_slice1.sql` — **NOT APPLIED (owner gate).** 10 applied of 11 tracked, one pending |
> | Rollback tag | `pre-finance-receivables-slice1` @ `110baf1` |
...
> | Deploy | **none** — no build, no Preview, no Production, no Edge/Auth/secret change |
>
...
>
> **The five composite same-owner keys** (each preserving its EXISTING delete action byte for byte — changing one
> while widening a key would be two decisions hiding in one): `quotes (client_id, user_id) → clients` CASCADE;
> `transactions (client_id, user_id) → clients` SET NULL (client_id); `charges (client_id, user_id) → clients`
> SET NULL (client_id); `charges (quote_id, user_id) → quotes` SET NULL (quote_id); `payments (charge_id, user_id) →
> charges` CASCADE. **The column list on SET NULL is load-bearing:** a bare `SET NULL` also nulls `user_id`
...
>
> **Owner gates still ahead, in order — none of them taken:**
> 1. ⏳ **Apply migration `20260731120000`** — needs its own explicit owner approval. Not taken.
> 2. ⏳ **PART A** — read-only catalog verification from the migration's VERIFICATION section.
> 3. ⏳ **PART B** — the mutating acceptance controls, on **two QA accounts** and **disposable QA records only**,
>    with **cleanup verified zero by query** rather than assumed (controls 13-14 delete rows). The real owner account
>    is never touched.
> 4. ⏳ Build / deploy — a later, separately-approved step; the Finance screen is user-visible, so unlike Campaigns
>    slice 2 this slice **will** eventually need one.

> ### ✅ CAMPAIGNS SLICE 2 — **MERGED + MIGRATION APPLIED / VERIFIED LIVE / NOT RELEASED** (merged 2026-07-29, applied 2026-07-30)
>
...
>
> **STATE — the schema change is LIVE; the product release is NOT.** The migration is **applied and verified live**
> (10 applied of 10 tracked, none pending *at that moment*; today 10 of 11, F1 pending), so `public.tasks.campaign_id` and the composite FK now exist in the
> database. There is still **no build, no deployment, no Edge/Auth/secret change** — and none is needed, because the
> frontend is untouched. The Production release, bundle, rollback target and Edge version in the boxes below are
> **UNCHANGED** by this slice and remain authoritative.
>
...
> included) and **must always be resolved live** (`git rev-parse origin/main`). The live schema is **no longer behind**
> the repo: anyone reading `supabase/migrations/**` off `main` now sees a column and a foreign key that **do exist in
> the database**. *(Historical: between the 2026-07-29 merge and the 2026-07-30 apply, `main` deliberately carried
> merged, unapplied database work — a known owner gate, not drift. That window is over.)*
>
...
## Baseline (current — CAMPAIGNS SLICE 1 CLOSED / LIVE in Production)

> ### ✅ CAMPAIGNS SLICE 1 — **CLOSED / LIVE IN PRODUCTION** (2026-07-28)
>
...
> | Migration | **`20260728120000_campaigns_slice1.sql` APPLIED 2026-07-28 — 8 applied of 8 tracked, matching *at that release*; the project is at **10 applied of 11 tracked** — `20260731120000` (F1 Core Receivables) is MERGED but **PENDING**, awaiting a separate owner approval to apply.** No Auth or secret change |
...
> | **Edge** | `ai-gateway` **v36 ACTIVE, `verify_jwt=true`** — **UNCHANGED, not redeployed** by this slice |
> | Migration | **`20260727120000_asset_library_slice1.sql` APPLIED 2026-07-27 — 7 applied of 7 tracked, matching *at that time*; the project is at **10 applied of 11 tracked** — `20260731120000` (F1 Core Receivables) is MERGED but **PENDING**, awaiting a separate owner approval to apply.** No Auth or secret change |
...
- **Current Production deploy: `90a7dc15-1ed1-4d2a-ad6e-9044c786334c`** (Environment Production, branch `main`, source **`97b4229`**, bundle **`index-9FYipeQ9.js`**) — **LIVE (Campaigns slice 1 — durable per-account business campaigns)**. **(HISTORICAL — the previous deploy `5bcf1ef0` / `index-dUN1r8PM.js`, source `87fed4b`, Asset Library slice 1, is now the current rollback target.)** Deployed by reusing the exact Preview-accepted `dist/` — **NOT rebuilt** (wrangler "Uploaded 0 files (12 already uploaded)"). **Served-bytes proof: 12/12 files fetched from the canonical URL are byte-identical (SHA256) to the accepted artifact**; entry `index-dUN1r8PM.js` = `03e8f062be62b678bbb442639fa2a75033a123e1a6277a7842b8b54c10ac225c` (697,879 bytes). Authenticated QA-account **Preview acceptance PASS** and a non-mutating **Account A Production smoke PASS** (one `GET /rest/v1/assets` → 200, 2 GETs across five routes, 0 writes, 0 console messages).
- **Rollback hierarchy — exactly ONE deployment may be called the current target. Do not collapse these levels.**
  1. **CURRENT frontend rollback target: `5bcf1ef0-fa3e-4ff6-b55f-7a22f68db3aa`** (source `87fed4b`, bundle `index-dUN1r8PM.js`, Asset Library slice 1) — the immediately previous Production deployment, retained and **verified HTTP 200 post-deploy, still serving its own bundle**. **This is the deployment to roll back to.**
  2. **HISTORICAL FALLBACK ONLY — NOT the current target: `b3708cc2-ab2e-44ee-a557-8cc2ae688635`** (source `2c8b1df`, bundle `index-C4frcMDi.js`, complete local-engine retirement) — **demoted from current target by this release; retained, NOT deleted**, verified HTTP 200. Rolling back here would revert Asset Library slice 1 as well. Use only if target 1 is itself unusable. (Older `247ef9ec` Studio containment / `476830a2` P1 / `e63198b7` S0F.1 / `4b86993d` S0E / `69f8a175` S0D / `cec116b9` S0C / `31cb521d` S0B / `4cb17aee` S0A are retained historically — likewise fallbacks only, progressively further behind.)
  3. **Git rollback tag: `pre-campaigns-slice-1` @ `a548518`.** **No Edge rollback applies to this release — `ai-gateway` v36 was NOT redeployed.** **A frontend rollback does NOT undo migration `20260728120000`** — it is additive and stays applied; the previous bundle simply never queries `public.campaigns`. (The previous release's tag `pre-asset-library-slice-1` @ `3803b61` is retained.)
- Preview (retained): **`19a58ba9-9b85-4f87-a19a-5f218cb0e8b5`** (branch `campaigns-slice-1-preview`, source `97b4229`, bundle `index-9FYipeQ9.js`) — two-QA-account acceptance PASSED; **this is the artifact promoted to Production unchanged**. Previously: **`d1235743-564d-43a9-bf26-bb7a25d3850d`** (branch `assets-preview-87fed4b`, source `87fed4b`, bundle `index-dUN1r8PM.js`) — full authenticated QA-account acceptance PASSED; **this is the artifact promoted to Production unchanged**. The retirement Preview `17bba0b3`, the Studio-containment Preview `ec239e3b`, the P1 Preview `c999988e` and the S0F.1 Preview `0760f00e` are retained historically.
...
- Supabase: project `weciwurjfwmqihcyexzj`; **10 applied of 11 tracked — ONE PENDING.** ⚠️ **This is NOT a fully-applied baseline, so a preflight may NOT conclude that the live schema matches `supabase/migrations/**`.** `20260731120000_finance_receivables_slice1.sql` (F1 Core Receivables slice 1) is **merged into `main` and NOT APPLIED**, awaiting its own owner approval — so `public.charges`, `public.payments` and the five composite same-owner keys exist in the repo and **not** in the database. The other 10 match, including `20260729120000_campaigns_slice2_task_link.sql` (Campaigns slice 2), **APPLIED and verified live 2026-07-30**. *(Historical: between 2026-07-29 and 2026-07-30 this line recorded the same kind of window for Campaigns slice 2; that one is closed.)*
...
## Status ledger
- **F1 Core Receivables slice 1 (charges, payments, five same-owner composite keys)** — **CODE MERGED / MIGRATION PENDING / NOT RELEASED (2026-07-30).**
  - **Chain so far:** PR [#134](https://github.com/natanMeT/ArtValue20/pull/134) → merge `56d13ef` (head-gated to `c811fac`) → ⏳ **migration `20260731120000` NOT APPLIED — 10 applied of 11 tracked** → ⏳ PART A → ⏳ PART B → ⏳ build/deploy. Nothing after the merge has been taken.
...
- At that point `supabase db push --dry-run --linked` proposed exactly the one then-pending atomic migration and nothing else. *(It has since been applied. Campaigns slice 2 then made the dry-run propose `20260729120000` for a day; that migration was applied 2026-07-30, so the dry-run reports "Remote database is up to date" again — see the Baseline.)*
...
## Next action
**F1 Core Receivables slice 1 is MERGED with its migration NOT APPLIED — see the ⏸ box at the top of this file.**
Its PR ([#134](https://github.com/natanMeT/ArtValue20/pull/134)) merged as `56d13ef` (parents `110baf1` + the
owner-approved head `c811fac`, `--match-head-commit`), and migration `20260731120000` is **PENDING** —
**10 applied of 11 tracked** — so `main` carries merged database work that is not yet in the live schema. Codex closed
at 21 rounds / 41 findings, all fixed and resolved, 0 unresolved threads; full suite **122 files / 3,511 passed /
0 failed** re-run on merged `main`. It produced **no build and no deployment**.

**The next step is a SEPARATE owner approval, and it is not implied by the merge:**
1. ⏳ **Approve and apply `20260731120000`.** Until then the repo and the database disagree by design, and no preflight
   may claim they match.
2. ⏳ Then **PART A** (read-only catalog) and **PART B** (mutating acceptance) from the migration's VERIFICATION
   section — **two QA accounts, disposable QA records only, cleanup verified zero by query**, the real owner account
   never touched. PART B controls 13-14 delete rows, so the order in that section is load-bearing.
3. ⏳ Only after that is a build/deploy decision on the table — the Finance screen is user-visible, so this slice will
   eventually need one, unlike Campaigns slice 2.
...
> rule it illustrates still holds: **merged is not live.** *(It holds again today, for F1's migration — see the ⏸ box
> at the top: merged is not applied either.)*

...
> | Migration | **NONE — this slice touches no database object.** Migrations remain **12 applied of 12 tracked, none pending**; `db push --dry-run --linked` = **"Remote database is up to date"**, verified at this session's preflight |
...
> | Migration | `20260801120000_schedule_core_slice1.sql` — **APPLIED and VERIFIED LIVE 2026-07-30, FIRST attempt.** **12 applied of 12 tracked, none pending**; `db push --dry-run --linked` = **"Remote database is up to date"** |
...
> 2. ✅ **Apply migration `20260801120000`** — 2026-07-30, **first attempt**. **12 applied of 12 tracked.**
...
- **Current Production deploy: `88b20584-b375-4073-a762-f91dc2f1a1e8`** (Environment Production, branch `main`, commit-hash **`c281cda`**, artifact **built from `110bb1e`**, bundle **`index-BLR2aev7.js`**) — **LIVE (F1 Core Receivables slice 1 — charges vs payments, derived payment status)**, deployed 2026-07-30 on explicit owner approval, promoted unchanged from Preview `10dbbf8d` (10/10 byte-identical; "Uploaded 0 files (12 already uploaded)"), authenticated non-mutating smoke PASSED. *(Superseded: `90a7dc15` / `index-9FYipeQ9.js` (source `97b4229`, Campaigns slice 1) was the Production deploy until this release and is now the **current rollback target**.)* — *the line below is the superseded description, retained as history:* **`90a7dc15-1ed1-4d2a-ad6e-9044c786334c`** (branch `main`, source **`97b4229`**, bundle **`index-9FYipeQ9.js`**) — **was LIVE (Campaigns slice 1)**. **UNCHANGED by F1, re-verified 2026-07-30 after the F1 Preview went up.** ⚠️ **An F1 Preview exists and is NOT Production:** Preview `10dbbf8d-d02d-4fcb-8255-6d83a5bff70b` (branch `f1-preview-110bb1e`, source `110bb1e`, bundle `index-BLR2aev7.js`) is awaiting authenticated UI acceptance; it has never been promoted. **(HISTORICAL — the previous deploy `5bcf1ef0` / `index-dUN1r8PM.js`, source `87fed4b`, Asset Library slice 1, is now the current rollback target.)** Deployed by reusing the exact Preview-accepted `dist/` — **NOT rebuilt** (wrangler "Uploaded 0 files (12 already uploaded)"). **Served-bytes proof: 12/12 files fetched from the canonical URL are byte-identical (SHA256) to the accepted artifact**; entry `index-dUN1r8PM.js` = `03e8f062be62b678bbb442639fa2a75033a123e1a6277a7842b8b54c10ac225c` (697,879 bytes). Authenticated QA-account **Preview acceptance PASS** and a non-mutating **Account A Production smoke PASS** (one `GET /rest/v1/assets` → 200, 2 GETs across five routes, 0 writes, 0 console messages).
- **Rollback hierarchy — exactly ONE deployment may be called the current target. Do not collapse these levels.**
  1. **CURRENT frontend rollback target: `90a7dc15-1ed1-4d2a-ad6e-9044c786334c`** (source `97b4229`, bundle `index-9FYipeQ9.js`, Campaigns slice 1) — the immediately previous Production deployment, promoted to current target by the F1 release and **verified HTTP 200 post-deploy on 2026-07-30, still serving its own bundle**. **This is the deployment to roll back to.** *(`5bcf1ef0-fa3e-4ff6-b55f-7a22f68db3aa` (source `87fed4b`, bundle `index-dUN1r8PM.js`, Asset Library slice 1) was the current target until this release and is now a **historical fallback only** — retained, NOT deleted, re-verified HTTP 200 still serving its own bundle.)*

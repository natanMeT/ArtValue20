...
- **Repository main HEAD:** resolve **live** at each task's preflight (e.g. `git rev-parse origin/main`). Repository main may differ from the active application release code anchor **without** indicating deployment drift. ⚠️ **As of the PR #114 merge this gap is no longer documentation-only:** `main` = `29cccddda52e1c546b4ae46be052285ec24d2116` now contains **merged application code that has never been built for release or deployed** (the Studio local-engine containment slice). The running Production artifact is still built from `7e30199`. Do not treat "merged into main" as "live".
...
- **Merge:** commit `29cccddda52e1c546b4ae46be052285ec24d2116`, parents `4f4180b` (prior main) + `c30dff2` (the approved head). The merge was executed pinned to that exact head SHA.
- **Review:** Codex round 2 reported no major issues (reviewed commit `bb8e955ef2`); **0 unresolved review threads**. The `src/` tree on `main` is byte-identical to the reviewed tree (`255bd4c7edb0aa2fbd181a1b22cb528fa6de9aee`), so exactly the reviewed code landed — the two commits after the reviewed one were documentation-only.
...
## Next action
**Studio / Local-Engine UI Containment is MERGED but NOT RELEASED.** PR #114 merged into `main`
(`29cccddda52e1c546b4ae46be052285ec24d2116`). **No Preview or Production deployment has occurred**, so nothing that is
live has changed: Production remains **`476830a2-f8ea-45dc-b0ce-a71876bc48dd` / `index-BrR14XIC.js`** (source

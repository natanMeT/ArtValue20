...
**Last updated:** 2026-08-02 — session: **ASSET LIBRARY SLICE 3 — CAMPAIGN LINK — RELEASED / LIVE IN PRODUCTION (see the box directly below).** **Production `be068244-87c0-4202-b86b-f9a0ae41a671` / `index-C0J4dBZV.js`**, Environment Production, branch `main`, source **`75a8a167`** — the **owner-QA'd Preview artifact promoted unchanged** (`wrangler` reported **"Uploaded 0 files (12 already uploaded)"**; proven **12/12 byte-identical to the QA'd Preview `d3ba756c` BEFORE the deploy and 12/12 identical on canonical AFTER it**, entry bundle SHA256 `872584b530200d9efd7f782e222950d23f2f9f89c80e918e9ea786e2ba4f117a`, 770,698 B). A gallery asset can now be linked to one of the account's campaigns from the card. **Migration `20260804120000_asset_library_slice3_campaign_link.sql` APPLIED 2026-08-02 — 16 applied of 16 tracked, NONE PENDING.** The link is a **composite** FK `(campaign_id, user_id) -> campaigns (id, user_id)` with `on delete set null (campaign_id)`, and the writable surface is the column GRANT `(is_favorite, campaign_id)`, not the policy. Merges `f05d30a6` (PR [#169](https://github.com/natanMeT/ArtValue20/pull/169), head-gated `e192fb7`) and `75a8a167` (PR [#170](https://github.com/natanMeT/ArtValue20/pull/170), head-gated `eb77abc`). **⚠️ The FIRST owner QA FAILED:** the campaign selector existed in the DOM but was **clipped to zero visible pixels** by the gallery tile's `overflow: hidden` — the build, all 3,886 tests and all nine live DB controls passed, because none of them can see a pixel. Fixed structurally in PR #170. **Current rollback target is now `6f0a20df` / `index-hjKIewHB.js`**, verified HTTP 200 after the deploy still serving its own bundle; `19eeb73d` / `index-BvcRPLhg.js` retained as a historical fallback. **Git rollback tag `pre-campaign-link-promotion` @ `934722f4` created and pushed.** ⚠️ A frontend rollback does **NOT** undo the migration — it is additive and stays applied. Full suite **139 files / 3,892 passed / 0 failed**. Previous session: **VIDEO TAB TRUTHFULNESS — RELEASED / LIVE IN PRODUCTION**, Production `6f0a20df` / `index-hjKIewHB.js`, source `934722f4`, PR [#166](https://github.com/natanMeT/ArtValue20/pull/166) — see the historical box below.
...
> ### ✅ ASSET LIBRARY SLICE 3 — CAMPAIGN LINK — **CLOSED / LIVE IN PRODUCTION** (merge + migration applied & verified live + a FAILED first QA + fix + owner Preview QA + promotion of the same artifact)
>
...
- **Three DISTINCT anchors — never collapse them.** This tracker records no fixed value for the repository head, because documentation merges advance it continuously and any pinned value would be false within minutes:
  ...
  3. **Deployed Production source:** **`75a8a167`** — the commit the running Production artifact was actually built from (Asset Library slice 3 — Campaign Link, deployment `be068244`). The prior `934722f4` (Video Tab Truthfulness, deployment `6f0a20df`) is now the rollback target, not current. The prior `272bc5ec` (Asset Library slice 2 — Favorites, deployment `19eeb73d`) is retained as the **current rollback deployment's** source. *(⚠️ This anchor read `87fed4b` — Asset Library slice 1 — for many releases; corrected 2026-07-31. Same stale-block class as the Production line and the migration count.)*
...
- **Current Production deploy: `be068244-87c0-4202-b86b-f9a0ae41a671`** (Environment Production, branch `main`, source **`75a8a167`**, bundle **`index-C0J4dBZV.js`**, SHA256 `872584b530200d9efd7f782e222950d23f2f9f89c80e918e9ea786e2ba4f117a`, 770,698 B) — **LIVE (Asset Library slice 3 — Campaign Link)**, promoted unchanged with no rebuild, canonical HTTP 200 verified 10/10.
...
- **Test evidence for the CURRENT release (Asset Library slice 3 — Campaign Link, source `75a8a167`, deployment `be068244`, bundle `index-C0J4dBZV.js`):** full suite **139 files / 3,892 passed / 0 failed**, re-run on the merged `main` (baseline before the slice: 136 / 3,839). **Migration `20260804120000` APPLIED — 16 applied of 16 tracked, none pending**, `db push --dry-run --linked` reports "Remote database is up to date." **NO Storage change, NO Edge/Auth/secret/package change** — Edge `ai-gateway` v36 stays ACTIVE / `verify_jwt=true`. Negative controls were run against the real migration text (single-column FK, bare `SET NULL`, dropped `is_favorite` re-grant — each fails the suite), and the live DB controls were 9/9 with impersonation proven inside every transaction.
  - *(HISTORICAL — the line below is the superseded test evidence of the previous release, retained as history and NOT current: )* **Test evidence for the previous release (Video Tab Truthfulness, source `934722f4`, deployment `6f0a20df`, bundle `index-hjKIewHB.js`):** full suite **136 files / 3,839 passed / 0 failed**, re-run on the merged `main`; **NO migration, NO DB/schema change, NO Storage change, NO Edge/Auth/secret/package change** — migrations remain **15 applied of 15 tracked, none pending**, and Edge `ai-gateway` v36 stays ACTIVE / `verify_jwt=true`. **Exactly one build**, promoted **unchanged** ("Uploaded 0 files (12 already uploaded)"), canonical **12/12 byte-identical by SHA-256** to the QA'd Preview artifact.

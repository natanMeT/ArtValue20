...
> ### ✅ ACCESSIBILITY SLICE A2 — THE LOGIN LABELS ARE NOW ASSOCIATED WITH THEIR INPUTS — **CLOSED / LIVE IN PRODUCTION** (merge + owner Preview QA + promotion of the same artifact)
>
...
## Baseline (current — ACCESSIBILITY SLICE A2 CLOSED / LIVE in Production, deployment `7be49ef5`)
...
- **Active application release code anchor:** **`3e7d7d07b32ab6952094a61bbbe2d7665e1a8474`** (PR #182 merge — Accessibility slice A2, the Login labels; the running Production artifact was built from this commit). *(Historical anchors, retained only as the sources of older deployments: `69193895` — Accessibility slice A1, now the **current rollback target's** source; `a9f96eeb` — Modal Stacking Fix, now the **historical fallback's** source; `d1dcc64b` — Schedule slice 2, now two steps behind the fallback; `ba760451` — Monthly Plan; `75a8a167` — Asset Library slice 3, now the **current rollback target's** source; `934722f4` — Video Tab Truthfulness, now the historical fallback's source; `87fed4b` — Asset Library slice 1; `2c8b1df` — complete local-engine retirement; `03c23c2` — Studio containment; `7e30199` — P1; `983f4899` — S0F.1.)*
- **Three DISTINCT anchors — never collapse them.** This tracker records no fixed value for the repository head, because documentation merges advance it continuously and any pinned value would be false within minutes:
  ...
  3. **Deployed Production source:** **`3e7d7d07`** — the commit the running Production artifact was actually built from (Accessibility slice A2 — the Login labels, deployment `7be49ef5`). The prior `69193895` (Accessibility slice A1, deployment `268e3f83`) is now the **current rollback target's** source, and `a9f96eeb` (Modal Stacking Fix, deployment `56d9cd56`) the historical fallback's.
...
- **Current Production deploy: `7be49ef5-6d5f-4034-a007-27fca0022b0e`** (Environment Production, branch `main`, source **`3e7d7d07`**, bundle **`index-DvnPUY6C.js`**, SHA256 `7cc8da5a578b92660bbdd27c208badfc5238861e628e268497a0f6edbc7708a0`, **782,068 B**) — **LIVE (Accessibility slice A2 — the two Login labels are now associated with their inputs via `id`/`htmlFor`).** The owner-QA'd Preview `4a6c4569` artifact was **promoted UNCHANGED**: `wrangler` reported **"Uploaded 0 files (12 already uploaded)"**, **12/12 byte-identical BEFORE the deploy and 12/12 on canonical AFTER it**, and the canonical URL was polled cache-busted to **30/30 across three rounds**. **Every sibling chunk hash is unchanged from A1 — only the entry hash and `index.html` moved.** ⚠️ **This release added NO database object, no API, no Edge, no package and no secret, so a frontend rollback to `268e3f83` is COMPLETE — and it also restores the two unassociated Login labels.**
  - *(SUPERSEDED 2026-08-03, retained as history and NOT current. ⚠️ It IS the CURRENT ROLLBACK TARGET: )* **`268e3f83-318d-40d0-ab6c-9d7a0864c87f`** (Environment Production, branch `main`, source **`69193895`**, bundle **`index-CM2Rtz83.js`**, **781,984 B**) — **was LIVE (Accessibility slice A1 — the 10 nameless form controls carry an `aria-label`; that behaviour is carried forward in the live bundle).** Verified HTTP 200 after the A2 deploy, still serving its own bundle.
  - *(SUPERSEDED 2026-08-03, retained as history and NOT current. ⚠️ It is now the HISTORICAL FALLBACK: )* **`56d9cd56-8517-4293-8a62-a576dee0b75e`** (Environment Production, branch `main`, source **`a9f96eeb`**, bundle **`index-CR-yRHJq.js`**, SHA256 `21e73104121cf251e903fce65d890caa5ba4d92052fc918829976e9b47bf029b`, 781,472 B) — **was LIVE (Modal Stacking Fix — the dialog is portalled to `document.body` and paints above the topbar and the sidebar; that behaviour is carried forward in the live bundle).**
...
- **Rollback hierarchy — exactly ONE deployment may be called the current target. Do not collapse these levels.**
  1. **CURRENT frontend rollback target: `268e3f83-318d-40d0-ab6c-9d7a0864c87f`** (source `69193895`, bundle `index-CM2Rtz83.js`, Accessibility slice A1; git tag `pre-a11y-a2-promotion` @ `69193895`, pushed). HTTP 200 confirmed after the A2 deploy, still serving its own bundle. ⚠️ **A frontend rollback of the live release is COMPLETE** — Accessibility slice A2 added no database object, no API, no Edge, no package and no secret. ⚠️ It does **NOT** undo migration `20260804120000` (Asset Library slice 3), which is additive and stays applied. ⚠️ **Rolling back also RESTORES the two unassociated Login labels** — that is the trade, and it should be a deliberate choice, not a surprise. It does **not** restore A1's 10 nameless controls, which are already in this target.
  1. *(HISTORICAL FALLBACK — demoted 2026-08-03 by A2, retained, NOT the current target; §22 requires exactly one: )* **`56d9cd56-8517-4293-8a62-a576dee0b75e`** (source `a9f96eeb`, bundle `index-CR-yRHJq.js`, Modal Stacking Fix; git tag `pre-a11y-a1-promotion` @ `a9f96eeb`, pushed). Verified HTTP 200 after the A2 deploy, still serving its own bundle.
  2. **HISTORICAL FALLBACK ONLY — NOT the current target: `4294aba9-e244-4531-bcbd-1a75522f6515`** (source `d1dcc64b`, bundle `index-vlblhC-w.js`, Schedule slice 2 — the read-only month grid; git tag `pre-modal-stacking-promotion` @ `d1dcc64`, pushed). Older fallbacks (`90868363`, `be068244`, `6f0a20df`, `19eeb73d`, `c2bb560e`, `88b20584`) are retained further down as history only.
  3. **Git rollback tag: `pre-a11y-a2-promotion` @ `69193895` — created AND pushed** (confirmed on `origin`: `refs/tags/pre-a11y-a2-promotion` → `69193895247b559ba85eaa3f14c88c2e60eb3342`). **No Edge rollback applies to this release — `ai-gateway` v36 was NOT redeployed.** **The live release added NO migration**, so there is nothing left applied behind a frontend revert. Earlier tags retained: `pre-a11y-a1-promotion` @ `a9f96eeb`, `pre-modal-stacking-promotion` @ `d1dcc64`, `pre-schedule-month-promotion` @ `ba76045`, `pre-monthly-plan-promotion` @ `75a8a167`, `pre-campaign-link-promotion` @ `934722f4`, `pre-ui-polish-action-card` @ `e80a1341`.
...
- [x] **Accessibility slice A2 — the Login labels: DONE, CLOSED / LIVE** *(Production `7be49ef5` / `index-DvnPUY6C.js`, source `3e7d7d07`, PR [#182](https://github.com/natanMeT/ArtValue20/pull/182))*. `id`+`htmlFor` on the two Login controls — **the first two of the 109 `.field` gaps ever closed; the bucket is now 107 and the total 128.** **Do not re-select it.** ⚠️ **The pattern question flagged when this was a candidate ("fixing it via `id`+`htmlFor` would set the pattern for the other 108") WAS decided, deliberately — and the decision is a RULE, not just a precedent: a literal `id` is safe only on a screen PROVEN to render at most once (Login's duplicate-`id` count was measured as 1 and 1 in the live DOM); anything that renders per row MUST derive its `id` from stable domain data, because N rows sharing one `id` bind every label to the first control — a mis-associated label is worse than an unassociated one.** Full rule in the A2 box at the top of this file.
...
Production is `7be49ef5` / `index-DvnPUY6C.js` (source `3e7d7d07`, Accessibility slice A2 — the Login labels),
migrations are **16 applied of 16 tracked, none pending**, Edge `ai-gateway` v36 is ACTIVE / `verify_jwt=true`, and the
current rollback target is `268e3f83` / `index-CM2Rtz83.js` (with `56d9cd56` / `index-CR-yRHJq.js` demoted to
historical fallback).

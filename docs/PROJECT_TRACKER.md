...
- **Three DISTINCT anchors — never collapse them.** This tracker records no fixed value for the repository head, because documentation merges advance it continuously and any pinned value would be false within minutes:
  ...
  2. **Merged-but-unreleased application-code anchor:** `29cccddda52e1c546b4ae46be052285ec24d2116` — the **PR #114 merge commit** (a historical anchor, not a head). From this merge onwards, `main` contains application code for the Studio local-engine containment slice that has **never been built for release or deployed**.
  3. **Deployed Production source:** `7e30199` — the commit the running Production artifact was actually built from.
  Anchors 2 and 3 differ, and a live head will normally differ from both. **Divergence between them is expected and is NOT deployment drift. Never treat "merged into `main`" as "live".**
...
- [x] **Next product slice — SELECTED by Nathan: Studio / local-engine UI containment.** **MERGED / NOT RELEASED** (PR #114 merged; no deployment) — see its section below. The candidate list below is preserved for the slice AFTER this one.
...
## Studio / Local-Engine UI Containment — **MERGED / NOT RELEASED** (2026-07-26)

**Status: PR [#114](https://github.com/natanMeT/ArtValue20/pull/114) is MERGED into `main`; the slice is NOT built for release, NOT deployed to Preview or Production, and NOT live.**

- **Merge (historical application-code anchor, not the repository head):** commit `29cccddda52e1c546b4ae46be052285ec24d2116`, parents `4f4180b` (main at that moment) + `c30dff2` (the approved head). The merge was executed pinned to that exact head SHA. Resolve the *current* head live — later documentation merges advance `main` past this commit.
- **Review:** Codex round 2 reported no major issues (reviewed commit `bb8e955ef2`); **0 unresolved review threads**. The `src/` tree **at that merge commit** is byte-identical to the reviewed tree (`255bd4c7edb0aa2fbd181a1b22cb528fa6de9aee`), so exactly the reviewed code landed — the two commits after the reviewed one were documentation-only.
- **Deployment: NONE.** No Preview build, no Production build, no artifact promoted. **Production remains `476830a2-f8ea-45dc-b0ce-a71876bc48dd` / `index-BrR14XIC.js` (source `7e30199`)** and Edge `ai-gateway` **v35** is unchanged and was not redeployed.
- **Open release gate: authenticated cloud acceptance on Preview.** All browser verification to date was local/demo; the authenticated cloud path has never been exercised. **The slice must not be marked complete or live until that passes.**
- **Retained:** branch `studio/local-engine-ui-containment` @ `c30dff2` and rollback tag `pre-studio-local-engine-containment` @ `4f4180b`.
...
2. ~~Merge to `main`~~ — **DONE (merge `29cccdd`). Merging changed nothing that is live.**
3. Preview deploy and **authenticated cloud acceptance** — the runtime smoke above was local/demo only; the
   authenticated cloud path (where the engine gate is closed and the local-only modes are hidden) has **not** been
   exercised in a browser.
4. Confirm on Preview that `/studio` issues **zero** requests to any local address and that no engine terminology
   appears for an authenticated account, including in Jake's replies.
5. Confirm `/adstudio` still renders `BetaUnavailable` and Growth remains contained for an authenticated account.
6. Only then a Production deploy, reusing the Preview-accepted artifact, with the current Production retained as rollback.
...
## Next action
**Studio / Local-Engine UI Containment is MERGED but NOT RELEASED.** PR #114 merged into `main` as merge commit
`29cccddda52e1c546b4ae46be052285ec24d2116` (a historical anchor — resolve the current `origin/main` head live at
preflight). **No Preview or Production deployment has occurred**, so nothing that is live has changed: Production remains **`476830a2-f8ea-45dc-b0ce-a71876bc48dd` / `index-BrR14XIC.js`** (source
`7e30199`), Edge `ai-gateway` **v35** unchanged, schema unchanged, **no migration**.

**The open release gate is authenticated cloud acceptance on Preview** — every browser check so far was local/demo. Next
steps, in order: build and deploy a Preview from `main`; run authenticated acceptance (`/studio` issues zero local-address
requests and shows no engine terminology, including in Jake's replies; `/adstudio` still `BetaUnavailable`; Growth still
contained); only then promote the accepted artifact to Production with the current deployment retained as rollback. **Do
not mark this slice complete or live before that.**
...
The versioned roadmaps (Business OS v0.9 / AI Gateway v5.5) and the `.docx` exports are **deliberately NOT updated** by
this documentation PR — release closure will update this tracker and the roadmaps after Production acceptance. The slice
AFTER this one remains PENDING NATHAN DECISION.

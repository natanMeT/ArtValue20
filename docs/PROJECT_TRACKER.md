...
**ACCEPTED COSMETIC GAP (owner-accepted 2026-07-30, NOT a blocker):** in the **stale-tab** race — a payment recorded elsewhere after the delete control was already rendered — the screen may show the **generic** error toast instead of the specific "לחיוב הזה רשומים תשלומים…". The refusal itself is proven server-side, so **no payment can be destroyed**; only the message text is unverified end-to-end. The mechanism it depends on IS measured: PostgREST returns the raw PostgreSQL SQLSTATE verbatim in the JSON `code` field (observed as `42501` on an `anon` call over HTTP), which is what `supabase-js` exposes as `error.code`. What was never exercised is that path for `23514` / `P0002` specifically.
...
> ### ✅ FINANCE CHARGE SAFE DELETE — **CLOSED / LIVE IN PRODUCTION** (applied + Preview + owner UI QA + Production release 2026-07-30)
>
...
> | ⚠️ Not exercised | The **stale-tab refusal** (step 6 of the handover script) — the only path that shows `23514` reaching `error.code` as the specific Hebrew message. **Owner-accepted as a cosmetic gap**: the refusal is proven server-side, so no payment can be destroyed; only the toast text is unverified |
...
- **Current Production deploy: `86d5cca9-88e2-40db-9869-664cfc1567e8`** (Environment Production, branch `main`, source **`1eb7b2a`**, bundle **`index-o0xZrfkL.js`**) — **LIVE (Finance Charge Safe Delete — frontend + one additive function migration)**, deployed 2026-07-30, the accepted Preview artifact **promoted unchanged** ("Uploaded 0 files (12 already uploaded)"; local `dist/` ↔ Preview `5a9e7277` ↔ Production are **12/12 byte-identical by SHA-256**). Migrations are **13 applied of 13 tracked, none pending**; Edge `ai-gateway` v36 ACTIVE / `verify_jwt=true`, untouched. **Full evidence: the box at the top of this file — that box, not this line, is the source of truth for the current release.** *(Superseded, in order: `c45518fb-17b6-448a-b35e-cb9a3a46367d` / `index-B21Es_EZ.js` (source `d9d5bf0`, Client Profile slice 1) is now the **current rollback target**; `478e4d62` / `index-PcQFaAu-.js` (UI Polish + Dashboard action card) is a **historical fallback only**, retained.)*
...
**Open items carried forward from Finance Charge Safe Delete (neither is a blocker):**
1. The **stale-tab refusal message** was never exercised — owner-accepted as a cosmetic gap. Closing it needs one
   two-tab test on a QA account: record a payment in tab B, then click delete in tab A and check whether
   "לחיוב הזה רשומים תשלומים…" appears rather than the generic error toast.
2. **No authenticated Production smoke** was run for this release, unlike the previous three.

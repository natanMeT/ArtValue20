...
- **Active application release code anchor:** **`45243b53e7d5d0b1c09d6cccc2e616ce15ee8534`** (PR #204 merge — Asset Library slice 4, user file upload; the running Production artifact was built from this commit and carries every earlier slice's behaviour forward). ⚠️ **CORRECTED 2026-08-05: this line had stood at `8882461731c90f06fca0fc7fb44b5f65e8e20873` (PR #191, Jake Context Truthfulness) through several later releases — the same stale-anchor class this file keeps catching.** *(Historical anchors, retained only as the sources of older deployments: `521b7a0b` — Jake Campaigns Context, now the **current rollback target's** source; `e3555eed` — Jake sees the calendar, now a historical fallback's source; `8882461731c90f06fca0fc7fb44b5f65e8e20873` — PR #191; `c138f852` — Accessibility slice A5.)*
...
- **Current Production deploy: `378115bc`** (Environment Production, branch `main`, source **`45243b53e7d5d0b1c09d6cccc2e616ce15ee8534`**, bundle **`index-BiwDxmw5.js`**, SHA256 `12948DE215DF6E6B0A5AB46392696A330D728324DCC28A640C01D9ECD89D0D83`, **794,709 B**) — **LIVE (Asset Library slice 4 — a user can upload their own image into the durable cloud gallery; every earlier slice's behaviour is carried forward in this bundle).** Promoted **UNCHANGED** from the owner-QA'd Preview `197b2fe3` — `wrangler` reported "Uploaded 0 files (12 already uploaded)", canonical↔local `dist/` **12/12 byte-identical**, propagation correct on the **first** no-cache poll, and the unauthenticated non-mutating smoke was **9/9 HTTP 200, zero writes**. Deployed 2026-08-05. **No migration, no bucket, no Storage policy, no Edge, no package, no secret, and `src/lib/api.js` untouched.**
  - *(SUPERSEDED 2026-08-05, retained as history and NOT current. ⚠️ It IS the CURRENT ROLLBACK TARGET: )* (Environment Production, branch `main`, source **`521b7a0b9ea0852cc38b7267542fc46cbfdbe577`**, bundle **`index-CqSrIXWY.js`**, SHA256 `2535BEFC45935432841974B7C73E5FC5FE89CB5176AAF4AC593A850EE4CE2824`, **792,585 B**) — **LIVE (Jake Campaigns Context — the durable per-account business campaigns reach Jake's context as READ-ONLY data; every earlier slice's behaviour is carried forward in this bundle).** One PR: [#202](https://github.com/natanMeT/ArtValue20/pull/202) (merge `521b7a0b`, head-gated `b53ff1db`, **7 files / +653 −7**). The owner-QA'd Preview `12e0a8e4` artifact was **promoted UNCHANGED**: `wrangler` reported **"Uploaded 0 files (12 already uploaded)"**, canonical↔local `dist/` is **12/12 byte-identical by SHA-256**, and the served entry bundle hashes to the QA'd value above. Unauthenticated non-mutating Production smoke **9/9 HTTP 200, zero writes**. **NO migration, NO Edge, NO schema, NO package, NO secret** — migrations remain **16 applied of 16 tracked, NONE PENDING**; `ai-gateway` v36 ACTIVE / `verify_jwt=true`, untouched — so a **frontend rollback is COMPLETE**.
...
- **Rollback hierarchy — exactly ONE deployment may be called the current target. Do not collapse these levels.**
  1. **CURRENT frontend rollback target: `af65f5fe`** (source `521b7a0b9ea0852cc38b7267542fc46cbfdbe577`, bundle `index-CqSrIXWY.js`, 792,585 B, Jake Campaigns Context; git tag `pre-asset-user-upload` @ `df9cfae1ea0546fc035237ceb845ff660cb413b2`, pushed). **HTTP 200 confirmed AFTER the Asset Library slice 4 deploy, still serving its own bundle `index-CqSrIXWY.js`** — verified, not assumed.
     - *(HISTORICAL FALLBACK ONLY — **DEMOTED 2026-08-05** by the Asset Library slice 4 release, retained and NOT the current target (§22: exactly one current target). **HTTP 200 confirmed after that deploy, still serving `index-CBCh_jpO.js`**: )* **`123ea5aa`** (source `e3555eedc691ef2f37cf5ad68db10d6cb9f40c56`, bundle `index-CBCh_jpO.js`, 790,830 B, Jake sees the calendar + its stale-read fix; git tag `pre-jake-campaigns` @ `885e8093bf431cdb8d260514962689542ada08bf`, pushed). **HTTP 200 confirmed AFTER the Jake Campaigns deploy, still serving its own bundle `index-CBCh_jpO.js`** — verified, not assumed. ⚠️ **Rolling back to it removes campaigns from Jake's context entirely — he returns to having NO campaign data AND NO declaration of absence, so an ungrounded answer becomes possible again for "מה קורה עם הקמפיינים שלי".** ⚠️ **A frontend rollback here is COMPLETE** — this release added no database object, no API, no Edge, no package and no secret.

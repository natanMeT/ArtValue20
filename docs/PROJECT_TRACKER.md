...
> ### ✅ MODAL STACKING FIX — THE DIALOG PAINTS ABOVE THE APP CHROME — **CLOSED / LIVE IN PRODUCTION** (merge + owner QA + promotion)
>
...
> 1. **The same `{cond && <motion.*>}`-inside-one-wrapper shape exists elsewhere and was NOT changed here.** Measured, not assumed: **`src/components/ai/Assistant.jsx` × 3** (`ai-bubble` at 982, `agent-reminder` at 1010, and the `ai-scrim` + `ai-panel` pair at 1038) and **`src/components/ai/DemoMode.jsx` × 1** (`demo-overlay` at 128). ⚠️ **`DemoMode` is a FULL-SCREEN overlay, so it carries the same exposure as the Modal did.** *(The hand-off to this session said Assistant × 2; the measured count is 3.)* A small follow-up slice, deliberately out of scope here — this fix stayed at one file.
...
> 3. **Accessibility — form controls without a label.** A crude scan over `src/**/*.jsx` counted **153** `<input>` / `<select>` / `<textarea>` carrying neither an `id` nor an `aria-label` (the same order as the ~156 measured on 2026-08-02; the two scans differ in method, not in conclusion). This is a scan, **not an audit**.
...
## Baseline (current — MODAL STACKING FIX CLOSED / LIVE in Production, deployment `56d9cd56`)
...
- **Active application release code anchor:** **`a9f96eeb3a6ebdbc88b72d836cd9be7c9518b5fc`** (PR #177 merge — the Modal Stacking Fix; the running Production artifact was built from this commit. The fix itself entered `main` at `a855352`). *(Historical anchors, retained only as the sources of older deployments: `d1dcc64b` — Schedule slice 2, now the **current rollback target's** source; `ba760451` — Monthly Plan, now the historical fallback's source; `75a8a167` — Asset Library slice 3, now the **current rollback target's** source; `934722f4` — Video Tab Truthfulness, now the historical fallback's source; `87fed4b` — Asset Library slice 1; `2c8b1df` — complete local-engine retirement; `03c23c2` — Studio containment; `7e30199` — P1; `983f4899` — S0F.1.)*
- **Three DISTINCT anchors — never collapse them.** This tracker records no fixed value for the repository head, because documentation merges advance it continuously and any pinned value would be false within minutes:
  ...
  3. **Deployed Production source:** **`a9f96eeb`** — the commit the running Production artifact was actually built from (Modal Stacking Fix, deployment `56d9cd56`). The prior `d1dcc64b` (Schedule slice 2, deployment `4294aba9`) is now the **current rollback target's** source, and `ba760451` (Monthly Plan, deployment `90868363`) the historical fallback's.
...
- **Current Production deploy: `56d9cd56`** (Environment Production, branch `main`, source **`a9f96eeb`**, bundle **`index-CR-yRHJq.js`**, SHA256 `21e73104121cf251e903fce65d890caa5ba4d92052fc918829976e9b47bf029b`, 781,472 B) — **LIVE (Modal Stacking Fix — the dialog is portalled to `document.body` and now paints above the topbar and the sidebar).** Re-measured live at close-out by the Layer A checker: canonical **HTTP 200**, serving `index-CR-yRHJq.js` at **781,472 bytes**. ⚠️ **This release added NO database object, no API and no Edge change, so a frontend rollback to `4294aba9` is COMPLETE — and it also restores the stacking defect, which was live in every earlier release.**
  - *(SUPERSEDED 2026-08-02, retained as history and NOT current. ⚠️ It IS the CURRENT ROLLBACK TARGET: )* **`4294aba9-e244-4531-bcbd-1a75522f6515`** (Environment Production, branch `main`, source **`d1dcc64b`**, bundle **`index-vlblhC-w.js`**, SHA256 `a7c36219841cebacc8e74db22dbdf145c9ad8865b4109a0db8257773cd569cdc`, 781,441 B) — **was LIVE (Schedule slice 2 — the read-only month grid on `/schedule`). Promoted UNCHANGED from the owner-QA'd Preview `df4e95af`: "Uploaded 0 files (12 already uploaded)", 12/12 byte-identical before AND after**, and the canonical URL was polled cache-busted until **10/10** consecutive requests served the new bundle.
  - *(SUPERSEDED 2026-08-02, retained as history and NOT current. ⚠️ It is the HISTORICAL FALLBACK: )* **`90868363-fd44-4f4a-b6c7-d1197676e6cd`** (Environment Production, branch `main`, source **`ba760451`**, bundle **`index-BAajglJn.js`**, SHA256 `238d2c8424e628f491181560a422c2875ee0539bd738b16f0a83cee31d30e214`, 778,331 B) — **LIVE (Monthly Plan `/plan` — read-only account-aware planner). Promoted UNCHANGED from the owner-QA'd Preview `6c669c39`: "Uploaded 0 files (12 already uploaded)", 12/12 byte-identical before AND after.** ⚠️ **This release added NO database object, so a frontend rollback to `be068244` is COMPLETE.**
...
- **Rollback hierarchy — exactly ONE deployment may be called the current target. Do not collapse these levels.**
  1. **CURRENT frontend rollback target: `4294aba9-e244-4531-bcbd-1a75522f6515`** (source `d1dcc64b`, bundle `index-vlblhC-w.js`, Schedule slice 2 — the read-only month grid; git tag `pre-modal-stacking-promotion` @ `d1dcc64`, pushed). ⚠️ **A frontend rollback of the live release is COMPLETE** — the Modal Stacking Fix added no database object, no API and no Edge change. ⚠️ It does **NOT** undo migration `20260804120000` (Asset Library slice 3), which is additive and stays applied. ⚠️ **Rolling back also RESTORES the stacking defect** — the modal would again paint under the topbar and the sidebar, in a short window. That is the trade, and it should be a deliberate choice, not a surprise.
  1. *(HISTORICAL FALLBACK — demoted 2026-08-02, retained, NOT the current target; §22 requires exactly one: )* **`90868363-fd44-4f4a-b6c7-d1197676e6cd`** (source `ba760451`, bundle `index-BAajglJn.js`, Monthly Plan `/plan`; git tag `pre-schedule-month-promotion` @ `ba76045`, pushed).
  2. **HISTORICAL FALLBACK ONLY — NOT the current target: `be068244-87c0-4202-b86b-f9a0ae41a671`** (source `75a8a167`, bundle `index-C0J4dBZV.js`, Asset Library slice 3 — Campaign Link; git tag `pre-monthly-plan-promotion` @ `75a8a167`, pushed). Older fallbacks (`6f0a20df`, `19eeb73d`, `c2bb560e`, `88b20584`) are retained further down as history only.
  3. **Git rollback tag: `pre-modal-stacking-promotion` @ `d1dcc64` — created AND pushed** (confirmed on `origin`: `refs/tags/pre-modal-stacking-promotion` → `d1dcc64bed843971489cb61e2b3c349277f45728`). **No Edge rollback applies to this release — `ai-gateway` v36 was NOT redeployed.** **The live release added NO migration**, so there is nothing left applied behind a frontend revert. Earlier tags retained: `pre-schedule-month-promotion` @ `ba76045`, `pre-monthly-plan-promotion` @ `75a8a167`, `pre-campaign-link-promotion` @ `934722f4`, `pre-ui-polish-action-card` @ `e80a1341`.
...
- **Test evidence for the CURRENT release (Modal Stacking Fix, source `a9f96eeb`, deployment `56d9cd56`, bundle `index-CR-yRHJq.js`):** full suite **143 files / 4,000 passed / 0 failed**, re-run on the merged `main` at close-out (baseline before the slice: 142 / 3,993). **NO migration — no database object, no API, no Edge and no Storage was touched; 16 applied of 16 tracked, none pending.** Two files changed, one of them the new guard `src/components/ui/__tests__/modalStacking.test.js`. ⚠️ **What the suite CANNOT prove, stated rather than implied:** jsdom has no layout and no compositing, so **no test can assert that the overlay actually paints above the topbar.** The load-bearing evidence is the `elementFromPoint` hit-test in a real browser and the owner's QA; the test file guards the *structure* that produced it (the portal target), not the pixels.
...
## Next action
**Nothing is mid-flight. As of 2026-08-02 there is no undeployed slice and no release gate awaiting an owner.**
Production is `56d9cd56` / `index-CR-yRHJq.js` (source `a9f96eeb`, Modal Stacking Fix), migrations are **16 applied of
16 tracked, none pending**, Edge `ai-gateway` v36 is ACTIVE / `verify_jwt=true`, and the current rollback target is
`4294aba9` / `index-vlblhC-w.js`.

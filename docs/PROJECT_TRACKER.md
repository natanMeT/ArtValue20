...
> 4. ⏳ Build / deploy — a later, separately-approved step, **NOT taken**. The Finance screen is user-visible, so
>    unlike Campaigns slice 2 this slice **will** eventually need one. Until then the tables exist and no user can reach them.
>
...
> (10 applied of 10 tracked, none pending *at that moment*; today **11 of 11, none pending**), so `public.tasks.campaign_id` and the composite FK now exist in the
...
## Status ledger
- **F1 Core Receivables slice 1 (charges, payments, five same-owner composite keys)** — **MIGRATION APPLIED / PART A + PART B VERIFIED LIVE / NOT RELEASED (2026-07-30).**
  - **Chain so far:** PR [#134](https://github.com/natanMeT/ArtValue20/pull/134) → merge `56d13ef` (head-gated to `c811fac`) → ❌ apply attempt 1 failed (`42601`, reserved word `notnull`) → PR [#136](https://github.com/natanMeT/ArtValue20/pull/136) → merge `f442355` (head-gated to `7f4e537`) → ❌ apply attempt 2 failed (`55000`, `record "c" is not assigned yet`; **full rollback verified**) → PR [#137](https://github.com/natanMeT/ArtValue20/pull/137) → merge `601b3c9` (head-gated to `590d987`) → ✅ **migration `20260731120000` APPLIED — 11 applied of 11 tracked, dry-run clean** → ✅ **PART A matched exactly** → ✅ **PART B nine controls decisive, QA residue zero** → ⏳ build/deploy **NOT taken**.
  ...
  - **✅ The database now has all of it.** Migration applied and verified live 2026-07-30 (11 of 11, dry-run clean); PART A matched exactly and all nine PART B controls were decisive on disposable QA records, with cleanup verified zero per-account and RLS-independently. **⚠️ Still NOT released:** the frontend is merged but not built or deployed, so no user can reach it. See the ✅ box at the top for the full evidence and for the two failed applies.
...
## Next action
**F1 Core Receivables slice 1 has its migration APPLIED and PART A + PART B VERIFIED LIVE, and is STILL NOT RELEASED —
see the ✅ box at the top of this file.** Feature PR [#134](https://github.com/natanMeT/ArtValue20/pull/134) merged as
...
**The next step is a SEPARATE owner approval, and it is not implied by the apply:**
1. ⏳ **Approve a build + Preview + Production deploy for the Finance receivables area.** Unlike Campaigns slice 2 this
   slice **is** user-visible, so it needs one before it is genuinely released. Until then the tables exist, the code is
   on `main`, and **no user can reach the feature** — Production still serves the pre-F1 bundle.
2. ⏳ Then a QA-account **Preview acceptance** run and a non-mutating **Production smoke**, per the pattern the last
   three releases used.

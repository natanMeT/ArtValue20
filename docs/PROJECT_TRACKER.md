...
> ### ✅ ASSET LIBRARY SLICE 2 — FAVORITES — **CLOSED / LIVE IN PRODUCTION** (merge + migration applied & verified live + Preview QA + Production promotion)
>
...
- **Three DISTINCT anchors — never collapse them.** This tracker records no fixed value for the repository head, because documentation merges advance it continuously and any pinned value would be false within minutes:
  ...
  3. **Deployed Production source:** **`272bc5ec`** — the commit the running Production artifact was actually built from (Asset Library slice 2 — Favorites, deployment `19eeb73d`). The prior `5bea9ee` (Campaign Delete Safety, deployment `c2bb560e`) is retained as the **current rollback deployment's** source. *(⚠️ This anchor read `87fed4b` — Asset Library slice 1 — for many releases; corrected 2026-07-31. Same stale-block class as the Production line and the migration count.)*
...
- [ ] **Slice after this one — PENDING NATHAN DECISION.** Do NOT begin/design/invent the next slice until Nathan selects one and approves a spec. Candidate open items: **account-aware Growth data model** (the last of the three Growth reopening prerequisites); **Asset Library beyond gallery images** (approve/reject, usage rights, versions, campaign association, logo/brand assets — **favourites is DONE and LIVE, do not re-select it**); **video generation** (a GENERATION-lane slice, not a storage one — animated WebP is already an allowed MIME; what blocks it is that the cloud lane cannot produce video at all and no managed provider has been chosen. The permanently-empty cloud וידאו tab is the visible symptom); **the authenticated Production smoke for Asset Library slice 2** (**still never run** — needs an owner sign-in; the unauthenticated smoke passed and Preview QA covered the behaviour on byte-identical bytes, so this is a coverage gap, not a suspected defect); **the cloud video tab truthfulness gap** (the gallery renders a וידאו tab in cloud mode that can never be non-zero — see the video-generation entry above; the fix is either to hide the tab or to ship the generation lane, and that is a product choice); **Products / Projects / Inventory / Templates / Activity durability**; **organization boundaries**; **credits / cost controls**; **Website Scanner** (per the section above); Jake conversation-refresh UX. **NOT candidates — already done, and selecting either would mean re-implementing merged work:**

...
> * **The `!userId` silent branch.** It reduces optimistically then returns `{ ok: false }` with **no restore at all and no toast** — fully silent. Untouched by explicit decision.
>
> ---
>
...
- **Rollback hierarchy — exactly ONE deployment may be called the current target. Do not collapse these levels.**
  1. **CURRENT frontend rollback target: `0fa5c3ea`** (source `6635f3d6f44b430b2d32c95b620d7ec02307761d`, bundle `index-04boVitS.js`, Bulk Delete False-Success; git tag `pre-stale-list-race-promotion` @ `6635f3d6f44b430b2d32c95b620d7ec02307761d`, pushed). HTTP 200 confirmed after the Stale-List Refetch Race deploy, still serving its own bundle. ⚠️ **Rolling back to it restores the stale-list refetch race.**

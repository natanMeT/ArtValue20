...
| סטטוס נוכחי | Jake, Outreach, Diagnose ו־ImageStudio נשארים LIVE — VERIFIED. **P1 — Atomic Quote Persistence ו־Studio / Local-Engine UI Containment נסגרו LIVE בפרודקשן, שניהם ללא שינוי Gateway/Edge כלשהו. ai-gateway נשאר v35 ACTIVE עם verify_jwt=true ולא נפרס מחדש בשני הסלייסים.** router, actionTypes, contracts, payloads, provider routing, ולידציה, תקציב/usage וכל הפרופילים — ללא שינוי. ה־Business Context של החשבון ובלוק פלטת המותג ממשיכים להיות מורכבים ומוזרקים על ידי ה־frontend לפני קריאת ה־Gateway הקיימת. הסלייס הבא ממתין להחלטת Nathan. |
...
| P1 — Atomic Quote Persistence | CLOSED / LIVE VERIFIED — PRs #108 → #109 → #110 → #111 → docs #112; active release source 7e30199; **ZERO Gateway/Edge change**; ai-gateway v35/JWT-on unchanged and NOT redeployed. One additive migration (`20260726120000_atomic_quote_persistence.sql`) created `public.save_quote_atomic` — a **database** RPC called directly by the frontend through Supabase, **not** a Gateway actionType. Router, action types, contracts/payloads, provider routing, validation, budgets/usage controls and every profile are UNCHANGED. |
| --- | --- |

| Studio / Local-Engine UI Containment | CLOSED / LIVE VERIFIED — PR #114 → 29cccdd → docs PR #115 → active release source 03c23c2; **ZERO Gateway/Edge change**; ai-gateway v35/JWT-on unchanged and NOT redeployed; **no migration**. Frontend-only removal of user-facing local-engine/provider/model complexity. The hosted `studio.generate_image` / `studio.prompt_enhance` lanes and their contracts are untouched; the slice **removed** three mount-time local-engine discovery requests and replaced them with fail-closed configuration-derived capability flags. |
| --- | --- |
...
| Edge Function | LIVE GREEN | ai-gateway ב־Supabase עם JWT verification ON. **גרסה v35** (נפרסה ב־S0F.1 עם טקסט פרסונה בלבד; **P1 ו־Studio Containment לא שינו ולא פרסו אותה מחדש**). |
...
| Atomic quote persistence (P1) | LIVE — VERIFIED (database RPC, לא Gateway) | `public.save_quote_atomic` שומר הצעת מחיר ואת שורותיה **בטרנזקציה אחת**. זו קריאת RPC ישירה ל־Supabase מה־frontend — **אינה actionType של ה־Gateway ואינה עוברת דרכו**. SECURITY INVOKER עם `search_path` ריק; **`authenticated` הוא תפקיד ה־EXECUTE היחיד הפונה ללקוח — `anon` ו־PUBLIC נדחים (אומת `42501` / HTTP 401 מול HTTP אמיתי)**. אין שינוי חוזה, adapter או פרופיל ב־Gateway. |
| Studio local-engine discovery (Containment) | REMOVED — VERIFIED (frontend) | שלוש בקשות גילוי מנוע מקומי שרצו בעליית הסטודיו (`/object_info/CheckpointLoaderSimple`, בדיקות PuLID ו־Qwen) **הוסרו** והוחלפו בדגלי יכולת נגזרי־קונפיגורציה שנכשלים סגור. בפרודקשן מאומת נמדדו **0 בקשות בפתיחת הסטודיו, 0 בהמתנת 17 שניות ו־0 בקשות לכתובת מקומית**. **חוזי ה־Gateway של `studio.generate_image` ו־`studio.prompt_enhance` לא השתנו כלל.** |
...
| Release gate | **Studio / Local-Engine UI Containment CLOSED / LIVE VERIFIED** בפרודקשן על עוגן קוד ההשקה הפעיל 03c23c23568905cb42e7f154014dd2ddc32bb58f (לפניו: P1 על 7e30199, S0F.1 על 983f4899, S0E על 272fc14). **שני הסלייסים האחרונים לא שינו את ה־Edge כלל — ai-gateway נשאר v35, ACTIVE עם verify_jwt=true, ולא נפרס מחדש.** router / actionTypes / חוזים / payloads / provider routing / ולידציה / תקציב ו־usage / כל הפרופילים — ללא שינוי. כל מסלולי ה־Gateway שכבר נסגרו נשארים LIVE — VERIFIED; אין לפתוח אותם מחדש ללא ראיה חדשה. |
| --- | --- |
...
| Release state | Production online. **P1 ו־Studio / Local-Engine UI Containment — CLOSED / LIVE VERIFIED**; ai-gateway **v35 ACTIVE/JWT-on — נפרס לאחרונה ב־S0F.1 ולא נגע בו אף אחד משני הסלייסים האחרונים**. Jake, Outreach, Diagnose ו־ImageStudio נשארים LIVE — VERIFIED. יעד ה־rollback הנוכחי היחיד הוא 476830a2. |
...
| D-037 | **`public.save_quote_atomic` (P1) הוא RPC של בסיס הנתונים ולא משטח Gateway.** הוא נקרא ישירות מה־frontend דרך Supabase, אינו actionType, אינו עובר דרך ה־Edge Function ואינו משנה חוזה, adapter, ניתוב, ולידציה, תקציב או פרופיל כלשהו. אבטחתו נשענת על SECURITY INVOKER עם `search_path` ריק ועל RLS: **`authenticated` הוא תפקיד ה־EXECUTE היחיד הפונה ללקוח; `anon` ו־PUBLIC נדחים.** אין להסיק ממנו עבודת Gateway כלשהי, ואין להעביר שמירת הצעות מחיר דרך ה־Gateway ללא סלייס נפרד מאושר. | Locked |
| D-038 | **Studio / Local-Engine UI Containment הוא סלייס frontend בלבד, ללא שינוי Gateway/Edge ו־ללא migration.** הוא **מסיר** גילוי מנוע מקומי מצד הלקוח ומחליף אותו בדגלי יכולת נגזרי־קונפיגורציה **שנכשלים סגור** — יכולת אופציונלית נשארת בלתי־זמינה אלא אם הוצהרה במפורש; קונפיגורציה חסרה, לא ידועה או חלקית לעולם אינה חושפת מסלול. חוזי `studio.generate_image` ו־`studio.prompt_enhance` לא השתנו. **הסלייס אינו החלפת ספק ולא הוכנס בו ספק image/video מנוהל חדש**; בחירת ספק כזה תידרש כסלייס נפרד עם scope, ראיות ואישור Nathan. נרשם מעקב אמת לא־חוסם: ג׳יק עדיין מונה יכולות קריאייטיב שהמסלול הענני אינו יכול להריץ (`systemCapabilities()` מונה כל workflow חי ללא תלות בזמינות מנוע) — קדם לסלייס, אינו רגרסיה, ואינו פריט Gateway. | Locked |
...
| שער נוכחי | ai-gateway **v35** ACTIVE עם verify_jwt=true — **ללא פריסה מחדש בשני הסלייסים האחרונים**. Jake, Outreach, Diagnose ו־ImageStudio LIVE — VERIFIED ומוקפאים. אין שינוי נוסף ללא ראיה והיקף מאושר. |
| --- | --- |
...
| Closure target | ACHIEVED — P1 and Studio / Local-Engine UI Containment released and verified in Production **with the Edge untouched**; ai-gateway remains v35/JWT-on from the S0F.1 persona deploy; all previously released AI lanes remain LIVE — VERIFIED with unchanged contracts. |

## Change Log

| גרסה | תאריך | שינוי | עורך |
| --- | --- | --- | --- |
| 5.6 | 26.07.2026 | **P1 — Atomic Quote Persistence and Studio / Local-Engine UI Containment BOTH CLOSED / LIVE VERIFIED with ZERO Gateway/Edge change.** ai-gateway remains **v35, ACTIVE, verify_jwt=true — not redeployed in either slice**; router, action types, contracts, request/response payloads, provider routing, input validation, budget/usage controls, confirmation behavior and every profile are UNCHANGED, and no secret or configuration changed. **P1:** PRs #108 → #109 → #110 → #111 → docs #112 merged at main 7e30199; migration `20260726120000_atomic_quote_persistence.sql` APPLIED and verified; `public.save_quote_atomic` writes a quote and its line items in **one transaction**. It is a **database RPC called directly through Supabase — explicitly NOT a Gateway actionType** (D-037). SECURITY INVOKER with an empty `search_path`; **`authenticated` is the only client-facing EXECUTE role — `anon`/PUBLIC denied, proven `42501`/HTTP 401 over real HTTP**. Failure-injection acceptance **13/13** (fault injected after the parent insert → no partial quote; failed replacement left the original items and parent intact; foreign-owner update `P0002` and invisible under RLS); Preview `c999988e` UI acceptance showed exactly one RPC (HTTP 204) per save; Production frontend `476830a2` serves index-BrR14XIC.js. Tests 120 files / 3,065 passed. **Studio Containment:** PR #114 merged at 29cccdd, docs PR #115 → **03c23c2 = the current Production source**; **one build only**, promoted to Production without rebuilding (wrangler "Uploaded 0 files (12 already uploaded)", **12/12 served files byte-identical**); Preview `ec239e3b` authenticated QA acceptance PASS; Production **`247ef9ec` / index-BZ3B-0yd.js** + authenticated non-mutating Account A smoke PASS. It **removed three mount-time local-engine discovery requests** and replaced them with fail-closed configuration-derived capability flags, and removed every user-facing engine name — including the workflow-catalog text that reaches Jake's system prompt (verified live: a 1,174-character Jake reply with **0 engine names**). Measured in Production: **0 requests on opening the Studio, 0 during a 17-second idle, 0 local-address requests, 0 engine terms, 0 console errors, 0 mutating requests.** **No migration for this slice.** Test scope, stated exactly: the full suite (121 files / 3,098 passed) was green on the **earlier implementation head**; the **focused affected suite** (27 files / 1,608 passed) ran on the **final corrected code**, with the full suite deliberately not rerun on that head; production build green at `03c23c2`. Decisions D-037 and D-038 recorded. Rollback hierarchy corrected to a single current target (`476830a2`), with `e63198b7` and earlier as historical fallbacks only. | ChatGPT |
...
> הסעיפים הבאים נשמרים כארכיון של נקודות המעבר v4.4–v5.6 לצורך רצף היסטורי. הם אינם מקור האמת הנוכחי; המצב הפעיל מתואר ב־Current Control Panel וב־Change Log לעיל. **הערת עריכה:** תמצית התוצאה, ה־SHA והסטטוס של כל נקודת מעבר נשמרו; בלוקי ה־prompt התפעוליים החד־פעמיים לפייבל (audit/יישום/release) ופִסקאות ה־״פתיח לסשן חדש״ שהופיעו במקור v5.2 קוצרו כאן ולא שוכפלו מילה־במילה, מכיוון שהם הוראות עבודה חד־פעמיות שכבר בוצעו ונסגרו — לא החלטות, סיכונים או תוכן roadmap. כל ההחלטות (D-001…D-038), היסטוריית הסטטוס, הסיכונים ויומן השינויים המלא (1.0…5.6) שמורים לעיל.
...
## עדכון נקודת מעבר v5.6 — P1 ATOMIC QUOTE PERSISTENCE + STUDIO LOCAL-ENGINE UI CONTAINMENT CLOSED / LIVE VERIFIED (הערת אי־שינוי ל־Gateway)

**התוצאה:** שני סלייסי מוצר נסגרו ונפרסו בפרודקשן **בלי לגעת ב־Gateway או ב־Edge כלל**. ai-gateway נשאר **v35, ACTIVE, `verify_jwt=true`**, ולא נפרס מחדש באף אחד מהם. router, actionTypes, חוזים, payloads, provider routing, ולידציה, תקציב/usage, זרימת האישור וכל הפרופילים — ללא שינוי; לא שונה שום secret או קונפיגורציה.
...
**היררכיית rollback — יעד נוכחי יחיד.** `476830a2` (P1) הוא יעד ה־rollback הנוכחי. `e63198b7` (S0F.1) ומוקדמים ממנו הם **נפילה־אחורה היסטורית בלבד**; חזרה אליהם הייתה מבטלת גם את P1.
...
**גבולות אי־ההסקה:** אין להסיק או לתזמן עבודת Gateway כלשהי משני הסלייסים האלה. **Studio Containment הוא ניקוי והכלה — לא החלפת ספק**; לא נבחר ולא שולב ספק image/video מנוהל חדש, וכל בחירה כזו תידרש כסלייס נפרד עם scope, ראיות ואישור Nathan. מעקב אמת לא־חוסם שנרשם: ג׳יק מונה יכולות קריאייטיב שהמסלול הענני אינו יכול להריץ — קדם לסלייס, אינו רגרסיה, ואינו פריט Gateway.
